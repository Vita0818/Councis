...
public struct CouncisComposerDraftAttachment: Identifiable, Equatable, Sendable {
...
public struct CouncisComposerAttachmentFile: Equatable, Sendable {
    public var name: String
...
public enum CouncisComposerAttachmentFileReader {
    public static func read(_ url: URL) throws -> CouncisComposerAttachmentFile {
        ...
        #endif
        return CouncisComposerAttachmentFile(
...
public enum CouncisComposerAttachmentResolutionError: LocalizedError, Sendable {
...
public struct CouncisComposerAttachmentStore: Sendable {
    ...
    public func preserve(
        _ file: CouncisComposerAttachmentFile
    ) async throws -> CouncisComposerDraftAttachment {
        ...
        guard verifiedRef == ref,
            ...
            throw CouncisError.io("attachment read-back verification failed")
        }
        try Task.checkCancellation()
        return CouncisComposerDraftAttachment(
            id: ref.id,
            name: file.name,
            mime: file.mime)
    }
    ...
    public func imageAttachments(
        ...
        for id in ids {
            ...
            guard let ref = await store.ref(for: id) else {
                throw CouncisComposerAttachmentResolutionError.missing(id)
            }
            guard ref.mime.hasPrefix("image/") else {
                throw CouncisComposerAttachmentResolutionError.unsupported(
                    id,
                    mime: ref.mime,
                    surface: surface)
            }
            ...
            do {
                ...
                throw CouncisComposerAttachmentResolutionError.unreadable(
                    id,
