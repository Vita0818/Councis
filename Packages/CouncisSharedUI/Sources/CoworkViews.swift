...
private func councisNormalizedStatus(_ status: String) -> String {
    status
...
private func councisLocalizedRawStatus(_ status: String) -> String {
    switch councisNormalizedStatus(status) {
    case "idle": return CouncisLocalization.string("idle")
    case "active": return CouncisLocalization.string("active")
    case "running": return CouncisLocalization.string("running")
    case "thinking": return CouncisLocalization.string("thinking")
    case "tool": return CouncisLocalization.string("tool")
    case "assigned": return CouncisLocalization.string("assigned")
    case "paused": return CouncisLocalization.string("paused")
    case "pending": return CouncisLocalization.string("pending")
    case "queued": return CouncisLocalization.string("queued")
    case "ready": return CouncisLocalization.string("ready")
    case "mailbox": return CouncisLocalization.string("mailbox")
    case "completed", "complete", "done":
        return CouncisLocalization.string("completed")
    case "failed": return CouncisLocalization.string("failed")
    case "error": return CouncisLocalization.string("error")
    case "rejected": return CouncisLocalization.string("rejected")
    case "blocked": return CouncisLocalization.string("blocked")
    case "budget_limited": return CouncisLocalization.string("budget limited")
    case "usage_limited": return CouncisLocalization.string("usage limited")
    case "in_progress", "inprogress":
        return CouncisLocalization.string("in progress")
    case "cancelled", "canceled":
        return CouncisLocalization.string("cancelled")
    case "detached", "removed", "cleaned":
        return CouncisLocalization.string("detached")
    default: return status
    }
}

private func councisLocalizedDisplayStatus(_ status: String) -> String {
    switch councisNormalizedStatus(status) {
    case "idle": return CouncisLocalization.string("Idle")
    case "active": return CouncisLocalization.string("Active")
    case "running": return CouncisLocalization.string("Running")
    ...
    default:
        let display = status
            ...
            .capitalized
        return CouncisLocalization.string(display)
    }
}

private func councisLocalizedAgentRole(_ role: String) -> String {
    switch role.lowercased() {
    case "main": return CouncisLocalization.string("main")
    case "coordinator": return CouncisLocalization.string("coordinator")
    case "worker": return CouncisLocalization.string("worker")
    case "reviewer": return CouncisLocalization.string("reviewer")
    default: return role
    }
}
...
public struct CoworkAgentInfo: Identifiable, Equatable, Sendable {
    ...
    public var statusLine: String {
        ...
        if queued > 0 {
            return CouncisLocalization.format(
                "%@ · %lld queued",
                councisLocalizedRawStatus(status),
                Int64(queued))
        }
        if completedTasks > 0 {
            return CouncisLocalization.format(
        ...
        return councisLocalizedRawStatus(status)
    }
    ...
    public var inferenceDisplayLabel: String? {
        ...
        case .unresolved:
            return CouncisLocalization.string("Inference unavailable")
        case .incompatible:
            return CouncisLocalization.string("Inference incompatible")
        ...
        if !components.isEmpty {
            ...
            return inferenceResolution == .legacy
                ? CouncisLocalization.format("Legacy · %@", label)
                : label
        }
        return inferenceResolution == .resolved
            ? CouncisLocalization.string("Inference resolved")
            : CouncisLocalization.string("Legacy inference")
    }
...
private enum CoworkInspectorTab: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .agents: return CouncisLocalization.string("Agents")
        case .tasks: return CouncisLocalization.string("Tasks")
        case .context: return CouncisLocalization.string("Context")
        }
    }
}
...
public struct CoworkProjectInfo: Equatable, Sendable {
    ...
    public init(sessionID: String = "",
                ...
                defaultModel: String = CouncisLocalization.string("current model"),
                defaultPermission: String = CouncisLocalization.string("reviewed"),
    ...
    func body(content: Content) -> some View {
        content.accessibilityValue(
            selection.selectedAgentID == agentID
                ? CouncisLocalization.string("Selected")
                : CouncisLocalization.string("Not selected"))
    }
}
...
public struct CoworkShell: View {
    ...
    private let presentationScope: CouncisThreadPresentationScope
    ...
    private let threadStyle: CouncisThreadStyle
    ...
    private let headerActions: [CouncisThreadHeaderAction]
    ...
    @StateObject private var scrollCoordinator = CouncisThreadScrollCoordinator()
    ...
    public init(threadPage: CoworkAgentThreadPage,
                ...
                sessionTitle: String = CouncisLocalization.string("Cowork"),
                ...
                threadStyle: CouncisThreadStyle = .standard(.light),
                splitLayout: CouncisSplitColumnLayout = .workspace,
                ...
                composerTrailingAction:
                    CouncisThreadComposerSecondaryAction? = nil,
                headerActions: [CouncisThreadHeaderAction] = [],
    ...
    private var threadErrors: [CouncisThreadErrorEntry] {
        CouncisThreadErrorPresentation.errors(
    ...
    private func content(outerSize: CGSize) -> some View {
        ...
        let inspectorLayout = CouncisCoworkStatusRailLayoutPolicy.resolve(
        ...
        let primaryScrollerClearance: CGFloat = inspectorLayout.isVisible
            ? CouncisCoworkStatusRailLayoutPolicy.scrollerClearance
            : 0
    ...
    private func threadColumn(layout: CouncisThreadContentLayout,
    ...
    private func resolvedHeaderActions(
        ...
        if showsCompactActions {
            if let onShowProjectSettings {
                actions.append(CouncisThreadHeaderAction(
                    title: CouncisLocalization.string("Project"),
            ...
            if let onAddAgent {
                actions.append(CouncisThreadHeaderAction(
                    title: CouncisLocalization.string("Attach"),
        ...
        let inspectorTitle = permissionPinsInspector
            ? CouncisLocalization.string("Permission Review")
            : inspectorIsVisible
                ? CouncisLocalization.string("Hide Inspector")
                : CouncisLocalization.string("Show Inspector")
        actions.append(CouncisThreadHeaderAction(
    ...
    private var inspectorColumn: some View {
        ScrollView {
            ...
            VStack(
                alignment: .leading,
                spacing: CouncisCoworkStatusRailLayoutPolicy.cardSpacing
            ) {
            ...
            .frame(width: CouncisCoworkStatusRailLayoutPolicy.cardWidth)
            .padding(
                .leading,
                CouncisCoworkStatusRailLayoutPolicy.leadingInset)
            .padding(
                .trailing,
                CouncisCoworkStatusRailLayoutPolicy.trailingInset)
            .padding(.vertical, 16)
        }
    ...
    @ViewBuilder private var permissionStatusSection: some View {
        if let pending {
            PermissionCard(
                ...
                onResolve: onResolve)
                .frame(
                    width: CouncisCoworkStatusRailLayoutPolicy.cardWidth,
                    alignment: .leading)
                .councisClearLiquidGlass(cornerRadius: 22)
    ...
    private func agentStatusRowContent(_ agent: CoworkAgentInfo) -> some View {
            ...
            if agent.isAttached && agent.inferenceResolution.requiresAttention {
                Image(systemName: "exclamationmark.triangle.fill")
                    ...
                    .accessibilityLabel(agent.inferenceDisplayLabel
                        ?? CouncisLocalization.string("Inference unavailable"))
            }
        }
        ...
        .help(["\(agent.name): \(councisLocalizedRawStatus(agent.status))", agent.inferenceDisplayLabel]
            .compactMap { $0 }
            .joined(separator: " · "))
    }
                ...
                VStack(alignment: .leading, spacing: 9) {
                    HStack(alignment: .firstTextBaseline, spacing: 7) {
                        ...
                        Label(formatElapsed(goal.elapsedSeconds(at: timeline.date)), systemImage: "timer")
                            .font(.councisCaption.monospacedDigit())
                            .foregroundStyle(threadStyle.secondaryText)
                            .lineLimit(1)
                    }

                    Text(goal.objective)
                        .font(.councisCallout.weight(.semibold))
                        .foregroundStyle(threadStyle.primaryText)
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        goalMetric(
                            CouncisLocalization.string("Tokens"),
                            value: tokenSummary(for: goal))
                        goalMetric(
                            CouncisLocalization.string("Run"),
                            value: goal.currentRunOrdinal.map { "#\($0)" }
                                ?? CouncisLocalization.string("Not started"))
                    }
    ...
    private func goalMetric(_ title: String, value: String) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(CouncisLocalization.string(title).uppercased())
                .font(.councisCaption.bold())
                .foregroundStyle(threadStyle.tertiaryText)
            Text(value)
                .font(.councisCaption.monospacedDigit())
    ...
    @ViewBuilder private func goalActions(_ goal: CoworkGoalCardInfo) -> some View {
        if goal.canPause || goal.canResume || goal.canEdit || goal.canClear {
            ...
            CouncisGlassEffectGroup(spacing: 10) {
                HStack(spacing: 10) {
                    if goal.canPause {
    ...
    private func tokenSummary(for goal: CoworkGoalCardInfo) -> String {
        ...
        guard let tokenBudget = goal.tokenBudget else {
            return CouncisLocalization.format("%@ · no budget", used)
        }
    ...
    private func displayStatus(_ status: String) -> String {
        councisLocalizedDisplayStatus(status)
    }
    ...
    private func rightRailSection<Content: View>(
        ...
        VStack(alignment: .leading, spacing: 14) {
            Label(CouncisLocalization.string(title), systemImage: systemImage)
                .font(.councisTitle3.weight(.semibold))
                .foregroundStyle(threadStyle.primaryText)
            content()
        }
        .padding(18)
    ...
    private var inspectorOverview: some View {
        ...
        .padding(11)
        .frame(maxWidth: .infinity, alignment: .leading)
        .councisContentSurface(cornerRadius: 8)
    }
    ...
    @ViewBuilder private var contextInspector: some View {
        ...
        if let latestTurnStats {
            inspectorSection("Last Turn") {
                CouncisTurnStatsSummaryView(stats: latestTurnStats, style: threadStyle)
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 7) {
            inspectorRow(
                ...
                value: project.sessionID.isEmpty
                    ? CouncisLocalization.string("current")
                    : project.sessionID)
    ...
    private var inspectorHeader: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Inspector")
                .font(.councisFixed(size: 16, weight: .semibold))
                .foregroundStyle(threadStyle.primaryText)
            ...
            ForEach(summary.failedTasks) { task in
                taskCompactRow(
                    ...
                    actionTitle: onRetryTask == nil
                        ? nil
                        : CouncisLocalization.string("Retry"),
    ...
    private func statusStrip(layout: CouncisThreadContentLayout) -> some View {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 5) {
                        Text("@\(agent.name)")
                            ...
                            .foregroundStyle(threadStyle.primaryText)
                            .lineLimit(1)
                        Text(councisLocalizedAgentRole(agent.role))
                            .font(.councisCaption2.bold())
                            .foregroundStyle(threadStyle.secondaryText)
                            .lineLimit(1)
                    }
        ...
        VStack(alignment: .leading, spacing: 7) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text("@\(agent.name)")
                    ...
                    .lineLimit(1)
                Spacer(minLength: 8)
                Text(councisLocalizedRawStatus(agent.status))
                    .font(.councisCaption2.bold())
                    .foregroundStyle(statusColor(for: agent.status))
                    .lineLimit(1)
            ...
            Text(agent.workspace)
                ...
                .textSelection(.enabled)
            Divider().opacity(0.35)
            agentDetailRow("Role", value: councisLocalizedAgentRole(agent.role))
            ...
            agentDetailRow(
                "Queued",
                value: CouncisLocalization.format(
    ...
    private func taskCompactRow(task: CoworkTaskLine,
        ...
        VStack(alignment: .leading, spacing: 4) {
            HStack(alignment: .firstTextBaseline, spacing: 6) {
                ...
                Text(councisLocalizedDisplayStatus(task.status))
                    .font(.councisCaption2.bold())
    ...
    private func agentRoster(layout: CouncisThreadContentLayout) -> some View {
        VStack(alignment: .leading, spacing: 10) {
                    ...
                    LazyHStack(spacing: 8) {
                        if agents.isEmpty {
                            Text("No agents in this session")
                                ...
                                .padding(.horizontal, 12)
                                .padding(.vertical, 9)
                                .councisContentSurface(cornerRadius: 18)
                        }
                        ForEach(agents) { agent in
                            agentPill(agent)
                        }
    ...
    private func agentPill(_ agent: CoworkAgentInfo) -> some View {
        ...
        return Button {
            ...
            .padding(.horizontal, 12)
            .padding(.vertical, 8)
            .councisLiquidGlass(cornerRadius: 18, interactive: true)
        }
        .buttonStyle(.plain)
        .disabled(!agent.isConversationSelectable)
    }
    ...
    @ViewBuilder private func taskHighlights(layout: CouncisThreadContentLayout) -> some View {
            ...
            ScrollView {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        if showsVisibleThinkingIndicator {
                            CouncisThreadThinkingRow(
                                layout: layout,
                                style: threadStyle,
                                phaseID: thinkingPhaseID)
                                .id("councis-cowork-thinking-\(thinkingPhaseID)")
                        }
                        Color.clear
                            .frame(height: 1)
                            .padding(.bottom, 16)
                            .id(CouncisThreadBottomAnchorID(scope: pageScope))
                    ...
                    .environment(
                        ...
                        resolvedViewportAdmission(
                            for: pageScope))
                    .environment(
                        \.councisThreadScrollCoordinator,
                        scrollCoordinator)
                    .environment(
                ...
                .onScrollGeometryChange(
                    for: CouncisThreadScrollGeometry.self
                ) { geometry in
                    CouncisThreadScrollGeometry.measure(
    ...
    private func resolvedViewportAdmission(
        for pageScope: CouncisThreadPresentationScope
    ) -> CouncisMessageViewportAdmission {
    ...
    private var showsThinkingIndicator: Bool {
        CouncisThreadActivity.isAwaitingModelOutput(
            items: threadPage.items,
    ...
    private var defersRichUntilInitialRestore: Bool {
        CouncisThreadRichEntryPolicy.defersUntilInitialRestore(
            richRowCount: threadHistoryWindow.items.count)
    }

    private var threadHistoryWindow: CouncisThreadHistoryWindow<CodeItem> {
        CouncisThreadHistoryWindow(
            items: CouncisThreadErrorPresentation.transcriptItems(
                threadPage.items),
    ...
    private var threadPresentationScope: CouncisThreadPresentationScope {
    ...
    private var itemScrollSignature: CouncisThreadScrollSignature {
        let historyWindow = threadHistoryWindow
        let last = historyWindow.items.last
        return CouncisThreadScrollSignature(
    ...
    private var scrollReason: CouncisThreadScrollReason {
        let historyWindow = threadHistoryWindow
    ...
    private func scrollPerformer(
        ...
        { request in
            let anchorID = CouncisThreadBottomAnchorID(scope: request.scope)
    ...
    private func composerArea(layout: CouncisThreadContentLayout) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            CouncisThreadComposer(
                placeholder: CouncisLocalization.string("Give Main a project task..."),
                input: $input,
...
private struct CoworkWorkTaskRow: View {
    ...
    private var taskDetails: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            if !task.linkedInvocationIDs.isEmpty {
                detailList("Invocations", values: task.linkedInvocationIDs)
            }
        }
        .frame(maxWidth: .infinity, alignment: .leading)
    }
    ...
    private var displayStatus: String {
        councisLocalizedDisplayStatus(task.status)
    }
...
private struct CoworkTaskLineRow: View {
    ...
    init(task: CoworkTaskLine,
         style: CouncisThreadStyle = .standard(.light),
         actionTitle: String? = nil,
         actionDisabled: Bool = false,
         action: (() -> Void)? = nil) {
        self.task = task
