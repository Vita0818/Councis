...
struct ThreadView: View {
    ...
    private static let bottomAnchorID = "councis-shared-chat-thread-bottom"

    var body: some View {
        VStack(spacing: 0) {
            ScrollViewReader { proxy in
                ScrollView {
                    CouncisAdaptiveThreadStack(
            ...
            if !placesTurnStatsInComposer,
               let latestTurnStats = model.latestTurnStats {
                CouncisTurnStatsSummaryView(stats: latestTurnStats, style: .standard(scheme))
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.horizontal)
                    .padding(.vertical, 4)
            }
...
struct MessageRow: View {
    ...
    private var chatTextSize: CGFloat = CouncisTypography.spec(for: .chat).nominalPointSize
    @ScaledMetric(relativeTo: .caption)
    private var captionSize: CGFloat = CouncisTypography.spec(for: .caption).nominalPointSize
    @ScaledMetric(relativeTo: .caption2)
    private var metadataSize: CGFloat = CouncisTypography.spec(for: .metadata).nominalPointSize
        ...
        VStack(alignment: .leading, spacing: 4) {
            if CouncisMessageHeaderPolicy.showsIdentity(for: message.role)
                ...
                HStack(spacing: 6) {
                    if let roleLabel {
                        Text(roleLabel)
                            .font(CouncisTypography.metadata(metadataSize, .semibold))
                            .tracking(0.6)
                    ...
                    if (message.role == .assistant || message.role == .agent),
                       let timestamp = message.timestamp {
                        Text(CouncisMessageTimestampPresentation.string(for: timestamp))
                            .font(CouncisTypography.metadata(metadataSize))
                            .monospacedDigit()
            ...
            if message.role == .assistant || message.role == .agent {
                ...
                if !message.citations.isEmpty {
                    CouncisMessageCitationsView(citations: message.citations)
                }
            } else {
                if !displayText.isEmpty {
                    Text(displayText)
                        .font(CouncisTypography.chat(chatTextSize))
                ...
                if message.role == .user, !message.attachments.isEmpty {
                    Label(
                        ...
                        .font(CouncisTypography.caption(captionSize))
                        .foregroundStyle(.secondary)
                }
            }
            if let advice = message.recoveryAdvice {
                CouncisRecoveryAdviceView(advice: advice, tint: .red, style: style)
            }
        }
    }
    ...
    private var roleLabel: String? {
        ...
        case .assistant: return CouncisLocalization.string("Assistant")
        case .agent:
            return message.agent?.rawValue ?? CouncisLocalization.string("Agent")
...
public struct CouncisMessageCitationsView: View {
...
struct ComposerView: View {
    ...
    var body: some View {
        CouncisThreadComposer(
            ...
            accessory: placesTurnStatsInComposer
                ? AnyView(CouncisComposerUsageStrip(
    ...
    private var voiceAction: CouncisThreadComposerSecondaryAction? {
        #if canImport(AVFoundation)
        let voice = model.voiceInput
        return CouncisThreadComposerSecondaryAction(
    ...
    private var inputLeadingAccessory: AnyView? {
        #if os(iOS)
        let label = CouncisLocalization.string("Attachments and chat tools")
        return AnyView(
            Menu {
                Button {
                    model.generateImage()
                    ...
                    Label(
                        CouncisLocalization.string("Generate image from prompt"),
        ...
        List {
            Section("Status") {
                ...
                LabeledContent(
                    "Streaming",
                    value: isStreaming
                        ? CouncisLocalization.string("Yes")
                        : CouncisLocalization.string("No"))
                LabeledContent(
                    "Image job",
                    value: isGeneratingArtifact
                        ? CouncisLocalization.string("Running")
                        : CouncisLocalization.string("Idle"))
                LabeledContent(
                    "Artifact progress",
                    value: artifactProgress.isEmpty
                        ? CouncisLocalization.string("None")
                        : CouncisLocalization.format("%lld active", Int64(artifactProgress.count)))
