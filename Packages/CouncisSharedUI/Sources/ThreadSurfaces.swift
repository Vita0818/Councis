...
public struct CouncisThreadStyle {
    ...
    public static func standard(_: ColorScheme) -> CouncisThreadStyle {
        let stroke = councisPlatformSeparator
        return CouncisThreadStyle(
            primaryText: .primary,
...
public enum CouncisMessageHeaderPolicy {
...
struct CouncisThreadErrorEntry: Identifiable, Equatable, Sendable {
    let id: String
    var title: String?
    var details: [String]
...
enum CouncisThreadErrorPresentation {
    private struct Candidate {
        let fingerprint: String
        let entry: CouncisThreadErrorEntry
    }

    static func errors(
        items: [CodeItem],
        errorTexts: [String]
    ) -> [CouncisThreadErrorEntry] {
        let latestSubmissionID = items.reversed().first(where: {
            $0.kind == .user && $0.submissionID != nil
        })?.submissionID
        var orderedFingerprints: [String] = []
        var entriesByFingerprint: [String: CouncisThreadErrorEntry] = [:]
        ...
        if item.kind == .user,
           item.submissionStatus == .failed || item.submissionFailure != nil {
            ...
            return makeCandidate(
                ...
                title: CouncisLocalization.string("Needs attention"),
                primaryDetail: failure?.message
                    ?? advice?.detail
                    ?? CouncisLocalization.string("Needs attention"),
...
struct CouncisThreadErrorList: View {
    let errors: [CouncisThreadErrorEntry]
    ...
    private func errorRow(_ error: CouncisThreadErrorEntry) -> some View {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    ...
                    if let submissionID = error.retrySubmissionID,
                       let onRetrySubmission {
                        Button(CouncisLocalization.string("Retry")) {
...
public enum CouncisMessageTimestampPresentation {
...
public struct CouncisThreadPresentationScope: Hashable, Sendable {
...
public struct CouncisThreadBottomAnchorID: Hashable, Sendable {
    public let scope: CouncisThreadPresentationScope

    public init(scope: CouncisThreadPresentationScope) {
        self.scope = scope
    }
}

enum CouncisThreadScrollReason: Equatable, Sendable {
    case initialRestore
    case liveUpdate
    case completion
    ...
    var diagnosticReason: CouncisScrollDiagnosticReason {
        switch self {
        case .initialRestore:
...
struct CouncisThreadScrollRequest: Equatable, Sendable {
    let scope: CouncisThreadPresentationScope
    let generation: UInt64
    let reason: CouncisThreadScrollReason
    let animated: Bool
    let wasBottomFollowing: Bool
}

struct CouncisThreadScrollSignature: Equatable, Sendable {
...
struct CouncisThreadScrollGeometry: Equatable, Sendable {
    let isAtBottom: Bool
    ...
    static func == (
        lhs: CouncisThreadScrollGeometry,
        rhs: CouncisThreadScrollGeometry
    ) -> Bool {
...
enum CouncisThreadFollowState: Equatable, Sendable {
...
public enum CouncisMessageViewportAdmission: Equatable, Sendable {
    case immediate
...
private struct CouncisMessageViewportAdmissionEnvironmentKey: EnvironmentKey {
    static let defaultValue = CouncisMessageViewportAdmission.immediate
}

extension EnvironmentValues {
    public var councisMessageViewportAdmission:
        CouncisMessageViewportAdmission {
        get { self[CouncisMessageViewportAdmissionEnvironmentKey.self] }
        set { self[CouncisMessageViewportAdmissionEnvironmentKey.self] = newValue }
    }
}

enum CouncisThreadRichSettleToken: Hashable, Sendable {
    case finalDocument(
        messageID: String,
...
struct CouncisThreadRichSettleSource: Equatable, Sendable {
    let scope: CouncisThreadPresentationScope
    let activationGeneration: UInt64
}

private struct CouncisThreadScrollCoordinatorEnvironmentKey: EnvironmentKey {
    static let defaultValue: CouncisThreadScrollCoordinator? = nil
}

private struct CouncisThreadRichSettleSourceEnvironmentKey: EnvironmentKey {
    static let defaultValue: CouncisThreadRichSettleSource? = nil
}

extension EnvironmentValues {
    var councisThreadScrollCoordinator: CouncisThreadScrollCoordinator? {
        get { self[CouncisThreadScrollCoordinatorEnvironmentKey.self] }
        set { self[CouncisThreadScrollCoordinatorEnvironmentKey.self] = newValue }
    }

    var councisThreadRichSettleSource: CouncisThreadRichSettleSource? {
        get { self[CouncisThreadRichSettleSourceEnvironmentKey.self] }
        set { self[CouncisThreadRichSettleSourceEnvironmentKey.self] = newValue }
    }
}
...
struct CouncisThreadScrollCadenceModel: Sendable {
    ...
    struct Fire: Equatable, Sendable {
        let request: CouncisThreadScrollRequest
        let nextWake: Wake?
    }
    ...
    private(set) var executorRequest: CouncisThreadScrollRequest?
    /// The only pending request. New live updates replace this exact slot.
    private(set) var pendingRequest: CouncisThreadScrollRequest?
    ...
    var outstandingRequests: [CouncisThreadScrollRequest] {
        [executorRequest, pendingRequest].compactMap { $0 }
    }

    mutating func submit(
        _ request: CouncisThreadScrollRequest,
...
struct CouncisThreadRichSettleModel: Sendable {
    ...
    struct Epoch: Equatable, Sendable {
        let token: CouncisThreadRichSettleToken
    ...
    enum Check: Equatable, Sendable {
        ...
        case settled(token: CouncisThreadRichSettleToken)
    }

    private(set) var epoch: Epoch?
    private(set) var lastOpenedToken: CouncisThreadRichSettleToken?
...
@MainActor
final class CouncisThreadScrollCoordinator: ObservableObject {
    ...
    private(set) var activeScope: CouncisThreadPresentationScope?
    ...
    @Published private(set) var followState: CouncisThreadFollowState =
        .followingBottom
    @Published private(set) var viewportAdmission:
        CouncisMessageViewportAdmission = .suspended(generation: 0)
    @Published private(set) var richSettleSource:
        CouncisThreadRichSettleSource? = nil

    private var pendingTask: Task<Void, Never>?
    private var pendingWakeDeadlineNanoseconds: UInt64?
    private var pendingWakeToken: UInt64 = 0
    private var cadence = CouncisThreadScrollCadenceModel()
    private var scrollExecutor:
        (@MainActor (CouncisThreadScrollRequest) -> Void)?
    ...
    private var richSettle = CouncisThreadRichSettleModel()
    private var richSettleTask: Task<Void, Never>?
    private var richSettleTaskToken: UInt64 = 0
    private let performanceDiagnostics: CouncisPerformanceDiagnostics

    init(
        performanceDiagnostics: CouncisPerformanceDiagnostics = .shared
    ) {
    ...
    func effectiveViewportAdmission(
        for scope: CouncisThreadPresentationScope,
    ...
    func effectiveRichSettleSource(
        for scope: CouncisThreadPresentationScope
    ) -> CouncisThreadRichSettleSource? {
        guard activeScope == scope else { return nil }
        return richSettleSource
    }

    func activate(
        scope: CouncisThreadPresentationScope,
        ...
        richSettleSource = CouncisThreadRichSettleSource(
            scope: scope,
    ...
    func deactivate(scope: CouncisThreadPresentationScope) {
        guard activeScope == scope else { return }
        invalidatePending()
        invalidateGeometryObservation()
        invalidateBottomAnchorVisibility()
    ...
    func installScrollExecutor(
        scope: CouncisThreadPresentationScope,
        perform: @escaping @MainActor (CouncisThreadScrollRequest) -> Void
    ) {
        guard activeScope == scope else { return }
    ...
    func userInteractionDidBegin(scope: CouncisThreadPresentationScope) {
        guard activeScope == scope else { return }
    ...
    func userInteractionDidEnd(scope: CouncisThreadPresentationScope) {
        guard activeScope == scope,
    ...
    func jumpToLatest(
        ...
        guard activeScope == scope else {
            ...
            performanceDiagnostics.recordScrollRequest(
                reason: CouncisThreadScrollReason.jumpToLatest.diagnosticReason,
                outcome: .stale,
    ...
    func request(
        scope: CouncisThreadPresentationScope,
        reason: CouncisThreadScrollReason,
        perform: @escaping @MainActor (CouncisThreadScrollRequest) -> Void
    ) -> CouncisThreadScrollRequest? {
        guard activeScope == scope else {
            staleRejectionCount += 1
            performanceDiagnostics.recordScrollRequest(
    ...
    func richDocumentDidCommit(
        token: CouncisThreadRichSettleToken,
        source: CouncisThreadRichSettleSource
    ) {
    ...
    private func synchronizeRichSettleWake(
        _ check: CouncisThreadRichSettleModel.Check
    ) {
        switch check {
        case .inactive:
            return
...
public struct CouncisJumpToLatestButton: View {
...
public enum CouncisThreadHistoryWindowPolicy {
    public static let capacity = 16
}

enum CouncisThreadRichEntryPolicy {
...
struct CouncisThreadHistorySelection: Equatable {
    let scope: CouncisThreadPresentationScope
    let requestedUpperBound: Int?

    func upperBound(
        for currentScope: CouncisThreadPresentationScope
    ) -> Int? {
        scope == currentScope ? requestedUpperBound : nil
    }
}

public struct CouncisThreadHistoryWindow<Element> {
    ...
    public static func resolve(
        allItems: [Element],
        requestedUpperBound: Int?,
        capacity: Int = CouncisThreadHistoryWindowPolicy.capacity
    ) -> Self {
...
public struct CouncisThreadHistoryPager: View {
        ...
        HStack(spacing: 8) {
            if hasEarlier {
                Button(action: onEarlier) {
                    Label(
                        CouncisLocalization.string("Earlier"),
            ...
            if hasLater {
                Button(action: onNewer) {
                    Label(
                        CouncisLocalization.string("Newer"),
                ...
                Button(action: onLatest) {
                    Label(
                        CouncisLocalization.string("Latest"),
                        systemImage: "arrow.down.to.line")
                }
...
enum CouncisThreadStackLayoutMode: Equatable {
    case eager
    case lazy
...
public struct CouncisAdaptiveThreadStack<Content: View>: View {
    ...
    @ViewBuilder public var body: some View {
        switch CouncisThreadStackLayoutMode.resolve(
...
private var councisPlatformSeparator: Color {
    #if canImport(AppKit)
...
private struct CouncisContentSurfaceModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay {
                shape.stroke(councisPlatformSeparator, lineWidth: 1)
            }
    }
}

private struct CouncisSubtleContentSurfaceModifier: ViewModifier {
    let cornerRadius: CGFloat
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay {
                shape.stroke(
                    councisPlatformSeparator.opacity(0.55),
                    lineWidth: 0.5)
            }
    }
}
...
private struct CouncisClearLiquidGlassBackdrop: View, Equatable {
    let cornerRadius: CGFloat
    ...
    static func == (
        lhs: CouncisClearLiquidGlassBackdrop,
        rhs: CouncisClearLiquidGlassBackdrop
    ) -> Bool {
    ...
    private var stableOutline: some View {
        RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
            .strokeBorder(
                councisPlatformSeparator.opacity(0.42),
...
private struct CouncisLiquidGlassModifier: ViewModifier {
    ...
    @ViewBuilder func body(content: Content) -> some View {
        if usesClearVariant {
            content.background {
                CouncisClearLiquidGlassBackdrop(
                    cornerRadius: cornerRadius,
...
private struct CouncisGlassButtonModifier: ViewModifier {
...
public extension View {
    ...
    func councisContentSurface(cornerRadius: CGFloat = 16) -> some View {
        modifier(CouncisContentSurfaceModifier(cornerRadius: cornerRadius))
    }
    ...
    func councisSubtleContentSurface(cornerRadius: CGFloat = 12) -> some View {
        modifier(CouncisSubtleContentSurfaceModifier(cornerRadius: cornerRadius))
    }

    /// Native Liquid Glass on current systems, with semantic Material fallback.
    func councisLiquidGlass(cornerRadius: CGFloat = 16,
                            interactive: Bool = false) -> some View {
        modifier(CouncisLiquidGlassModifier(
            cornerRadius: cornerRadius,
    ...
    func councisClearLiquidGlass(cornerRadius: CGFloat = 16,
                                 interactive: Bool = false) -> some View {
        modifier(CouncisLiquidGlassModifier(
            cornerRadius: cornerRadius,
            isInteractive: interactive,
    ...
    func councisGlassButton(prominent: Bool = false) -> some View {
        modifier(CouncisGlassButtonModifier(isProminent: prominent))
    }
    ...
    func councisCompactIconButton(prominent: Bool = false) -> some View {
        labelStyle(.iconOnly)
            .controlSize(.regular)
            .buttonBorderShape(.circle)
            .councisGlassButton(prominent: prominent)
    }
    ...
    @ViewBuilder func councisComposerIconButton(
        prominent: Bool = false
    ) -> some View {
        #if os(iOS)
        labelStyle(.iconOnly)
            .controlSize(CouncisComposerControlMetrics.iconControlSize(
                for: .iOS))
            .buttonBorderShape(.circle)
            .councisGlassButton(prominent: prominent)
            .frame(
                width: CouncisComposerControlMetrics.controlHeight,
                height: CouncisComposerControlMetrics.controlHeight)
            .contentShape(Circle())
        #else
        councisCompactIconButton(prominent: prominent)
            .frame(
                width: CouncisComposerControlMetrics.controlHeight,
                height: CouncisComposerControlMetrics.controlHeight)
            .contentShape(Circle())
    ...
    func councisComposerSelectionMenu() -> some View {
        buttonStyle(.plain)
    }

    /// Shared 40-point capsule geometry for the model/profile Menu label.
    func councisComposerSelectionLabel() -> some View {
        padding(
            .horizontal,
            CouncisComposerControlMetrics.selectionHorizontalPadding)
            .frame(
                height: CouncisComposerControlMetrics.controlHeight,
                alignment: .leading)
            .frame(
                ...
                alignment: .leading)
            .councisLiquidGlass(
                cornerRadius: CouncisComposerControlMetrics.controlHeight / 2,
                interactive: true)
    }
...
public struct CouncisGlassEffectGroup<Content: View>: View {
    private let spacing: CGFloat?
    private let content: Content
...
public struct CouncisTurnStatsSummaryView: View {
    private let stats: TurnStatsSnapshot
    private let style: CouncisThreadStyle

    public init(stats: TurnStatsSnapshot, style: CouncisThreadStyle) {
        self.stats = stats
        self.style = style
    }
    ...
    private var parts: [String] {
        ...
        if let ttftMillis = stats.ttftMillis {
            values.append(CouncisLocalization.format(
                "ttft %@",
    ...
    private var tokenPart: String? {
        if let totalTokens = stats.totalTokens {
            let total = CouncisLocalization.format(
                "%@ tok",
        ...
        if let promptTokens = stats.promptTokens,
           let cachedPromptTokens = stats.cachedPromptTokens {
            let uncachedPromptTokens = max(promptTokens - cachedPromptTokens, 0)
            pieces.append(CouncisLocalization.format(
                "%@ input",
                formatNumber(uncachedPromptTokens)))
            pieces.append(CouncisLocalization.format(
                "%@ cached",
...
public struct CouncisComposerUsageStrip: View {
    private let stats: TurnStatsSnapshot?
    private let style: CouncisThreadStyle
                ...
                HStack(spacing: 18) {
                    ...
                    if stats.promptTokens == nil,
                       ...
                       let totalTokens = stats.totalTokens {
                        metric(
                            CouncisLocalization.string("Total"),
                            value: formatNumber(totalTokens))
                    }
                    if let totalMillis = stats.totalMillis {
                        metric(
                            CouncisLocalization.string("Time"),
...
public struct CouncisSessionHistoryItem: Identifiable, Hashable {
                    ...
                    LazyVStack(spacing: 5) {
                        ForEach(items) { item in
                            Button {
                                ...
                                CouncisSessionHistoryRow(item: item, style: style)
                            }
                            .buttonStyle(.plain)
                            .accessibilityIdentifier(
...
private struct CouncisSessionHistoryRow: View {
    let item: CouncisSessionHistoryItem
    let style: CouncisThreadStyle
...
public struct CouncisRecoveryAdviceView: View {
...
public struct CouncisThreadComposerSecondaryAction {
    public var systemImage: String
...
public enum CouncisComposerControlMetrics {
    ...
    static func glassEffectSpacing(
        for platform: CouncisComposerGlassPlatform
    ) -> CGFloat {
        switch platform {
        case .iOS:
            return 0
...
enum CouncisComposerGlassPlatform: Sendable {
    case iOS
    case macOS
}

public enum CouncisSidebarGesturePolicy {
...
public struct CouncisThreadComposer: View {
    @Binding private var input: String
    private let placeholder: String
    private let canSend: Bool
    private let isInputDisabled: Bool
    private let style: CouncisThreadStyle
    private let secondaryAction: CouncisThreadComposerSecondaryAction?
    private let trailingAction: CouncisThreadComposerSecondaryAction?
    private let stopAction: CouncisThreadComposerSecondaryAction?
    ...
    private var inputPointSize: CGFloat = CouncisTypography.spec(for: .chat).nominalPointSize

    private var inputFont: Font {
        CouncisTypography.chat(inputPointSize)
    }
    ...
    public init(placeholder: String,
                ...
                style: CouncisThreadStyle,
                secondaryAction: CouncisThreadComposerSecondaryAction? = nil,
                leadingAccessory: AnyView? = nil,
                inputLeadingAccessory: AnyView? = nil,
                trailingAction: CouncisThreadComposerSecondaryAction? = nil,
                stopAction: CouncisThreadComposerSecondaryAction? = nil,
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            CouncisGlassEffectGroup(spacing: composerGlassEffectSpacing) {
                composerControls
            }
        }
    }
    ...
    private var composerGlassEffectSpacing: CGFloat {
        #if os(iOS)
        CouncisComposerControlMetrics.glassEffectSpacing(for: .iOS)
        #else
        CouncisComposerControlMetrics.glassEffectSpacing(for: .macOS)
        #endif
    }
    ...
    private var inputControl: some View {
        TextField(placeholder, text: $input, axis: .vertical)
            ...
            .padding(.horizontal, CouncisComposerControlMetrics.inputHorizontalPadding)
            .padding(.vertical, CouncisComposerControlMetrics.inputVerticalPadding)
            .frame(
                minHeight: CouncisComposerControlMetrics.controlHeight,
                alignment: .center)
            .councisLiquidGlass(
                cornerRadius: CouncisComposerControlMetrics.inputCornerRadius,
                interactive: true)
    }

    private func compactActionButton(
        _ action: CouncisThreadComposerSecondaryAction
    ) -> some View {
    ...
    private var sendButton: some View {
        Button(action: onSend) {
            Label(CouncisLocalization.string("Send"), systemImage: "arrow.up")
                .councisComposerIconLabel()
        }
        .councisComposerIconButton(prominent: true)
        .disabled(!canSend)
        .accessibilityLabel(CouncisLocalization.string("Send"))
    ...
    private func stopButton(
        ...
        Button(role: .destructive, action: action.action) {
            Label(
                CouncisLocalization.string("Stop"),
        ...
        .help(action.help)
        .disabled(action.isDisabled)
        .accessibilityLabel(CouncisLocalization.string("Stop"))
        .accessibilityIdentifier("thread.composer.stop")
    }

}

public struct CouncisThreadContentLayout {
...
enum CouncisThreadBubbleWidthPolicy: Equatable {
...
public struct CouncisThreadBubbleRow<Content: View>: View {
    ...
    @ViewBuilder private var row: some View {
        switch CouncisThreadBubbleWidthPolicy.resolve(
            isTrailing: isTrailing,
...
public struct CouncisThreadThinkingRow: View {
    private let layout: CouncisThreadContentLayout
    private let style: CouncisThreadStyle
    private let label: String
    private let phaseID: String

    public init(layout: CouncisThreadContentLayout,
                style: CouncisThreadStyle,
                label: String = CouncisLocalization.string("Thinking…"),
...
public struct CouncisThinkingElapsedLabel: View {
    private let label: String
    private let phaseID: String
    ...
    public var body: some View {
        TimelineView(.periodic(from: startedAt, by: 1)) { context in
            ...
            let text = CouncisLocalization.format(
                "%llds %@",
                Int64(elapsed),
                label)
            Text(text)
...
public enum CouncisThreadHeaderActionPresentation: Sendable {
    case glass
    case compactSystemIcon
}

public struct CouncisThreadHeaderAction {
    let title: String
    let systemImage: String
    let isDisabled: Bool
    let isIconOnly: Bool
    let presentation: CouncisThreadHeaderActionPresentation
    let help: String
    ...
    public init(title: String,
                ...
                presentation: CouncisThreadHeaderActionPresentation = .glass,
                help: String? = nil,
...
struct CouncisWorkspaceThreadHeader: View {
    let title: String
    let subtitle: String?
    let style: CouncisThreadStyle
    let actions: [CouncisThreadHeaderAction]
    ...
    private var titleBlock: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(title)
                .font(CouncisTypography.largeTitle())
                .foregroundStyle(style.primaryText)
            if let subtitle, !subtitle.isEmpty {
                Text(subtitle)
                    .font(CouncisTypography.caption(13, .medium))
                    .foregroundStyle(style.secondaryText)
                    .lineLimit(2)
            }
        }
    ...
    @ViewBuilder private var actionRow: some View {
        if !actions.isEmpty {
            CouncisGlassEffectGroup(spacing: 8) {
                HStack(spacing: 8) {
    ...
    @ViewBuilder private func actionButton(
        _ action: CouncisThreadHeaderAction
    ) -> some View {
