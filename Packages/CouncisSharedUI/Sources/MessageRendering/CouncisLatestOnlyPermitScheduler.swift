...
struct CouncisLatestOnlySubmissionID: Hashable, Comparable, RawRepresentable, Sendable {
...
struct CouncisLatestOnlyPermit<Key: Hashable & Sendable>: Sendable {
  public let key: Key
  public let submissionID: CouncisLatestOnlySubmissionID

  fileprivate let schedulerID: UUID
}

/// The caller-reported reason a permit's real work ended.
enum CouncisLatestOnlyFinishOutcome: Sendable {
  case success
  case cancelled
  case failed
}
...
struct CouncisLatestOnlyPermitSchedulerSnapshot<Key: Hashable & Sendable>: Sendable {
...
enum CouncisLatestOnlyPermitSchedulerConfigurationError: Error, Equatable, Sendable {
...
actor CouncisLatestOnlyPermitScheduler<Key: Hashable & Sendable> {
  public typealias Permit = CouncisLatestOnlyPermit<Key>
  public typealias Snapshot = CouncisLatestOnlyPermitSchedulerSnapshot<Key>

  private struct PendingAcquire: Sendable {
    let submissionID: CouncisLatestOnlySubmissionID
  ...
  private var latestSubmissionByKey: [Key: CouncisLatestOnlySubmissionID] = [:]
  ...
  public init(
    ...
    guard maxConcurrentPermits > 0 else {
      throw CouncisLatestOnlyPermitSchedulerConfigurationError.nonPositiveConcurrentMaximum(
        maxConcurrentPermits
      )
    }
    guard maxPendingAcquires > 0 else {
      throw CouncisLatestOnlyPermitSchedulerConfigurationError.nonPositivePendingMaximum(
        maxPendingAcquires
      )
    }
  ...
  public func finish(
    _ permit: Permit,
    outcome: CouncisLatestOnlyFinishOutcome = .success
  ) -> Bool {
    finishInternal(
      permit,
      outcome: outcome,
  ...
  private func makeSubmissionID() -> CouncisLatestOnlySubmissionID {
    precondition(
      nextSubmissionRawValue < UInt64.max,
      "CouncisLatestOnlySubmissionID space exhausted"
    )
    nextSubmissionRawValue += 1
    return CouncisLatestOnlySubmissionID(rawValue: nextSubmissionRawValue)
  }

  private func suspendAcquire(
    for key: Key,
    submissionID: CouncisLatestOnlySubmissionID
  ) async -> Permit? {
    await withCheckedContinuation { continuation in
      registerAcquire(
        for: key,
  ...
  private func finishInternal(
    _ permit: Permit,
    outcome: CouncisLatestOnlyFinishOutcome,
