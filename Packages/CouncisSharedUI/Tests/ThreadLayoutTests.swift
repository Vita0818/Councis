...
final class ThreadLayoutTests: XCTestCase {
    func testIOSComposerGlassMergeThresholdStaysBelowRowGap() {
        XCTAssertEqual(CouncisComposerControlMetrics.rowSpacing, 8)
        XCTAssertEqual(
        ...
        XCTAssertLessThan(
            CouncisComposerControlMetrics.glassEffectSpacing(for: .iOS),
            CouncisComposerControlMetrics.rowSpacing)
    }

    func testComposerUsesPlatformControlSizeWithinSharedFortyPointGeometry() {
        XCTAssertEqual(CouncisComposerControlMetrics.controlHeight, 40)
        XCTAssertEqual(
            CouncisComposerControlMetrics.iconControlSize(for: .iOS),
            .small)
        XCTAssertEqual(
            CouncisComposerControlMetrics.iconControlSize(for: .macOS),
            .regular)
    }

    func testSidebarOpenGestureRequiresLeadingEdgeHorizontalIntent() {
        XCTAssertTrue(CouncisSidebarGesturePolicy.shouldOpen(
            startX: 8,
            translation: CGSize(width: 72, height: 8)))
        XCTAssertFalse(CouncisSidebarGesturePolicy.shouldOpen(
            startX: 40,
            translation: CGSize(width: 72, height: 8)))
        XCTAssertFalse(CouncisSidebarGesturePolicy.shouldOpen(
            startX: 8,
    ...
    func testSidebarCloseGestureRequiresHorizontalLeftIntent() {
        XCTAssertTrue(CouncisSidebarGesturePolicy.shouldClose(
            translation: CGSize(width: -60, height: 8)))
        XCTAssertFalse(CouncisSidebarGesturePolicy.shouldClose(
    ...
    func testLeadingAssistantAndAgentRowsUseTheFullAvailableWidth() {
        XCTAssertEqual(
            CouncisThreadBubbleWidthPolicy.resolve(
                isTrailing: false,
    ...
    func testUserMessagesDoNotRepeatAnIdentityHeader() {
        XCTAssertFalse(CouncisMessageHeaderPolicy.showsIdentity(for: .user))
        XCTAssertTrue(CouncisMessageHeaderPolicy.showsIdentity(for: .assistant))
        XCTAssertTrue(CouncisMessageHeaderPolicy.showsIdentity(for: .agent))
        XCTAssertTrue(CouncisMessageHeaderPolicy.showsIdentity(for: .system))
    }
    ...
    func testOnlyUserConversationRowsUseNativeLiquidGlassBubble() throws {
        ...
        XCTAssertTrue(sharedMessageRow.contains(
            ".councisLiquidGlass(cornerRadius: 10)"))
        XCTAssertFalse(sharedMessageRow.contains(".councisContentSurface("))
        ...
        let macChatSource = try String(
            contentsOf: repositoryRoot
                .appendingPathComponent(
                    "Apps/CouncisMac/Sources/CouncisChatScreen.swift"),
            encoding: .utf8)
        let macMessageBubble = try sourceSlice(
            macChatSource,
            from: "struct CouncisMessageBubble: View",
            to: "struct CouncisComposer: View")
        ...
        XCTAssertFalse(macMessageBubble.contains(".councisContentSurface("))
        ...
        XCTAssertFalse(codeBubbleContent.contains(".councisContentSurface("))
        XCTAssertFalse(codeSource.contains("private func bubbleStroke(isUser:"))
    }

    func testCouncisMacRootHidesModesWithoutDeletingInheritedSurfaces() throws {
        let testFile = URL(fileURLWithPath: #filePath)
        let packageRoot = testFile
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        ...
        let source = try String(
            contentsOf: repositoryRoot.appendingPathComponent(
                "Apps/CouncisMac/Sources/CouncisMacRootView.swift"),
            encoding: .utf8)

        XCTAssertTrue(source.contains("case chat, code, cowork"))
        ...
        XCTAssertTrue(source.contains("case .code:"))
        XCTAssertTrue(source.contains("case .cowork:"))

        let sidebarStart = try XCTUnwrap(
            source.range(of: "struct CouncisSidebar: View"))
        let titleBlockStart = try XCTUnwrap(source.range(
            of: "private var titleBlock: some View",
            range: sidebarStart.upperBound..<source.endIndex))
        let sidebarBody = source[
            sidebarStart.lowerBound..<titleBlockStart.lowerBound]
        XCTAssertFalse(sidebarBody.contains("modeNavigation"))
        XCTAssertTrue(source.contains("private var modeNavigation: some View"))
        XCTAssertTrue(source.contains("Text(\"Councis\")"))
    }
    ...
    func testThreadErrorsCollectEveryConversationSourceAndLeaveTranscriptClean() throws {
        ...
        let errors = CouncisThreadErrorPresentation.errors(
            items: items,
            errorTexts: [
                "Voice input failed.",
        ...
        let transcript = CouncisThreadErrorPresentation.transcriptItems(items)
    ...
    func testThreadErrorCardIsAbsentWhenEveryErrorSourceIsEmpty() {
        ...
        XCTAssertTrue(CouncisThreadErrorPresentation.errors(
            items: items,
            errorTexts: ["  ", "\n"]).isEmpty)
        XCTAssertEqual(
            CouncisThreadErrorPresentation.transcriptItems(items),
            items)
        ...
        XCTAssertTrue(CouncisThreadErrorPresentation.errors(
            ...
            errorTexts: []).isEmpty)
        XCTAssertEqual(
            CouncisThreadErrorPresentation
    ...
    func testWorkspaceShellsUseOneConditionalRightRailErrorList() throws {
        ...
        XCTAssertTrue(codeSource.contains("CouncisThreadErrorList("))
        ...
        XCTAssertTrue(coworkSource.contains("CouncisThreadErrorList("))
    ...
    func testWorkspaceInspectorUsesOnlyTheStableOuterWidth() {
        let hidden = CouncisWorkspaceInspectorLayoutPolicy.resolve(
            availableWidth: 979,
            isRequested: true,
            activationWidth: 980,
        ...
        XCTAssertEqual(
            hidden,
            CouncisWorkspaceInspectorLayout(
                isVisible: false,
                threadWidth: 979,
                inspectorWidth: 0))

        let visible = CouncisWorkspaceInspectorLayoutPolicy.resolve(
            availableWidth: 980,
            isRequested: true,
            activationWidth: 980,
            minimumThreadWidth: 620,
        ...
        let integratedCowork = CouncisWorkspaceInspectorLayoutPolicy.resolve(
            availableWidth: 980,
            isRequested: true,
            activationWidth: 980,
            minimumThreadWidth: 620,
        ...
        for _ in 0..<10_000 {
            XCTAssertEqual(
                CouncisWorkspaceInspectorLayoutPolicy.resolve(
                    availableWidth: 980,
                    isRequested: true,
                    activationWidth: 980,
                    minimumThreadWidth: 620,
    ...
    func testCoworkStatusRailAndConversationWidthStayFixedAcrossContentStates() {
        XCTAssertEqual(CouncisCoworkStatusRailLayoutPolicy.railWidth, 348)
        XCTAssertEqual(CouncisCoworkStatusRailLayoutPolicy.cardWidth, 318)
        XCTAssertEqual(CouncisCoworkStatusRailLayoutPolicy.cardSpacing, 18)

        for availableWidth in [CGFloat(980), 1_100, 1_372, 1_800] {
            let emptyConversation =
                CouncisCoworkStatusRailLayoutPolicy.resolve(
    ...
    func testCoworkStatusRailRenderSnapshotExcludesConversationSelection() {
        ...
        let changedErrorState = CoworkStatusRailRenderSnapshot(
            ...
            goal: nil,
            workTasks: CoworkWorkTaskSummary(),
            errors: CouncisThreadErrorPresentation.errors(
                items: [],
    ...
    func testWorkspaceInspectorNeverProducesInvalidGeometry() {
        for width in stride(from: CGFloat(-100), through: 2_000, by: 0.5) {
            let layout = CouncisWorkspaceInspectorLayoutPolicy.resolve(
    ...
    func testWorkspaceThreadsDoNotVendWindowToolbarOrNestedInspectorPreferences() throws {
        ...
        XCTAssertFalse(
            coworkSource.contains(
                "CouncisGlassEffectGroup(\n                spacing: CouncisCoworkStatusRailLayoutPolicy"))
        ...
        XCTAssertTrue(surfaceSource.contains(
            "private struct CouncisClearLiquidGlassBackdrop"))
        XCTAssertTrue(surfaceSource.contains(
        ...
        let appSource = try String(
            contentsOf: repositoryRoot
                .appendingPathComponent("Apps/CouncisMac/Sources/CouncisMacApp.swift"),
            encoding: .utf8)
