...
final class ThreadScrollCoordinatorTests: XCTestCase {
    private let codeA = CouncisThreadPresentationScope(
        kind: "code",
        sessionID: "code-a")
    private let codeB = CouncisThreadPresentationScope(
        kind: "code",
        sessionID: "code-b")
    private let coworkA = CouncisThreadPresentationScope(
        kind: "cowork",
        sessionID: "cowork-a")
    ...
    func testScopeAndBottomAnchorAreSessionSpecific() {
        ...
        XCTAssertNotEqual(
            CouncisThreadBottomAnchorID(scope: codeA),
            CouncisThreadBottomAnchorID(scope: codeB))
    }
    ...
    func testBottomVisibilityDoesNotUseGeometryPreferences() throws {
        ...
        let surfaces = try String(
            ...
            encoding: .utf8)
        XCTAssertFalse(
            surfaces.contains("CouncisThreadViewportFramesPreferenceKey"))
    ...
    func testScopeChangeCancelsPendingRequest() async {
        ...
        var executed: [CouncisThreadScrollRequest] = []
    ...
    func testLazyEntrySuspendsRichUntilRawBottomAnchorIsVisible() async {
        ...
        var admissionInsideExecutor: CouncisMessageViewportAdmission?
    ...
    func testCompletionCanReplaceInitialRestoreWithoutLeavingRichSuspended() async {
        let coordinator = CouncisThreadScrollCoordinator()
        var executed: [CouncisThreadScrollReason] = []
        coordinator.activate(
            scope: codeA,
            defersRichUntilInitialRestore: true)
    ...
    func testRapidABARestoreOnlyFinalScopeCanReleaseRichAdmission() async {
        let coordinator = CouncisThreadScrollCoordinator()
        var executedScopes: [CouncisThreadPresentationScope] = []
    ...
    func testLateRichCommitCannotCrossScopeOrABAActivation() async throws {
        let coordinator = CouncisThreadScrollCoordinator()
        var executions: [CouncisThreadPresentationScope] = []
    ...
    func testFiveHundredUpdatesUseFixedWindowLeadingTrailingCadence() {
        var model = CouncisThreadScrollCadenceModel()
        var executed: [CouncisThreadScrollRequest] = []
    ...
    func testCadenceDeadlineIsNotResetByEveryUpdate() {
        ...
        XCTAssertEqual(
            ...
            CouncisThreadScrollCadenceModel.intervalNanoseconds)
        XCTAssertEqual(
    ...
    func testWindowLocalCoordinatorsDoNotCancelEachOther() async {
        let firstWindow = CouncisThreadScrollCoordinator()
        let secondWindow = CouncisThreadScrollCoordinator()
    ...
    func testGeometryUsesToleranceAndIgnoresSubpixelHeightJitter() {
        let atBottom = CouncisThreadScrollGeometry.measure(
            contentOffsetY: 476,
            containerHeight: 500,
            bottomInset: 0,
            contentHeight: 1_000)
        ...
        let previous = CouncisThreadScrollGeometry(
            isAtBottom: true,
            contentHeight: 1_000)
        let jitter = CouncisThreadScrollGeometry(
            isAtBottom: false,
            contentHeight: 1_000.5)
        let materialGrowth = CouncisThreadScrollGeometry(
            isAtBottom: false,
            contentHeight: 1_002)
        ...
        XCTAssertEqual(previous, CouncisThreadScrollGeometry(
            isAtBottom: true,
            contentHeight: 9_999))
    ...
    func testMacRichTranscriptSurfacesUseBoundedEagerWindows() throws {
        ...
        for sourceURL in sources {
            ...
            XCTAssertTrue(
                ...
                sourceURL.lastPathComponent)
            XCTAssertTrue(
                transcript.contains("CouncisThreadHistoryPager("),
                sourceURL.lastPathComponent)
            ...
            XCTAssertFalse(
                transcript.contains("CouncisAdaptiveThreadStack("),
                sourceURL.lastPathComponent)
            XCTAssertFalse(
    ...
    func testRichSettleClosesBeforeReturningSingleSettlement() {
        var model = CouncisThreadRichSettleModel()
        let token = CouncisThreadRichSettleToken.finalDocument(
            messageID: "message",
    ...
    func testRichSettleWaitsForQuietButHonorsHardCap() {
        var model = CouncisThreadRichSettleModel()
        let token = CouncisThreadRichSettleToken.width(900_000)
        _ = model.open(
            token: token,
            contentHeight: 1_000,
            nowNanoseconds: 0)
    ...
    func testScrollDiagnosticReasonsPreserveSemanticSource() {
        XCTAssertEqual(
            CouncisThreadScrollReason.initialRestore.diagnosticReason.rawValue,
            CouncisScrollDiagnosticReason.initialRestore.rawValue)
        XCTAssertEqual(
            CouncisThreadScrollReason.liveUpdate.diagnosticReason.rawValue,
            CouncisScrollDiagnosticReason.liveContent.rawValue)
        XCTAssertEqual(
            CouncisThreadScrollReason.completion.diagnosticReason.rawValue,
            CouncisScrollDiagnosticReason.completion.rawValue)
        XCTAssertEqual(
            CouncisThreadScrollReason.richHeightCorrection
                .diagnosticReason.rawValue,
            CouncisScrollDiagnosticReason.richSettle.rawValue)
        XCTAssertEqual(
            CouncisThreadScrollReason.jumpToLatest.diagnosticReason.rawValue,
            CouncisScrollDiagnosticReason.manualJump.rawValue)
    }
    ...
    private func fireDueRequests(
        model: inout CouncisThreadScrollCadenceModel,
        through now: UInt64,
        executed: inout [CouncisThreadScrollRequest]
    ) {
