...
private func waitForMarkdownSchedulerSnapshot<Key: Hashable & Sendable>(
    _ scheduler: CouncisLatestOnlyPermitScheduler<Key>,
    attempts: Int = 2_000,
    matching predicate: @Sendable (CouncisLatestOnlyPermitSchedulerSnapshot<Key>) -> Bool
) async throws -> CouncisLatestOnlyPermitSchedulerSnapshot<Key> {
    for _ in 0..<attempts {
...
final class MarkdownSchedulerTests: XCTestCase {
    func testRejectsNonPositiveBounds() {
        XCTAssertThrowsError(try CouncisLatestOnlyPermitScheduler<String>(
            maxConcurrentPermits: 0,
            maxPendingAcquires: 1
        )) { error in
            XCTAssertEqual(
                error as? CouncisLatestOnlyPermitSchedulerConfigurationError,
    ...
    func testGlobalConcurrencyAndPendingCapacityStayBounded() async throws {
        let scheduler = try CouncisLatestOnlyPermitScheduler<Int>(
    ...
    func testSameKeyKeepsOneRunningAndOneReplaceablePending() async throws {
        let scheduler = try CouncisLatestOnlyPermitScheduler<String>(
            maxConcurrentPermits: 1,
            maxPendingAcquires: 1)
