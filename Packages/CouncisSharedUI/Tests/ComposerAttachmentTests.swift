...
final class ComposerAttachmentTests: XCTestCase {
    func testSharedAttachmentFlowReadsPreservesAndResolvesImage() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-composer-attachments-\(UUID().uuidString)",
                isDirectory: true)
        ...
        let file = try CouncisComposerAttachmentFileReader.read(sourceURL)
        ...
        XCTAssertEqual(
            try CouncisComposerAttachmentFileReader.read(jpegURL).mime,
            "image/jpeg")

        let artifactStore = try ArtifactStore(
            root: root.appendingPathComponent("artifacts", isDirectory: true))
        let attachmentStore = CouncisComposerAttachmentStore(store: artifactStore)
    ...
    func testSharedAttachmentFlowPreservesNonImageButRejectsProviderInput() async throws {
        ...
        } catch let error as CouncisComposerAttachmentResolutionError {
