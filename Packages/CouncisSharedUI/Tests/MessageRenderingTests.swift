...
private final class ViewportAdmissionFixtureModel: ObservableObject {
    @Published var admission: CouncisMessageViewportAdmission

    init(admission: CouncisMessageViewportAdmission) {
...
private struct StableThreadWindowHostingFixture: View {
    @ObservedObject var coordinator: CouncisThreadScrollCoordinator
    let rowCount: Int
    private let scope = CouncisThreadPresentationScope(
        kind: "cowork",
        sessionID: "stable-window")
        .historyWindowScope(requestedUpperBound: nil)

    init(
        coordinator: CouncisThreadScrollCoordinator,
        rowCount: Int = 13
    ) {
        ...
        ScrollViewReader { proxy in
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Color.clear
                        .frame(height: 1)
                        .id(CouncisThreadBottomAnchorID(scope: scope))
            ...
            .onAppear {
                ...
                coordinator.request(
                    ...
                    proxy.scrollTo(
                        CouncisThreadBottomAnchorID(scope: request.scope),
                        anchor: .bottom)
                }
            }
        }
...
private func waitForPublishedMarkdown(
    _ state: CouncisMicrosoftMarkdownRenderState,
    revision: CouncisMarkdownRenderRevision,
    attempts: Int = 4_000
) async throws -> CouncisMicrosoftMarkdownRenderState.PublishedDocument {
    for _ in 0..<attempts {
...
private func waitForPublishedMarkdown(
    _ state: CouncisMicrosoftMarkdownRenderState,
    request: CouncisMarkdownRenderRequest,
    attempts: Int = 4_000
) async throws -> CouncisMicrosoftMarkdownRenderState.PublishedDocument {
    for _ in 0..<attempts {
...
final class MessageRenderingTests: XCTestCase {
    ...
    private func rawRevision(
        ...
        lane: CouncisRawTextProjectionLane = .plain,
        text: String,
    ...
    private func renderRequest(
        ...
        appearance: CouncisMarkdownAppearanceRevision = .light,
        typography: CouncisMarkdownTypographyRevision = .large,
    ...
    func testRichFacadeDoesNotWrapDocumentInASecondSelectionOverlay() throws {
        ...
        let source = try String(
            contentsOf: packageRoot.appendingPathComponent(
                "Sources/MessageRendering/CouncisMessageContentView.swift"),
            encoding: .utf8)
        ...
        let plainEnd = try XCTUnwrap(
            source.range(
                of: ".accessibilityIdentifier(\"councis.message.plain.",
    ...
    func testProjectionLifecycleGateDeduplicatesAndRejectsLateInvisibleInput() {
        ...
        let rawState = CouncisRawTextProjectionState(revision: initial)
        let richState = CouncisMicrosoftMarkdownRenderState()
        let gate = CouncisMessageProjectionLifecycleGate()
        let first = CouncisMessageProjectionInput(
        ...
        let late = CouncisMessageProjectionInput(
    ...
    func testProjectionLifecycleGateRichToPlainSwitchDropsPublishedDocument() async throws {
        ...
        let richInput = CouncisMessageProjectionInput(
            rawRevision: initial,
        ...
        let plainInput = CouncisMessageProjectionInput(
    ...
    func testViewportInteractionBlocksAdmissionUntilLatestExactDwell() {
        ...
        let request = renderRequest(
            ...
            text: "initial",
            isComplete: false)
        let suspended = CouncisMessageProjectionInput(
            rawRevision: initial,
            richRequest: request,
            usesRichRenderer: true,
        ...
        let dwell = CouncisMessageProjectionInput(
            rawRevision: initial,
            richRequest: request,
            usesRichRenderer: true,
    ...
    func testThirteenRowStableWindowKeepsFiveRichRowsRawUntilExactDwell() {
        let window = CouncisThreadHistoryWindow.resolve(
            allItems: Array(0..<13),
        ...
        var rows: [(
            gate: CouncisMessageProjectionLifecycleGate,
            raw: CouncisRawTextProjectionState,
            rich: CouncisMicrosoftMarkdownRenderState,
            input: CouncisMessageProjectionInput
        )] = []
        ...
        for index in 0..<5 {
            ...
            let revision = rawRevision(
                ...
                text: text,
                isComplete: true)
            let rawState = CouncisRawTextProjectionState(revision: revision)
            let richState = CouncisMicrosoftMarkdownRenderState()
            let gate = CouncisMessageProjectionLifecycleGate()
            let input = CouncisMessageProjectionInput(
                rawRevision: revision,
                richRequest: renderRequest(
                    messageID: "entry-\(index)",
                    text: text),
        ...
        for row in rows {
            ...
            let dwellInput = CouncisMessageProjectionInput(
    ...
    func testViewportDwellRejectsSupersededRevision() {
        ...
        let latestRevision = rawRevision(
            ...
            text: "first latest",
            isComplete: false)
        let latest = CouncisMessageProjectionInput(
    ...
    func testSuspensionRetainsOnlyExactPublishedDocument() async throws {
        ...
        let exactRequest = renderRequest(
            ...
            text: "# exact")
        let immediate = CouncisMessageProjectionInput(
            rawRevision: initial,
        ...
        let suspendedExact = CouncisMessageProjectionInput(
            rawRevision: initial,
            richRequest: exactRequest,
            usesRichRenderer: true,
        ...
        let replacementRevision = rawRevision(
            ...
            text: "# replacement",
            isComplete: true)
        let suspendedReplacement = CouncisMessageProjectionInput(
    ...
    func testNSHostingStableWindowRestoresRawBeforeMountingRichRows() async throws {
        ...
        UserDefaults.standard.set(
            CouncisMessageRendererMode.microsoft.rawValue,
            forKey: CouncisMessageRendererMode.defaultsKey)
        defer {
            if let previousMode {
                UserDefaults.standard.set(
        ...
        let coordinator = CouncisThreadScrollCoordinator()
    ...
    func testStyleOnlyRequestChangeSupersedesOldParseAndDuplicateIsNoOp() async throws {
        ...
        let revision = CouncisMarkdownRenderRevision(
            messageID: "style",
            rawText: text,
            isComplete: false,
            appearance: .light,
            typography: .large,
            configurationRevision: CouncisMarkdownRendererLimits.configurationRevision)
        let first = CouncisMarkdownRenderRequest(
            revision: revision,
            style: CouncisMarkdownStyleSnapshot(.standard(.light)))
        let replacementStyle = CouncisThreadStyle(
            primaryText: .red,
            secondaryText: .green,
            tertiaryText: .blue,
            accent: .orange,
            ...
            cardStroke: .yellow,
            error: .pink)
        let replacement = CouncisMarkdownRenderRequest(
            revision: revision,
            style: CouncisMarkdownStyleSnapshot(replacementStyle))
        XCTAssertNotEqual(first, replacement)

        let state = CouncisMicrosoftMarkdownRenderState()
    ...
    func testTypographyOnlyRequestChangeSupersedesOldParseAndScalesConfiguration() async throws {
        ...
        XCTAssertEqual(
            CouncisMarkdownTypographyRevision(DynamicTypeSize.large),
            .large)
        XCTAssertEqual(
            CouncisMarkdownTypographyRevision(DynamicTypeSize.accessibility3),
            .accessibility3)

        let baseline = CouncisMicrosoftMarkdownRenderState.makeConfiguration(
            style: .standard(.light),
            typography: .large)
        ...
        XCTAssertEqual(
            ...
            baseline.paragraphStyle.textFonts.normal.pointSize
                * CouncisMarkdownTypographyRevision.accessibility3.scale,
            accuracy: 0.001)
        XCTAssertEqual(
    ...
    func testRawProjectionUsesLeadingTrailingThrottleWithoutResettingDeadline() throws {
        var model = CouncisRawTextProjectionModel(
    ...
    func testRawStateFirstFrameBypassesThrottleForSemanticChangesAndReentry() {
        let initial = rawRevision(
            lane: .richFallback,
            text: "stream")
        let state = CouncisRawTextProjectionState(revision: initial)
    ...
    func testWholeMessageAdmissionIsSyntaxAgnosticAndUTF8Bounded() {
        let exact = CouncisMarkdownRenderRevision(
            messageID: "exact",
        ...
        let oversized = CouncisMarkdownRenderRevision(
            messageID: "oversized",
            ...
            typography: .large,
            configurationRevision: 1)
        let multiByteOversized = CouncisMarkdownRenderRevision(
            messageID: "multibyte",
            ...
            typography: .large,
            configurationRevision: 1)
        let empty = CouncisMarkdownRenderRevision(
            messageID: "empty",
            rawText: "",
            isComplete: false,
            appearance: .light,
    ...
    func testAdaptiveThreadStackRetainsItsNonProductionCompatibilityPolicy() {
        XCTAssertEqual(
            CouncisThreadStackLayoutMode.resolve(visibleRowCount: 2),
            .eager)
        XCTAssertEqual(
            CouncisThreadStackLayoutMode.resolve(visibleRowCount: 4),
            .eager)
        XCTAssertEqual(
            CouncisThreadStackLayoutMode.resolve(visibleRowCount: 5),
            .lazy)
    }

    func testThreadHistoryWindowKeepsThirteenRowsInOneLatestPage() {
        XCTAssertEqual(CouncisThreadHistoryWindowPolicy.capacity, 16)
    ...
    func testThreadHistoryWindowEarlierAndNewerUseStablePageBoundaries() {
        ...
        let middle = CouncisThreadHistoryWindow.resolve(
            allItems: items,
        ...
        let oldest = CouncisThreadHistoryWindow.resolve(
            allItems: items,
        ...
        let newer = CouncisThreadHistoryWindow.resolve(
            allItems: items,
    ...
    func testThreadHistoryWindowExplicitOlderPageDoesNotDriftOnAppend() {
        let before = CouncisThreadHistoryWindow.resolve(
            allItems: Array(0..<40),
            requestedUpperBound: 24)
        let after = CouncisThreadHistoryWindow.resolve(
            allItems: Array(0..<41),
            requestedUpperBound: 24)
    ...
    func testThreadHistoryWindowLatestPageFollowsAppendWithoutExceedingCapacity() {
        ...
        for total in totals {
            ...
            XCTAssertLessThanOrEqual(
                window.items.count,
                CouncisThreadHistoryWindowPolicy.capacity)
    ...
    func testThreadHistoryWindowScopesAreStableAndIsolatedByPage() {
        let base = CouncisThreadPresentationScope(
            kind: "cowork",
            sessionID: "scope")
    ...
    func testThreadHistorySelectionCannotLeakAcrossSessions() {
        let first = CouncisThreadPresentationScope(
            kind: "cowork",
            sessionID: "first")
        let second = CouncisThreadPresentationScope(
            kind: "cowork",
            sessionID: "second")
        let selection = CouncisThreadHistorySelection(
            scope: first,
            requestedUpperBound: 24)
    ...
    func testFirstReleaseConfigurationDisablesOptionalUnboundedFeatures() {
        let configuration = CouncisMicrosoftMarkdownRenderState.makeConfiguration(
            style: .standard(.light))
        ...
        XCTAssertEqual(
            ...
            CouncisMarkdownRendererLimits.mathMode.renderConfig.mode)
    ...
    func testLatexMathIsDefaultAndHasAnIndependentLaunchKillSwitch() {
        XCTAssertEqual(
            CouncisMarkdownMathMode.resolve(arguments: ["Councis"]),
            .latex)
        XCTAssertEqual(
            CouncisMarkdownMathMode.resolve(arguments: [
                "Councis",
                CouncisMarkdownRendererLimits.disableSingleDollarMathLaunchArgument,
            ]),
            .disabled)
        XCTAssertEqual(
            CouncisMarkdownMathMode.latex.renderConfig.mode,
            .latex)
        XCTAssertEqual(
            CouncisMarkdownMathMode.disabled.renderConfig.mode,
            .disabled)
    }

    func testLinkPolicyAllowsOnlyProductSchemes() throws {
        XCTAssertTrue(CouncisMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "https://example.com"))))
        XCTAssertTrue(CouncisMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "http://example.com"))))
        XCTAssertTrue(CouncisMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "mailto:test@example.com"))))
        XCTAssertFalse(CouncisMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "file:///tmp/message"))))
        XCTAssertFalse(CouncisMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "data:text/plain,hello"))))
        XCTAssertFalse(CouncisMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "javascript:alert(1)"))))
        XCTAssertFalse(CouncisMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "relative/path"))))
    }
    ...
    func testMarkdownDiagnosticsAggregateQueueParseAndMainActorPublish() async throws {
        let diagnostics = CouncisPerformanceDiagnostics()
        let state = CouncisMicrosoftMarkdownRenderState(
    ...
    func testSingleConsumerKeepsOnlyTheLatestStreamingSnapshot() async throws {
        ...
        let finalRevision = CouncisMarkdownRenderRevision(
            messageID: messageID,
            rawText: finalText,
            isComplete: true,
            appearance: .dark,
    ...
    func testSanitizedIncidentReplaysAll1249DeltasInOriginalOrder() async throws {
        ...
        var states: [CouncisMicrosoftMarkdownRenderState] = []
        var expectedRevisions: [CouncisMarkdownRenderRevision] = []
        var yieldedDeltas = 0
