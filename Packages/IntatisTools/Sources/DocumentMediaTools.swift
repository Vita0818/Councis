    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        case "$BACKEND" in
          ...
          auto)
            ...
            if [ -z "$DOCLING" ] && [ -z "$MARKITDOWN" ]; then
              printf 'No local document backend was found. Install docling or markitdown[all] into the Councis document runtime.\\n' >&2
            fi
            exit 127
            ;;
        esac
        ...
        guard result.exitCode == 0 else {
            ...
            throw IntatisError.io(
                "document reading failed. Install a backend in the Councis document runtime or select another backend.\(legacyHint) \(transcript)")
        }
