...
public struct KnowledgeSecureFileSystem: Sendable {
    ...
    public static func canonicalBundleDigest(_ entries: [KnowledgeChecksumEntry]) throws -> String {
        ...
        return try KnowledgeDigest.canonical(
            Projection(version: "councis-okf-bundle-digest/1", files: selected))
    }
    ...
    public func snapshotSealDigest(root: URL) throws -> String {
        ...
        return try KnowledgeDigest.canonical(Projection(
            version: "councis-knowledge-snapshot-content-seal/1",
            files: files))
    }
    ...
    private static func role(for path: String) -> String {
        if path == ".councis-rag/profile.json" { return "profile" }
        if path == ".councis-rag/chunks.jsonl" { return "chunk_manifest" }
        if path.hasPrefix(".councis-rag/dense/") { return "dense_index" }
        if path.hasPrefix(".councis-rag/lexical/") { return "lexical_index" }
        if path.hasPrefix(".councis-rag/auxiliary/") { return "auxiliary" }
