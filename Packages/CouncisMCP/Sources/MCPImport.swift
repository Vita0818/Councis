...
private struct ImportParserState {
    ...
    mutating func parseImportedValue(
        ...
        if let object = rawValue as? [String: Any] {
            let allowed: Set<String> = [
                "$councisSecretRef", "storage", "sourceBindingFingerprint",
            ]
            guard Set(object.keys).isSubset(of: allowed),
                  let identifier = object["$councisSecretRef"] as? String,
...
public enum MCPConfigurationExporter {
    /// Emits a stable Councis MCP export. Secret values are structurally
    ...
    public static func sanitizedMCPJSON(
        ...
        let root: [String: Any] = [
            "$schema": "https://councis.invalid/schemas/mcp-import-v1.json",
            "version": 1,
            "mcpServers": servers,
        ]
    ...
    private static func exportedReference(_ reference: MCPSecretReference) -> [String: Any] {
        var result: [String: Any] = [
            "$councisSecretRef": reference.identifier,
