...
import Crypto
#else
#error("CouncisMCP requires CryptoKit or swift-crypto")
#endif
import Foundation
import CouncisCore
...
public actor MCPKeychainSecretStore: MCPSecretStore {
    ...
    public init(
        service: String = CouncisProductIdentity.mcpKeychainService,
        accessGroup: String? = nil
    ) {
        self.service = service
        self.legacyServices = service
    ...
    private func copy(account: String) throws -> Data {
        do {
            ...
            for legacyService in legacyServices {
                guard let data = try? copy(
                    service: legacyService,
                    account: account) else {
                    continue
                }
                // Best-effort, non-destructive bridge. Failure to create the
                // canonical item does not delete or invalidate the old item.
    ...
    private func copy(service: String, account: String) throws -> Data {
        var query = Self.makeBaseQuery(
            service: service,
            account: account,
            accessGroup: accessGroup,
            useDataProtectionKeychain: useDataProtectionKeychain)
...
public actor MCPCLIEncryptedSecretStore: MCPSecretStore {
    ...
    private static let authenticatedContext =
        Data("Councis MCP CLI credential store v1".utf8)
    private static let legacyAuthenticatedContext =
    ...
    private func open(
        ...
        do {
            ...
            plaintext = try AES.GCM.open(
                box,
                using: key,
                authenticating: Self.legacyAuthenticatedContext)
        }
