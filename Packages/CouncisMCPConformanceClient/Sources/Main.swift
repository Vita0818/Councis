...
@main
private enum CouncisMCPConformanceClient {
    static func main() async {
        do {
            ...
            FileHandle.standardError.write(
                Data("Councis MCP conformance client failed: \(summary)\n".utf8))
            exit(EXIT_FAILURE)
        }
    }
}
...
private struct Runner {
    ...
    func run() async throws {
        ...
        case "councis/task-complete":
            try await runTaskAugmented(.complete)
        case "councis/task-timeout":
            try await runTaskAugmented(.timeout)
        case "councis/task-cancel":
    ...
    private func makeSession(
        ...
        return MCPClientSession(
            configuration: MCPClientSessionConfiguration(
                ...
                clientName: "councis-conformance-client",
                clientVersion: "1.0.0",
    ...
    static func make() async throws -> ConformanceResources {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-mcp-conformance-\(UUID().uuidString)",
                isDirectory: true)
        ...
        try await store.initialize(
            passphrase: Data(
                "councis-conformance-disposable-key".utf8))
...
private struct OAuthResources {
    ...
    static func make(
        ...
        try await store.initialize(
            passphrase: Data(
                "councis-oauth-conformance-key".utf8))
