...
#include "CouncisMCPStdioGuard.h"
...
#define COUNCIS_PTRACE_SYSCALL_INFO_NONE 0U
#define COUNCIS_PTRACE_SYSCALL_INFO_ENTRY 1U
#define COUNCIS_PTRACE_SYSCALL_INFO_EXIT 2U
#define COUNCIS_PTRACE_SYSCALL_INFO_SECCOMP 3U
...
#define COUNCIS_AUDIT_ARCH AUDIT_ARCH_X86_64
#elif defined(__aarch64__)
#define COUNCIS_AUDIT_ARCH AUDIT_ARCH_AARCH64
#else
#define COUNCIS_AUDIT_ARCH 0
#endif

#define COUNCIS_SECCOMP_ALLOW SECCOMP_RET_ALLOW
#define COUNCIS_SECCOMP_TRACE SECCOMP_RET_TRACE
#define COUNCIS_SECCOMP_DENY \
    (SECCOMP_RET_ERRNO | (EPERM & SECCOMP_RET_DATA))
#define COUNCIS_SECCOMP_NOT_IMPLEMENTED \
...
typedef struct {
    ...
    uint64_t file_id;
    uint64_t byte_count;
} councis_owned_identity_t;

struct councis_mcp_stdio_guard {
    pid_t group_id;
    pid_t initial_pid;
    pid_t target_pid;
    int wrapper_seen;
    ...
    int fatal_errno;
    councis_mcp_stdio_network_policy_t network_policy;
    councis_owned_identity_t wrapper;
    councis_owned_identity_t primary;
    councis_owned_identity_t *helpers;
    size_t helper_count;
    councis_tracee_t *tracees;
    size_t tracee_count;
    size_t tracee_capacity;
};

#define COUNCIS_MAXIMUM_TRACEE_COUNT 4096U
#define COUNCIS_FREEZE_TIMEOUT_MILLISECONDS 2000U
#define COUNCIS_NETWORK_SYSCALL_TIMEOUT_MILLISECONDS 5000U

typedef struct {
    struct sock_filter *items;
    size_t count;
    size_t capacity;
} councis_filter_builder_t;

static void councis_close_fd(int *fd) {
    if (*fd >= 0) {
        (void)close(*fd);
        *fd = -1;
    }
...
static int councis_set_cloexec(int fd) {
...
static int councis_set_nonblocking(int fd) {
    int flags = fcntl(fd, F_GETFL);
    if (flags < 0 || fcntl(fd, F_SETFL, flags | O_NONBLOCK) < 0) {
        return -1;
    }
    return councis_set_cloexec(fd);
}

static int councis_pipe_cloexec(int descriptors[2]) {
    ...
    if (councis_set_cloexec(descriptors[0]) < 0 ||
        councis_set_cloexec(descriptors[1]) < 0) {
        int saved = errno;
        close(descriptors[0]);
        close(descriptors[1]);
        errno = saved;
...
static int councis_append_filter(
    councis_filter_builder_t *builder,
...
#define COUNCIS_STMT(code_value, k_value) \
    ((struct sock_filter)BPF_STMT((code_value), (k_value)))
#define COUNCIS_JUMP(code_value, k_value, true_jump, false_jump) \
    ((struct sock_filter)BPF_JUMP( \
        (code_value), (k_value), (true_jump), (false_jump)))

static int councis_append_deny_syscall(
    councis_filter_builder_t *builder,
    int syscall_number,
    uint32_t denial) {
    return councis_append_filter(
               builder,
               COUNCIS_JUMP(
                   ...
                   0,
                   1)) ||
           councis_append_filter(
               builder,
               COUNCIS_STMT(BPF_RET | BPF_K, denial));
}

static int councis_append_trace_syscall(
    councis_filter_builder_t *builder,
    int syscall_number) {
    return councis_append_deny_syscall(
        builder, syscall_number, COUNCIS_SECCOMP_TRACE);
}

static int councis_build_seccomp_filter(
    int helpers_enabled,
    struct sock_filter **items,
    size_t *count) {
    if (COUNCIS_AUDIT_ARCH == 0) {
        errno = ENOTSUP;
        return -1;
    }
    councis_filter_builder_t builder = {0};
    if (councis_append_filter(
            &builder,
            COUNCIS_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct councis_seccomp_data, arch))) ||
        councis_append_filter(
            &builder,
            COUNCIS_JUMP(
                BPF_JMP | BPF_JEQ | BPF_K,
        ...
        councis_append_filter(
            &builder,
            COUNCIS_STMT(BPF_RET | BPF_K, SECCOMP_RET_KILL_PROCESS)) ||
        councis_append_filter(
            &builder,
            COUNCIS_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct councis_seccomp_data, nr)))) {
        free(builder.items);
        return -1;
    }

#ifdef __NR_setsid
    if (councis_append_deny_syscall(
            &builder, __NR_setsid, COUNCIS_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_setpgid
    if (councis_append_deny_syscall(
            &builder, __NR_setpgid, COUNCIS_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_unshare
    if (councis_append_deny_syscall(
            &builder, __NR_unshare, COUNCIS_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_setns
    if (councis_append_deny_syscall(
            &builder, __NR_setns, COUNCIS_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_ptrace
    if (councis_append_deny_syscall(
            &builder, __NR_ptrace, COUNCIS_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_clone3
    ...
    if (councis_append_deny_syscall(
            &builder,
            __NR_clone3,
            COUNCIS_SECCOMP_NOT_IMPLEMENTED)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_fork
    if (!helpers_enabled &&
        councis_append_deny_syscall(
            &builder, __NR_fork, COUNCIS_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_vfork
    if (!helpers_enabled &&
        councis_append_deny_syscall(
            &builder, __NR_vfork, COUNCIS_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
    ...
    if (councis_append_filter(
        ...
        councis_append_filter(
            ...
            COUNCIS_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct councis_seccomp_data, args[0]))) ||
        councis_append_filter(
            &builder,
            COUNCIS_JUMP(
        ...
        councis_append_filter(
            &builder,
            COUNCIS_STMT(
                BPF_RET | BPF_K, COUNCIS_SECCOMP_DENY)) ||
        councis_append_filter(
            &builder,
            COUNCIS_STMT(
                BPF_RET | BPF_K, COUNCIS_SECCOMP_ALLOW))) {
        goto allocation_failure;
    }
    if (helpers_enabled) {
     ...
     */
#ifdef __NR_socket
    if (councis_append_trace_syscall(&builder, __NR_socket)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_connect
    if (councis_append_trace_syscall(&builder, __NR_connect)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_bind
    if (councis_append_trace_syscall(&builder, __NR_bind)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_listen
    if (councis_append_trace_syscall(&builder, __NR_listen)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_accept
    if (councis_append_trace_syscall(&builder, __NR_accept)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_accept4
    if (councis_append_trace_syscall(&builder, __NR_accept4)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendto
    if (councis_append_trace_syscall(&builder, __NR_sendto)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendmsg
    if (councis_append_trace_syscall(&builder, __NR_sendmsg)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendmmsg
    if (councis_append_trace_syscall(&builder, __NR_sendmmsg)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_setup
    if (councis_append_trace_syscall(&builder, __NR_io_uring_setup)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_enter
    if (councis_append_trace_syscall(&builder, __NR_io_uring_enter)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_register
    if (councis_append_trace_syscall(&builder, __NR_io_uring_register)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_bpf
    if (councis_append_trace_syscall(&builder, __NR_bpf)) {
        goto allocation_failure;
    }
#endif
    ...
    if (councis_append_filter(
            &builder,
            COUNCIS_STMT(BPF_RET | BPF_K, COUNCIS_SECCOMP_ALLOW))) {
        goto allocation_failure;
    }
    *items = builder.items;
    *count = builder.count;
...
static int councis_make_seccomp_fd(
    ...
    size_t count = 0;
    if (councis_build_seccomp_filter(
...
static int councis_close_from_four(void) {
...
static int councis_write_handshake_byte(int fd) {
...
static int councis_read_handshake_byte(int fd) {
...
static void councis_guarded_child(
     ...
     */
    if (councis_write_handshake_byte(startup_child) < 0 ||
        councis_read_handshake_byte(startup_child) < 0) {
        _exit(126);
    }
    (void)close(startup_child);
    if (councis_close_from_four() < 0 ||
...
static int councis_capture_owned_identity(
    const char *path,
    councis_owned_identity_t *identity) {
...
static int councis_copy_identity(
    const councis_mcp_stdio_exec_identity_t *source,
    councis_owned_identity_t *destination) {
...
static int councis_add_tracee(
    councis_mcp_stdio_guard_t *guard,
    pid_t pid,
    int is_wrapper_root) {
    ...
    if (guard->tracee_count >= COUNCIS_MAXIMUM_TRACEE_COUNT) {
        errno = E2BIG;
        return -1;
    }
    if (guard->tracee_count == guard->tracee_capacity) {
        ...
        councis_tracee_t *tracees = (councis_tracee_t *)realloc(
            guard->tracees, next * sizeof(councis_tracee_t));
        if (tracees == NULL) {
            return -1;
        }
...
static councis_tracee_t *councis_find_tracee(
    councis_mcp_stdio_guard_t *guard,
    pid_t pid) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].pid == pid) {
...
static int councis_identity_matches_pid(
    pid_t pid,
    const councis_owned_identity_t *expected) {
    char proc_path[64];
    char resolved[4096];
    struct stat metadata;
    int length = snprintf(
...
static int councis_match_helper(
    councis_mcp_stdio_guard_t *guard,
    pid_t pid) {
    for (size_t index = 0; index < guard->helper_count; index++) {
        if (councis_identity_matches_pid(pid, &guard->helpers[index])) {
            return 1;
        }
    }
    return 0;
...
typedef struct {
    ...
    uint64_t instruction_pointer;
    uint64_t stack_pointer;
} councis_ptrace_syscall_info_prefix_t;

static int councis_ptrace_syscall_operation(
    pid_t pid,
    uint8_t *operation) {
    councis_ptrace_syscall_info_prefix_t information;
    ...
    if (count < (long)sizeof(information) ||
        information.operation
            == COUNCIS_PTRACE_SYSCALL_INFO_NONE) {
        errno = ENOTSUP;
        return -1;
    }
...
static int councis_read_syscall_snapshot(
    pid_t pid,
    councis_syscall_snapshot_t *snapshot) {
#if defined(__x86_64__) || defined(__aarch64__)
    councis_native_registers_t registers;
...
static int councis_rewrite_syscall_number(
    pid_t pid,
    long syscall_number) {
#if defined(__x86_64__) || defined(__aarch64__)
    councis_native_registers_t registers;
...
static int councis_rewrite_syscall_result(
    pid_t pid,
    long result) {
#if defined(__x86_64__) || defined(__aarch64__)
    councis_native_registers_t registers;
...
static int councis_read_tracee_memory(
    pid_t pid,
...
static int councis_validate_network_syscall(
    councis_mcp_stdio_guard_t *guard,
    pid_t pid) {
    councis_syscall_snapshot_t snapshot;
    memset(&snapshot, 0, sizeof(snapshot));
    if (councis_read_syscall_snapshot(pid, &snapshot) < 0) {
        return -1;
    }
    ...
    if (snapshot.syscall_number == __NR_socket) {
        ...
        return guard->network_policy.mode
                    == COUNCIS_MCP_STDIO_NETWORK_EXACT_GATEWAY &&
               family == AF_INET &&
...
static uint64_t councis_monotonic_milliseconds(void);

static int councis_thread_group_leader(
    pid_t thread_id,
    pid_t *leader_id) {
 ...
 */
static int councis_emulate_exact_connect(
    const councis_mcp_stdio_guard_t *guard,
    pid_t pid,
    const councis_syscall_snapshot_t *snapshot,
     ...
     */
    pid_t leader_id = 0;
    if (councis_thread_group_leader(pid, &leader_id) < 0) {
        return -1;
    }
    int pid_descriptor =
    ...
    if (connect_result < 0 &&
        ...
        uint64_t start = councis_monotonic_milliseconds();
        if (start == 0) {
            connect_error = EIO;
            goto connect_failure;
        }
        for (;;) {
            uint64_t now = councis_monotonic_milliseconds();
            if (now == 0 ||
                now - start >=
                    COUNCIS_NETWORK_SYSCALL_TIMEOUT_MILLISECONDS) {
...
static void councis_mark_violation(
    councis_mcp_stdio_guard_t *guard,
    int error_number) {
    if (!guard->violation) {
        guard->violation = 1;
...
static void councis_mark_network_violation(
    councis_mcp_stdio_guard_t *guard,
    int error_number) {
    guard->network_violation = 1;
    councis_mark_violation(guard, error_number);
}

static int councis_decode_exit_status(int status);
static int councis_handle_exec_event(
    councis_mcp_stdio_guard_t *guard,
    councis_tracee_t *tracee);
...
static void councis_poll_pause(void) {
    struct timespec value;
    value.tv_sec = 0;
...
static void councis_record_tracee_exit(
    councis_mcp_stdio_guard_t *guard,
    pid_t pid,
    int status) {
    councis_tracee_t *tracee =
        councis_find_tracee(guard, pid);
    if (tracee == NULL) {
        return;
    }
    tracee->alive = 0;
    ...
    if (pid == guard->target_pid) {
        ...
        guard->target_exit_status =
            councis_decode_exit_status(status);
    }
}

static int councis_all_live_tracees_frozen(
    const councis_mcp_stdio_guard_t *guard) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].alive &&
...
static int councis_capture_frozen_stop(
    councis_mcp_stdio_guard_t *guard,
    pid_t pid,
    int status) {
    if (WIFEXITED(status) || WIFSIGNALED(status)) {
        councis_record_tracee_exit(guard, pid, status);
        return 0;
    }
 ...
 */
static int councis_execute_frozen_network_syscall(
    councis_mcp_stdio_guard_t *guard,
    pid_t pid) {
    councis_tracee_t *tracee =
        councis_find_tracee(guard, pid);
    ...
    councis_syscall_snapshot_t expected;
    memset(&expected, 0, sizeof(expected));
    if (councis_read_syscall_snapshot(
            pid, &expected) < 0) {
        return -1;
    }
    ...
    if (expected.syscall_number == __NR_connect) {
        if (councis_emulate_exact_connect(
                guard,
                pid,
                &expected,
                &emulated_result) < 0 ||
            councis_rewrite_syscall_number(pid, -1) < 0) {
            return -1;
        }
    ...
    for (;;) {
        ...
        if (waited == pid) {
            if (WIFEXITED(status) || WIFSIGNALED(status)) {
                councis_record_tracee_exit(
                    guard, pid, status);
                errno = EIO;
                return -1;
            }
            tracee = councis_find_tracee(guard, pid);
            ...
            uint8_t operation = 0;
            councis_syscall_snapshot_t observed;
            memset(&observed, 0, sizeof(observed));
            if (councis_ptrace_syscall_operation(
                    pid, &operation) < 0 ||
                councis_read_syscall_snapshot(
                    pid, &observed) < 0 ||
            ...
            if (operation
                    == COUNCIS_PTRACE_SYSCALL_INFO_EXIT) {
                if (has_emulated_result &&
                    councis_rewrite_syscall_result(
            ...
            if ((operation
                    != COUNCIS_PTRACE_SYSCALL_INFO_ENTRY &&
                 operation
                    != COUNCIS_PTRACE_SYSCALL_INFO_SECCOMP) ||
                ++syscall_stops > 2) {
                errno = EIO;
                return -1;
            }
 ...
 */
static int councis_freeze_validate_and_execute_network(
    councis_mcp_stdio_guard_t *guard,
    pid_t triggering_pid,
    int triggering_status) {
    councis_tracee_t *trigger =
        councis_find_tracee(guard, triggering_pid);
    if (trigger == NULL || !trigger->alive) {
        councis_mark_network_violation(guard, ECHILD);
        return -1;
    }
    trigger->frozen = 1;
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        councis_tracee_t *tracee = &guard->tracees[index];
        ...
        if (ptrace(
            ...
            councis_mark_network_violation(guard, errno);
            return -1;
        }
    }
    ...
    uint64_t start = councis_monotonic_milliseconds();
    if (start == 0) {
        councis_mark_network_violation(guard, EIO);
        return -1;
    }
    while (!councis_all_live_tracees_frozen(guard)) {
        int made_progress = 0;
        ...
        for (size_t index = 0; index < observed_count; index++) {
            ...
            if (waited < 0 ||
                councis_capture_frozen_stop(
                    guard, pid, status) < 0) {
                int saved = errno;
                councis_mark_network_violation(
        ...
        if (!made_progress) {
            uint64_t now = councis_monotonic_milliseconds();
            if (now == 0 || now - start
                    >= COUNCIS_FREEZE_TIMEOUT_MILLISECONDS) {
                councis_mark_network_violation(
                    guard, ETIMEDOUT);
                return -1;
            }
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        ...
        if (event == PTRACE_EVENT_EXEC &&
            councis_handle_exec_event(guard, tracee) < 0) {
            councis_mark_violation(guard, EACCES);
            return -1;
        }
    }
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        ...
        if (councis_ptrace_syscall_operation(
            ...
            operation
                != COUNCIS_PTRACE_SYSCALL_INFO_SECCOMP ||
            councis_validate_network_syscall(
                guard, pid) < 0 ||
            councis_execute_frozen_network_syscall(
                guard, pid) < 0) {
            int saved = errno;
            councis_mark_network_violation(
...
static int councis_all_tracees_exited(
    const councis_mcp_stdio_guard_t *guard) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].alive) {
            return 0;
...
static int councis_handle_exec_event(
    ...
    if (!guard->wrapper_seen) {
        if (!tracee->is_wrapper_root ||
            !councis_identity_matches_pid(tracee->pid, &guard->wrapper)) {
            return -1;
        }
    ...
    if (!guard->target_seen) {
        if (!councis_identity_matches_pid(tracee->pid, &guard->primary)) {
            return -1;
        }
        guard->target_seen = 1;
    ...
    if (!tracee->may_exec_helper ||
        ...
        !councis_match_helper(guard, tracee->pid)) {
        return -1;
    }
...
static void councis_execution_guard_probe_child(void) {
    struct sock_filter filter[] = {
        BPF_STMT(
            BPF_LD | BPF_W | BPF_ABS,
            offsetof(struct councis_seccomp_data, nr)),
...
int councis_mcp_stdio_execution_guard_probe(void) {
    ...
    pid_t child = fork();
    if (child < 0) {
        councis_close_fd(&startup_pair[0]);
        councis_close_fd(&startup_pair[1]);
        return 0;
    }
    if (child == 0) {
        councis_close_fd(&startup_pair[0]);
        if (councis_read_handshake_byte(startup_pair[1]) < 0) {
            _exit(111);
        }
        councis_close_fd(&startup_pair[1]);
        councis_execution_guard_probe_child();
    }
    ...
    if (ptrace(
        ...
        councis_write_handshake_byte(startup_pair[0]) < 0 ||
        waitpid(child, &status, __WALL) != child ||
        !WIFSTOPPED(status) ||
        (((unsigned int)status) >> 16)
            != PTRACE_EVENT_SECCOMP ||
        councis_ptrace_syscall_operation(
            child, &operation) < 0 ||
        operation
            != COUNCIS_PTRACE_SYSCALL_INFO_SECCOMP ||
        councis_rewrite_syscall_number(child, -1) < 0 ||
        ...
        operation
            != COUNCIS_PTRACE_SYSCALL_INFO_EXIT ||
        councis_rewrite_syscall_result(child, 4242) < 0 ||
        ptrace(PTRACE_CONT, child, NULL, NULL) < 0 ||
        waitpid(child, &status, __WALL) != child ||
        !WIFEXITED(status) ||
            WEXITSTATUS(status) != 0) {
        councis_close_fd(&duplicated_descriptor);
        councis_close_fd(&pid_descriptor);
        councis_close_fd(&startup_pair[0]);
...
int councis_mcp_stdio_guard_spawn(
    const char *wrapper_path,
    char *const argv[],
    char *const envp[],
    const char *working_directory,
    const char *runtime_directory,
    const councis_mcp_stdio_exec_identity_t *primary,
    const councis_mcp_stdio_exec_identity_t *helpers,
    size_t helper_count,
    const councis_mcp_stdio_network_policy_t *network_policy,
    councis_mcp_stdio_guard_spawn_result_t *result) {
    if (result == NULL) {
        return COUNCIS_MCP_STDIO_GUARD_UNAVAILABLE;
    }
    ...
    if (wrapper_path == NULL || argv == NULL || envp == NULL ||
        ...
        (network_policy->mode != COUNCIS_MCP_STDIO_NETWORK_DENIED &&
         network_policy->mode
            != COUNCIS_MCP_STDIO_NETWORK_EXACT_GATEWAY) ||
        (network_policy->mode
            == COUNCIS_MCP_STDIO_NETWORK_EXACT_GATEWAY &&
        ...
        (network_policy->mode
            == COUNCIS_MCP_STDIO_NETWORK_DENIED &&
         (network_policy->gateway_ipv4_network_order != 0 ||
          network_policy->gateway_port_host_order != 0)) ||
        COUNCIS_AUDIT_ARCH == 0) {
    ...
    if (councis_pipe_cloexec(stdin_pipe) < 0 ||
        councis_pipe_cloexec(stdout_pipe) < 0 ||
        councis_pipe_cloexec(stderr_pipe) < 0) {
        result->error_number = errno;
        councis_close_fd(&stdin_pipe[0]);
        councis_close_fd(&stdin_pipe[1]);
        councis_close_fd(&stdout_pipe[0]);
        councis_close_fd(&stdout_pipe[1]);
        councis_close_fd(&stderr_pipe[0]);
        councis_close_fd(&stderr_pipe[1]);
        return COUNCIS_MCP_STDIO_GUARD_PIPE_FAILED;
    }
    int seccomp_fd =
        councis_make_seccomp_fd(runtime_directory, helper_count > 0);
    if (seccomp_fd < 0) {
        result->error_number = errno;
        councis_close_fd(&stdin_pipe[0]);
        councis_close_fd(&stdin_pipe[1]);
    ...
    councis_mcp_stdio_guard_t *guard =
        (councis_mcp_stdio_guard_t *)calloc(1, sizeof(*guard));
    if (guard == NULL ||
        councis_capture_owned_identity(
            wrapper_path, &guard->wrapper) < 0 ||
        councis_copy_identity(primary, &guard->primary) < 0) {
        result->error_number = errno;
        councis_close_fd(&seccomp_fd);
        councis_close_fd(&stdin_pipe[0]);
        councis_close_fd(&stdin_pipe[1]);
        councis_close_fd(&stdout_pipe[0]);
        councis_close_fd(&stdout_pipe[1]);
        councis_close_fd(&stderr_pipe[0]);
        councis_close_fd(&stderr_pipe[1]);
        councis_mcp_stdio_guard_destroy(guard);
        return COUNCIS_MCP_STDIO_GUARD_IDENTITY_FAILED;
    }
    guard->network_policy = *network_policy;
    if (helper_count > 0) {
        guard->helpers = (councis_owned_identity_t *)calloc(
            helper_count, sizeof(councis_owned_identity_t));
    ...
    if (child < 0) {
        ...
        councis_close_fd(&stderr_pipe[1]);
        councis_mcp_stdio_guard_destroy(guard);
        return COUNCIS_MCP_STDIO_GUARD_FORK_FAILED;
    }
    if (child == 0) {
        councis_guarded_child(
            wrapper_path,
            argv,
            envp,
            working_directory,
    ...
    if (councis_add_tracee(guard, child, 1) < 0) {
        result->error_number = ENOMEM;
        (void)kill(-child, SIGKILL);
        (void)kill(child, SIGKILL);
        (void)waitpid(child, NULL, 0);
        ...
        councis_close_fd(&stderr_pipe[0]);
        councis_mcp_stdio_guard_destroy(guard);
        return COUNCIS_MCP_STDIO_GUARD_TRACE_FAILED;
    }
    ...
    if (councis_read_handshake_byte(startup_pair[0]) < 0 ||
        getpgid(child) != child ||
        ptrace(
            PTRACE_SEIZE,
            child,
        ...
        councis_set_nonblocking(stdin_pipe[1]) < 0 ||
        councis_set_nonblocking(stdout_pipe[0]) < 0 ||
        councis_set_nonblocking(stderr_pipe[0]) < 0 ||
        councis_write_handshake_byte(startup_pair[0]) < 0) {
        result->error_number = errno;
        (void)kill(-child, SIGKILL);
        (void)kill(child, SIGKILL);
    ...
    result->guard = guard;
    return COUNCIS_MCP_STDIO_GUARD_OK;
}

int councis_mcp_stdio_guard_poll(
    councis_mcp_stdio_guard_t *guard,
    councis_mcp_stdio_guard_poll_result_t *result) {
    ...
    do {
        ...
        for (size_t index = 0; index < observed_count; index++) {
            ...
            if (waited < 0) {
                ...
                if (errno == ECHILD) {
                    councis_mark_violation(guard, ECHILD);
                    continue;
                }
                councis_mark_violation(guard, errno);
                continue;
            }
            made_progress = 1;
            if (WIFEXITED(status) || WIFSIGNALED(status)) {
            ...
            if (event == PTRACE_EVENT_EXEC) {
                if (councis_handle_exec_event(guard, tracee) < 0) {
                    councis_mark_violation(guard, EACCES);
                }
            } else if (event == PTRACE_EVENT_SECCOMP) {
                if (councis_freeze_validate_and_execute_network(
    ...
    if (!guard->target_seen &&
        councis_all_tracees_exited(guard)) {
        guard->target_exited = 1;
    ...
    result->all_tracees_exited = councis_all_tracees_exited(guard);
    ...
    return guard->violation
        ? COUNCIS_MCP_STDIO_GUARD_POLICY_VIOLATION
        : COUNCIS_MCP_STDIO_GUARD_OK;
}

void councis_mcp_stdio_guard_destroy(
    councis_mcp_stdio_guard_t *guard) {
    if (guard == NULL) {
        return;
    }
