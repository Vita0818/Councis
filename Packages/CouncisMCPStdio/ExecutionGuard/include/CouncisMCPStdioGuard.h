#ifndef COUNCIS_MCP_STDIO_GUARD_H
#define COUNCIS_MCP_STDIO_GUARD_H
...
typedef struct councis_mcp_stdio_guard councis_mcp_stdio_guard_t;
...
typedef struct {
    ...
    uint64_t file_id;
    uint64_t byte_count;
} councis_mcp_stdio_exec_identity_t;

typedef struct {
    int mode;
    uint32_t gateway_ipv4_network_order;
    uint16_t gateway_port_host_order;
} councis_mcp_stdio_network_policy_t;

enum {
    COUNCIS_MCP_STDIO_NETWORK_DENIED = 0,
    COUNCIS_MCP_STDIO_NETWORK_EXACT_GATEWAY = 1
};
...
} councis_mcp_stdio_guard_spawn_result_t;
...
typedef struct {
    ...
    int error_number;
} councis_mcp_stdio_guard_poll_result_t;

enum {
    COUNCIS_MCP_STDIO_GUARD_OK = 0,
    COUNCIS_MCP_STDIO_GUARD_UNAVAILABLE = 1,
    COUNCIS_MCP_STDIO_GUARD_PIPE_FAILED = 2,
    COUNCIS_MCP_STDIO_GUARD_FORK_FAILED = 3,
    COUNCIS_MCP_STDIO_GUARD_TRACE_FAILED = 4,
    COUNCIS_MCP_STDIO_GUARD_IDENTITY_FAILED = 5,
    COUNCIS_MCP_STDIO_GUARD_POLICY_VIOLATION = 6
};
