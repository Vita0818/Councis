...
final class MCPStdioExactNetworkGateway: @unchecked Sendable {
    ...
    private let acceptQueue = DispatchQueue(
        label: "com.vitemis.councis.mcp.stdio.network.accept")
    private var acceptSource: DispatchSourceRead?
    private let workerQueue = DispatchQueue(
        label: "com.vitemis.councis.mcp.stdio.network.tunnel",
    ...
    init(origins: [String]) throws {
        ...
        let basicPayload = Data(
            "councis:\(credential)".utf8).base64EncodedString()
        let authorization = "Basic \(basicPayload)"
        let proxyURL =
            "http://councis:\(credential)@127.0.0.1"
