...
public actor ManagedPipeProcess: Transport {
    ...
    private init(
        ...
        self.logger = Logger(
            label: "com.vitemis.councis.mcp.stdio.\(ticket.request.authority.server.serverID.rawValue)")
    ...
    @unchecked Sendable
{
    ...
    func poll() -> MCPStdioLinuxGuardObservation {
        ...
        var result = councis_mcp_stdio_guard_poll_result_t()
        let status = councis_mcp_stdio_guard_poll(
            pointer,
            &result)
        return MCPStdioLinuxGuardObservation(
    ...
    deinit {
        if let pointer {
            councis_mcp_stdio_guard_destroy(pointer)
            self.pointer = nil
        }
    }
}
...
private func spawnLinuxGuardedPipeProcess(
    ...
    var helperIdentities = helpers.map { helper in
        var value = councis_mcp_stdio_exec_identity_t()
    ...
    guard status == COUNCIS_MCP_STDIO_GUARD_OK,
          result.process_id > 0,
        ...
        if let guardPointer = result.guard {
            councis_mcp_stdio_guard_destroy(guardPointer)
        }
        if status == COUNCIS_MCP_STDIO_GUARD_UNAVAILABLE
            || status
                == COUNCIS_MCP_STDIO_GUARD_TRACE_FAILED {
            throw MCPManagedPipeError
                .descendantExecutionGuardUnavailable
        }
