...
} CouncisPTYChildError;

static int councis_set_cloexec(int descriptor) {
...
static int councis_maximum_descriptor(void) {
    struct rlimit limit;
...
static void councis_report_child_error(
    int descriptor,
    int32_t stage,
    int32_t error_number,
    int exit_code) {
    CouncisPTYChildError payload = {
        .stage = stage,
...
static void councis_reset_child_signals(void) {
    sigset_t empty_mask;
...
static void councis_kill_and_reap(pid_t pid) {
...
int32_t councis_spawn_managed_pty(
    ...
    *result = (CouncisPTYSpawnResult){
        .pid = -1,
        .master_fd = -1,
        .error_stage = COUNCIS_PTY_STAGE_PREPARE,
        .error_number = 0
    };
    ...
    int pipe_error = councis_set_cloexec(error_pipe[0]);
    if (pipe_error == 0) {
        pipe_error = councis_set_cloexec(error_pipe[1]);
    }
    if (pipe_error != 0) {
    ...
    int maximum_descriptor = councis_maximum_descriptor();
    ...
    if (pid == 0) {
        ...
        if (error_descriptor != 3) {
            if (dup2(error_descriptor, 3) == -1) {
                councis_report_child_error(
                    error_descriptor,
                    COUNCIS_PTY_STAGE_PREPARE,
                    errno,
                    126);
            }
        ...
        int cloexec_error = councis_set_cloexec(error_descriptor);
        if (cloexec_error != 0) {
            councis_report_child_error(
                error_descriptor,
                COUNCIS_PTY_STAGE_PREPARE,
        ...
        councis_report_child_error(
            error_descriptor,
            COUNCIS_PTY_STAGE_EXEC,
            errno,
            127);
    }

    (void)close(error_pipe[1]);
    CouncisPTYChildError child_error = {
        .stage = COUNCIS_PTY_STAGE_NONE,
        .error_number = 0
    };
    ...
    int master_error = councis_set_cloexec(master_descriptor);
    if (master_error != 0) {
        (void)close(master_descriptor);
        councis_kill_and_reap(pid);
        return master_error;
    ...
    result->error_stage = COUNCIS_PTY_STAGE_NONE;
