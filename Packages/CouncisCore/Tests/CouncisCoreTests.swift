...
final class CouncisCoreTests: XCTestCase {

    func testProfilePresets() {
        XCTAssertTrue(PlatformProfile.macDeveloperID.allowsShell)
        XCTAssertFalse(PlatformProfile.restricted.allowsWorkspace)
        XCTAssertEqual(PlatformProfile.restricted.surfaces, [.chat])
        XCTAssertFalse(PlatformProfile.restricted.supports(.code))
        XCTAssertFalse(PlatformProfile.restricted.allowsMCPRemoteHTTP)
        XCTAssertFalse(PlatformProfile.restricted.allowsMCPStdio)
    ...
    private func temporarySessionRoot() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("councis-sessions-(UUID().uuidString)", isDirectory: true)
