...
final class CouncisHangDiagnosticsTests: XCTestCase {
    ...
    func testIntervalDurationRecordsOnceEvenWhenTokenIsCopied() {
        ...
        XCTAssertEqual(
            CouncisDiagnosticDurationBucket.allCases.reduce(0) {
    ...
    func testMetricsSnapshotDecodesLegacyCountersWithoutDurations() throws {
        ...
        let snapshot = try JSONDecoder().decode(
            CouncisDiagnosticMetricsSnapshot.self,
            from: legacy)

        XCTAssertEqual(snapshot.value(for: .projectionBatches), 2)
        for metric in CouncisDiagnosticDurationMetric.allCases {
            XCTAssertEqual(
    ...
    func testSanitizerRemovesPathsURLsAndCredentialShapes() {
        ...
        """
        let sanitized = CouncisHangDiagnosticTextSanitizer.sanitize(raw)
        ...
        for index in 0..<3 {
            let manifest = CouncisHangDiagnosticManifest(
                ...
                metrics: .init(counters: [
                    CouncisDiagnosticCounter.mainThreadIncidents.rawValue:
                        UInt64(index + 1),
                ]))
    ...
    func testProductionRetentionIsFiveBundlesAndTwentyMegabytes() {
        XCTAssertEqual(
            CouncisHangDiagnosticRetentionPolicy.production.maximumBundleCount,
            5)
        XCTAssertEqual(
            CouncisHangDiagnosticRetentionPolicy.production.maximumTotalBytes,
            20 * 1_024 * 1_024)
    }
    ...
    func testHangBundleRetentionAlsoPrunesByTotalBytes() async throws {
        ...
        for index in 0..<2 {
            let attachment = CouncisHangDiagnosticAttachment.sanitizedText(
                kind: .sample,
                rawData: Data(
    ...
    func testHangBundleSanitizesExternalTextAndLimitsAttachments() async throws {
        ...
        let store = CouncisHangDiagnosticBundleStore(rootURL: root)
    ...
    func testHangBundleRejectsSymlinkRoot() async throws {
        ...
        let store = CouncisHangDiagnosticBundleStore(rootURL: link)
        ...
        } catch let error as CouncisHangDiagnosticStoreError {
    ...
    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "councis-hang-diagnostics-\(UUID().uuidString)",
            isDirectory: true)
        try FileManager.default.createDirectory(
            at: url,
            withIntermediateDirectories: false)
