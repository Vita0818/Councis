...
public enum CouncisDiagnosticConstants {
    public static let subsystem = "com.Vita0818.Councis"
...
public enum CouncisPerformanceIntervalKind: String, Codable, CaseIterable, Sendable {
...
public enum CouncisDiagnosticDurationMetric:
    String,
    Codable,
    CaseIterable,
    Sendable
...
public enum CouncisDiagnosticDurationBucket:
    String,
    Codable,
    CaseIterable,
    Sendable
...
public struct CouncisDiagnosticDurationHistogram:
    Codable,
    Equatable,
    Sendable
{
    ...
    public func value(
        for bucket: CouncisDiagnosticDurationBucket
    ) -> UInt64 {
        switch bucket {
        case .under1Millisecond:
            return under1Millisecond
    ...
    fileprivate mutating func record(
        _ bucket: CouncisDiagnosticDurationBucket
    ) {
        switch bucket {
        case .under1Millisecond:
            under1Millisecond =
                councisSaturatingAdd(under1Millisecond, 1)
        case .oneToUnder4Milliseconds:
            oneToUnder4Milliseconds =
                councisSaturatingAdd(oneToUnder4Milliseconds, 1)
        case .fourToUnder16Milliseconds:
            fourToUnder16Milliseconds =
                councisSaturatingAdd(fourToUnder16Milliseconds, 1)
        case .sixteenToUnder50Milliseconds:
            sixteenToUnder50Milliseconds =
                councisSaturatingAdd(sixteenToUnder50Milliseconds, 1)
        case .fiftyToUnder100Milliseconds:
            fiftyToUnder100Milliseconds =
                councisSaturatingAdd(fiftyToUnder100Milliseconds, 1)
        case .oneHundredToUnder500Milliseconds:
            oneHundredToUnder500Milliseconds =
                councisSaturatingAdd(oneHundredToUnder500Milliseconds, 1)
        case .fiveHundredToUnder2000Milliseconds:
            fiveHundredToUnder2000Milliseconds =
                councisSaturatingAdd(fiveHundredToUnder2000Milliseconds, 1)
        case .atLeast2000Milliseconds:
            atLeast2000Milliseconds =
                councisSaturatingAdd(atLeast2000Milliseconds, 1)
        }
    }
}

public struct CouncisDiagnosticDurationAggregate:
    Codable,
    Equatable,
    Sendable
{
    ...
    public private(set) var histogram: CouncisDiagnosticDurationHistogram

    public init(
        count: UInt64 = 0,
        totalNanoseconds: UInt64 = 0,
        maximumNanoseconds: UInt64 = 0,
        histogram: CouncisDiagnosticDurationHistogram = .init()
    ) {
        self.count = count
    ...
    fileprivate mutating func record(durationNanoseconds: UInt64) {
        count = councisSaturatingAdd(count, 1)
        totalNanoseconds = councisSaturatingAdd(
            totalNanoseconds,
            durationNanoseconds)
        maximumNanoseconds = max(maximumNanoseconds, durationNanoseconds)
        histogram.record(
            CouncisDiagnosticDurationBucket.bucket(
                forNanoseconds: durationNanoseconds))
    }
}

/// Fixed-shape duration section embedded in a metrics snapshot.
public struct CouncisDiagnosticDurationSummaries:
    Codable,
    Equatable,
    Sendable
{
    public let projectionBatch: CouncisDiagnosticDurationAggregate
    public let projectionFold: CouncisDiagnosticDurationAggregate
    public let projectionCommit: CouncisDiagnosticDurationAggregate
    public let markdownQueueWait: CouncisDiagnosticDurationAggregate
    public let markdownParse: CouncisDiagnosticDurationAggregate
    public let markdownPublish: CouncisDiagnosticDurationAggregate

    public init(
        projectionBatch: CouncisDiagnosticDurationAggregate = .init(),
        projectionFold: CouncisDiagnosticDurationAggregate = .init(),
        projectionCommit: CouncisDiagnosticDurationAggregate = .init(),
        markdownQueueWait: CouncisDiagnosticDurationAggregate = .init(),
        markdownParse: CouncisDiagnosticDurationAggregate = .init(),
        markdownPublish: CouncisDiagnosticDurationAggregate = .init()
    ) {
    ...
    fileprivate init(
        values: [CouncisDiagnosticDurationMetric:
            CouncisDiagnosticDurationAggregate]
    ) {
        self.init(
    ...
    public func value(
        for metric: CouncisDiagnosticDurationMetric
    ) -> CouncisDiagnosticDurationAggregate {
        switch metric {
        case .projectionBatch:
...
private func councisSaturatingAdd(
    _ lhs: UInt64,
    _ rhs: UInt64
) -> UInt64 {
...
public struct CouncisPerformanceIntervalFields: Codable, Equatable, Sendable {
...
public enum CouncisScrollDiagnosticReason: Int, Codable, Sendable {
    case unknown = 0
    case liveContent = 1
    case completion = 2
    case terminal = 3
...
public enum CouncisScrollDiagnosticOutcome: Int, Codable, Sendable {
    case requested = 1
    case executed = 2
    case cancelled = 3
    case stale = 4
...
public enum CouncisDiagnosticCounter: String, Codable, CaseIterable, Sendable {
...
public enum CouncisCoworkAgentSwitchDiagnosticOutcome: Int, Codable, Sendable {
    case requested = 1
    case committed = 2
    case stale = 3
}

public struct CouncisDiagnosticMetricsSnapshot: Codable, Equatable, Sendable {
    public let counters: [String: UInt64]
    public let durations: CouncisDiagnosticDurationSummaries

    public init(
        counters: [String: UInt64],
        durations: CouncisDiagnosticDurationSummaries = .init()
    ) {
        self.counters = counters
        self.durations = durations
    }

    public func value(for counter: CouncisDiagnosticCounter) -> UInt64 {
    ...
    public init(from decoder: Decoder) throws {
        ...
        durations = try container.decodeIfPresent(
            CouncisDiagnosticDurationSummaries.self,
...
public struct CouncisPerformanceIntervalToken: @unchecked Sendable {
    public let kind: CouncisPerformanceIntervalKind
    fileprivate let completion: CouncisPerformanceIntervalCompletion
    ...
    #if canImport(os)
    fileprivate init(
        kind: CouncisPerformanceIntervalKind,
        state: OSSignpostIntervalState?,
        startedAtNanoseconds: UInt64
    ) {
        self.kind = kind
        self.state = state
        self.completion = CouncisPerformanceIntervalCompletion(
...
private final class CouncisPerformanceIntervalCompletion:
    @unchecked Sendable
{
...
public struct CouncisProjectionBatchMetrics:
    Codable,
    Equatable,
    Sendable
{
...
public struct CouncisProjectionBatchPublication:
    @unchecked Sendable,
    Equatable
{
    public let metrics: CouncisProjectionBatchMetrics
    private let interval:
        CouncisProjectionBatchInterval

    fileprivate init(
        metrics: CouncisProjectionBatchMetrics,
        interval: CouncisProjectionBatchInterval
    ) {
    ...
    public static func == (
        lhs: CouncisProjectionBatchPublication,
        rhs: CouncisProjectionBatchPublication
    ) -> Bool {
...
public final class CouncisProjectionBatchInterval:
    @unchecked Sendable
{
    private let diagnostics:
        CouncisPerformanceDiagnostics
    private let token:
        CouncisPerformanceIntervalToken
    private let lock = NSLock()
    private var isFinished = false
    private var sealedMetrics:
        CouncisProjectionBatchMetrics?
    ...
    public func seal(
        metrics: CouncisProjectionBatchMetrics
    ) -> CouncisProjectionBatchPublication {
        lock.lock()
        ...
        let canonicalMetrics =
            sealedMetrics ?? metrics
        lock.unlock()
        return CouncisProjectionBatchPublication(
...
public final class CouncisPerformanceDiagnostics: @unchecked Sendable {
    public static let shared = CouncisPerformanceDiagnostics()

    private let lock = NSLock()
    private var counters: [CouncisDiagnosticCounter: UInt64] = [:]
    private var durations:
        [CouncisDiagnosticDurationMetric: CouncisDiagnosticDurationAggregate] =
            [:]

    #if canImport(os)
    private let signposter = OSSignposter(
        subsystem: CouncisDiagnosticConstants.subsystem,
    ...
    public func increment(
        _ counter: CouncisDiagnosticCounter,
        by amount: UInt64 = 1
    ) {
    ...
    public func snapshot() -> CouncisDiagnosticMetricsSnapshot {
        lock.lock()
        let values = counters
        let durationValues = durations
        lock.unlock()
        return CouncisDiagnosticMetricsSnapshot(
            counters: Dictionary(
                uniqueKeysWithValues: values.map { ($0.key.rawValue, $0.value) }),
            durations: CouncisDiagnosticDurationSummaries(
                values: durationValues))
    }

    public func beginInterval(
        _ kind: CouncisPerformanceIntervalKind,
        fields: CouncisPerformanceIntervalFields = .init()
    ) -> CouncisPerformanceIntervalToken {
        let startedAtNanoseconds = DispatchTime.now().uptimeNanoseconds
        #if canImport(os)
        guard signposter.isEnabled else {
            return CouncisPerformanceIntervalToken(
                kind: kind,
                state: nil,
                startedAtNanoseconds: startedAtNanoseconds)
        }
    ...
    public func endInterval(
        _ token: CouncisPerformanceIntervalToken,
        fields: CouncisPerformanceIntervalFields = .init()
    ) {
    ...
    func recordDuration(
        _ metric: CouncisDiagnosticDurationMetric,
        nanoseconds: UInt64
    ) {
        lock.lock()
    ...
    private func durationMetric(
        for interval: CouncisPerformanceIntervalKind
    ) -> CouncisDiagnosticDurationMetric {
        switch interval {
        case .projectionBatch:
            return .projectionBatch
        case .markdownQueueWait:
    ...
    public func recordScrollRequest(
        reason: CouncisScrollDiagnosticReason,
        outcome: CouncisScrollDiagnosticOutcome,
        pendingCount: Int = 0
    ) {
        let counter: CouncisDiagnosticCounter
        switch outcome {
        case .requested:
    ...
    public func recordCoworkAgentSwitch(
        outcome: CouncisCoworkAgentSwitchDiagnosticOutcome,
...
public struct CouncisMainThreadHeartbeatConfiguration: Equatable, Sendable {
    ...
    public static let production = Self(
        warningAfterNanoseconds:
            CouncisDiagnosticConstants.heartbeatWarningMilliseconds * 1_000_000,
        incidentAfterNanoseconds:
            CouncisDiagnosticConstants.heartbeatIncidentMilliseconds * 1_000_000,
        incidentCooldownNanoseconds:
            CouncisDiagnosticConstants.heartbeatIncidentCooldownMilliseconds * 1_000_000)
}

public enum CouncisMainThreadHeartbeatAction: Equatable, Sendable {
...
public struct CouncisMainThreadHeartbeatStateMachine: Sendable {
    ...
    public let configuration: CouncisMainThreadHeartbeatConfiguration
    ...
    public init(
        configuration: CouncisMainThreadHeartbeatConfiguration = .production
    ) {
    ...
    public mutating func tick(
    ...
    ) -> [CouncisMainThreadHeartbeatAction] {
        ...
        let delay = now >= pending.enqueuedAtNanoseconds
            ...
            : 0
        var actions: [CouncisMainThreadHeartbeatAction] = []
...
public enum CouncisHangDiagnosticSource: String, Codable, Sendable {
    case mainThreadHeartbeat
    case externalCapture
}

public struct CouncisHangCaptureResult: Codable, Equatable, Sendable {
...
public struct CouncisHangDiagnosticManifest: Codable, Equatable, Sendable {
    public let schemaVersion: Int
    public let source: CouncisHangDiagnosticSource
    public let recordedAt: Date
    public let processIdentifier: Int32
    public let mainThreadDelayMilliseconds: UInt64?
    public let applicationVersion: String?
    public let buildVersion: String?
    public let metrics: CouncisDiagnosticMetricsSnapshot?
    public let capture: CouncisHangCaptureResult?

    public init(
        source: CouncisHangDiagnosticSource,
        recordedAt: Date,
        processIdentifier: Int32,
        mainThreadDelayMilliseconds: UInt64? = nil,
        applicationVersion: String? = nil,
        buildVersion: String? = nil,
        metrics: CouncisDiagnosticMetricsSnapshot? = nil,
        capture: CouncisHangCaptureResult? = nil
    ) {
...
public enum CouncisHangDiagnosticAttachmentKind: String, Codable, Sendable {
    case sample
    case unifiedLog
    case captureErrors
...
public struct CouncisHangDiagnosticAttachment: Sendable {
    public let kind: CouncisHangDiagnosticAttachmentKind
    fileprivate let data: Data

    private init(
        kind: CouncisHangDiagnosticAttachmentKind,
        data: Data
    ) {
        self.kind = kind
        self.data = data
...
public enum CouncisHangDiagnosticTextSanitizer {
...
public struct CouncisHangDiagnosticRetentionPolicy: Equatable, Sendable {
...
public struct CouncisHangDiagnosticBundleLocation: Equatable, Sendable {
...
public enum CouncisHangDiagnosticStoreError:
...
public actor CouncisHangDiagnosticBundleStore {
    ...
    public let retentionPolicy: CouncisHangDiagnosticRetentionPolicy

    public init(
        rootURL: URL,
        retentionPolicy: CouncisHangDiagnosticRetentionPolicy = .production
    ) {
    ...
    public func writeBundle(
        manifest: CouncisHangDiagnosticManifest,
        attachments: [CouncisHangDiagnosticAttachment] = []
    ) throws -> CouncisHangDiagnosticBundleLocation {
        guard manifest.schemaVersion == 1,
              manifest.processIdentifier > 0 else {
            throw CouncisHangDiagnosticStoreError.invalidManifest
        }
        ...
        for attachment in attachments {
            ...
            guard !result.overflow else {
                throw CouncisHangDiagnosticStoreError.bundleTooLarge
            }
        ...
        return try DurableOwnerOnlyFile.withExclusiveLock(
            ...
            do {
                ...
                return CouncisHangDiagnosticBundleLocation(
                    directoryURL: directory)
            } catch {
                try? Self.removeNewBundleIfSafe(directory)
                if error is CouncisHangDiagnosticStoreError {
                    throw error
                }
                throw CouncisHangDiagnosticStoreError.writeFailed
            }
        }
    }
    ...
    public func latestManifest(
        processIdentifier: Int32,
        recordedAfter: Date
    ) throws -> CouncisHangDiagnosticManifest? {
            ...
            for descriptor in descriptors {
                ...
                guard let data = try DurableOwnerOnlyFile.read(from: url),
                      let manifest = try? Self.manifestDecoder()
                        .decode(CouncisHangDiagnosticManifest.self, from: data),
    ...
    private func enforceRetentionLocked() throws {
        ...
        if descriptors.count > retentionPolicy.maximumBundleCount
            || totalBytes > retentionPolicy.maximumTotalBytes {
            throw CouncisHangDiagnosticStoreError.retentionFailed
        }
    }
    ...
    private static func ensureOwnerOnlyDirectory(_ url: URL) throws {
        ...
        if existed {
            guard isDirectory.boolValue else {
                throw CouncisHangDiagnosticStoreError.unsafeDirectory
            }
        } else {
    ...
    private static func safeBundleByteCount(_ directory: URL) -> Int? {
        ...
        let allowed = Set([
            "incident.json",
            CouncisHangDiagnosticAttachmentKind.sample.fileName,
            CouncisHangDiagnosticAttachmentKind.unifiedLog.fileName,
            CouncisHangDiagnosticAttachmentKind.captureErrors.fileName,
        ])
        for child in children {
            guard child.deletingLastPathComponent().standardizedFileURL
