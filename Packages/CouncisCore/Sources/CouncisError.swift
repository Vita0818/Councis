...
/// Unified error type across all Councis modules.
public enum CouncisError: Error, Sendable, Equatable, LocalizedError {
    case config(String)
...
/// `CouncisError.cancelled`; callers must therefore check both the error shape
/// and the current task's cancellation bit before treating a turn as
/// user-interrupted.
public enum CouncisCancellation {
    public static func isCancellationSignal(_ error: Error) -> Bool {
        if error is CancellationError {
            return true
        }
        guard let councisError = error as? CouncisError else {
            return false
        }
