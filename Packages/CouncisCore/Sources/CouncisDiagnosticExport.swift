...
public struct CouncisDiagnosticFileSnapshot: Equatable, Sendable {
...
public enum CouncisDiagnosticSnapshotError:
...
public enum CouncisDiagnosticSnapshotReader {
    ...
    public static func readTail(
        ...
        guard descriptor >= 0 else {
            if errno == ELOOP {
                throw CouncisDiagnosticSnapshotError.unsafeFile
            }
            throw CouncisDiagnosticSnapshotError.readFailed
        }
        ...
        return CouncisDiagnosticFileSnapshot(
            data: result,
...
public struct CouncisDiagnosticRedactedEventLog: Equatable, Sendable {
    public let data: Data
...
public enum CouncisDiagnosticEventLogRedactor {
    ...
    public static func redact(
        _ snapshot: CouncisDiagnosticFileSnapshot,
        maximumOutputBytes: Int = 8 * 1_024 * 1_024
    ) -> CouncisDiagnosticRedactedEventLog {
        let outputLimit = max(
            1_024,
        ...
        return CouncisDiagnosticRedactedEventLog(
            data: output,
    ...
    private static func redactEnvelope(
        ...
        for key in ["ts", "session", "type"] {
            ...
            output[key] = CouncisHangDiagnosticTextSanitizer.sanitize(
                value,
                maximumBytes: 1_024)
        }
