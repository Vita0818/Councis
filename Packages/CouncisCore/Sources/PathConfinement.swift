...
public enum PathConfinement {
    ...
    public static func resolve(_ path: String, within root: URL) throws -> URL {
        if isSensitivePath(path) {
            throw CouncisError.permissionDenied("sensitive path is not allowed: \(path)")
        }
    ...
    public static func canonicalExistingDirectory(_ url: URL) throws -> URL {
        ...
        guard FileManager.default.fileExists(atPath: canonical.path, isDirectory: &isDir), isDir.boolValue else {
            throw CouncisError.notFound("workspace is not an existing directory: \(url.path)")
        }
        if isSensitivePath(canonical.path) {
            throw CouncisError.permissionDenied("sensitive workspace is not allowed: \(canonical.path)")
        }
        return canonical
    }
    ...
    private static func isAgentConfigSecretPath(components: [String], base: String) -> Bool {
        if base == "auth.json",
           ...
           (components.contains("opencode")
                || components.contains("councis")
                || components.contains("intatis")) {
            return true
        }

        let configBasenames = Set([
            "opencode.json", "opencode.jsonc", "councis.json", "councis.jsonc",
            "intatis.json", "intatis.jsonc",
        ...
        return owner == "opencode" || owner == "councis"
            || owner == "intatis"
    }
    ...
    private static func requireInside(_ candidate: URL, root: URL, original: String) throws {
        ...
        guard candidatePath == rootPath || candidatePath.hasPrefix(prefix) else {
            throw CouncisError.permissionDenied("path escapes workspace: \(original)")
        }
    }
