...
public actor Orchestrator {
    ...
    public static let judgeAgentID = AgentID(rawValue: "judge")
    ...
    /// Optional host-owned internal tools. CouncisCowork remains independent
    /// of every concrete implementation (including CouncisKnowledge).
    ...
    public func attach(_ agent: Agent,
        ...
        guard id != Self.judgeAgentID else {
            try? await log.append(.error(ErrorPayload(
                code: "reserved_agent",
                message: "@\(id.rawValue) is a fixed Cowork identity")))
            return false
        }
        ...
        let attachGate = PermissionReviewGateSnapshot(
            ...
            policyVersion: "councis.workspace-admission.v1")
        let attachAuthorization = ResolvedToolAuthorization(
            authorizationID: IDGen.random(prefix: "tool-authorization"),
            registryVersion: "councis.cowork.admission.v1",
            concreteToolID: "councis.cowork.admission.v1/agent.attach",
    ...
    /// constructing the ordinary read-only Judge and no-tools reviewer responder
    /// does not issue a model request. The host must supply both fixed roles'
    /// models and exact inference bindings explicitly; configuration
    /// compatibility is resolved before this boundary, so bootstrap can never
    /// derive either route from the live @main identity. All three identities
    /// and leases remain distinct.
    @discardableResult
    public func bootstrapFreshSession(
        ...
        if let judgeInferenceBinding,
           judgeInferenceBinding.modelID != judgeModel {
            return .failed(
                "The Judge model must match its exact inference profile binding.")
        }
        guard judgeInferenceBinding != nil else {
            return .failed("Judge requires an exact inference profile binding.")
        }
        ...
        let reviewer = Agent(
            ...
            coordinationDepth: 0)
        let judge = Agent(
            name: Self.judgeAgentID,
            workspaceRoot: canonical,
            model: judgeModel,
            agentInferenceBinding: judgeInferenceBinding,
            profile: .readOnly,
            coordinationDepth: 0)
        ...
        do {
            _ = try await resolvedProvider(for: proposedMain)
            _ = try await resolvedProvider(for: judge)
            _ = try await reviewerProviderFactory()
        } catch {
            return .failed("The exact @main/Judge/reviewer inference profile is unavailable or incompatible.")
        }
        ...
        let reviewedJudgeCatalogBinding = judge.agentInferenceBinding.flatMap {
            availableInferenceProfiles[$0.inferenceProfileID]
        }
        let mainLeases = prepareDefaultLeases(for: proposedMain)
        let judgeLeases = prepareDefaultLeases(
            for: judge,
        ...
        let judgeCatalogBindingBeforeResolution = judge.agentInferenceBinding.flatMap {
            availableInferenceProfiles[$0.inferenceProfileID]
        }
        releaseAdmissionLock()
        ...
        do {
            ...
            return .failed("The exact @main/Judge/reviewer inference profile changed before durable admission.")
        }
        ...
        if requiresInferenceBindings {
            ...
            let liveJudgeCatalogBinding = judge.agentInferenceBinding.flatMap {
                availableInferenceProfiles[$0.inferenceProfileID]
            }
            ...
            guard reviewedJudgeCatalogBinding == judgeCatalogBindingBeforeResolution,
                  judgeCatalogBindingBeforeResolution == liveJudgeCatalogBinding else {
                return .failed(
                    "Judge host-approved inference profile changed before durable admission.")
            }
        }
        guard let mainRootIdentity = mainLeases.workspace.rootIdentity,
              ...
              let judgeRootIdentity = judgeLeases.workspace.rootIdentity,
              judgeRootIdentity.matchesCurrentDirectory(
                rootPath: judgeLeases.workspace.rootPath),
        ...
        let mainAgentMetadata = CoworkEventMetadata(
            ...
            capabilityLeaseID: mainLeases.capability.id,
            scope: .agent)
        let judgeWorkspaceMetadata = CoworkEventMetadata(
            agentID: judge.name,
            workspaceID: judgeLeases.workspace.workspaceID,
            workspaceLeaseID: judgeLeases.workspace.id,
            scope: .workspace)
        let judgeCapabilityMetadata = CoworkEventMetadata(
            agentID: judge.name,
            capabilityLeaseID: judgeLeases.capability.id,
            scope: .capability)
        let judgeAgentMetadata = CoworkEventMetadata(
            agentID: judge.name,
            workspaceID: judgeLeases.workspace.workspaceID,
            workspaceLeaseID: judgeLeases.workspace.id,
            capabilityLeaseID: judgeLeases.capability.id,
        ...
        do {
            let persisted = try await appendFreshSessionAdmissionEventsIfEmpty([
                ...
                .workspaceLeaseGranted(WorkspaceLeaseGrantedPayload(
                    agent: judge.name,
                    lease: judgeLeases.workspace,
                    metadata: judgeWorkspaceMetadata)),
                .capabilityLeaseCreated(CapabilityLeaseCreatedPayload(
                    agent: judge.name,
                    lease: judgeLeases.capability,
                    metadata: judgeCapabilityMetadata)),
                .agentAttached(AgentAttachedPayload(
                    agent: judge.name,
                    path: judge.workspaceRoot.path,
                    model: judge.model,
                    profile: judge.profile.rawValue,
                    agentInferenceBinding: judge.agentInferenceBinding,
                    metadata: judgeAgentMetadata)),
                .workspaceLeaseGranted(WorkspaceLeaseGrantedPayload(
                    agent: reviewer.name,
        ...
        registry.add(judge)
        commitDefaultLeases(judgeLeases, for: judge.name)
    ...
    public func detach(_ name: AgentID, reason: String = "agent detached") async -> Bool {
        ...
        guard name != Self.judgeAgentID else {
            try? await log.append(.error(ErrorPayload(
                code: "reserved_agent",
                message: "@\(Self.judgeAgentID.rawValue) is a fixed Cowork identity")))
            return false
        }
    ...
    public func rebindAgentInferenceProfile(
        ...
        guard agentID != Self.judgeAgentID else {
            return .failed("Judge uses a fixed configuration-owned inference binding")
        }
        ...
        if let idle = registry.all()
            .filter({ candidate in
                candidate.name != requestedBy
                    ...
                    && candidate.name != Self.judgeAgentID
    ...
    private func requireValidWorkTaskGraph() throws {
        if let workTaskRecoveryFailure {
            throw CouncisError.config(workTaskRecoveryFailure)
        }
    }
    ...
    private static func provenWorkTaskCreatePreflightRejection(
        ...
        let code: String
        let recovery: String
        if let councisError = error as? CouncisError {
            switch councisError {
    ...
    func createWorkTask(canManage: Bool,
        ...
        do {
            guard canManage else {
                throw CouncisError.permissionDenied("the current capability lease cannot create WorkTasks")
            }
            let title = request.title.trimmingCharacters(in: .whitespacesAndNewlines)
            let description = request.description.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !title.isEmpty, !description.isEmpty else {
                throw CouncisError.decoding("WorkTask title and description must be non-empty")
            }
    ...
    func updateWorkTask(currentWorkTaskID: WorkTaskID?,
        ...
        do {
            ...
            guard let current = workTaskGraph.task(request.taskID) else {
                throw CouncisError.notFound("WorkTask \(request.taskID.rawValue)")
            }
            ...
            guard hasMutation else {
                throw CouncisError.decoding("task_update requires at least one mutable field")
            }
        ...
        guard let updated = preparedGraph.task(updatedTaskID) else {
            throw CouncisError.notFound("WorkTask \(updatedTaskID.rawValue)")
        }
    ...
    func getWorkTask(currentWorkTaskID: WorkTaskID?,
        ...
        guard let task = workTaskGraph.task(taskID) else {
            throw CouncisError.notFound("WorkTask \(taskID.rawValue)")
        }
        guard canReadWorkTask(
            task,
            currentWorkTaskID: currentWorkTaskID,
            canManage: canManage) else {
            throw CouncisError.permissionDenied("WorkTask is outside the caller's readable scope")
        }
    ...
    func createGoal(request: GoalCreateRequest,
        ...
        guard !objective.isEmpty else {
            throw CouncisError.decoding("Goal objective must be non-empty")
        }
        if let tokenBudget = request.tokenBudget, tokenBudget <= 0 {
            throw CouncisError.decoding("Goal token budget must be greater than zero")
        }
        let sessionID = await log.sessionID
    ...
    func editGoal(request: GoalEditRequest,
        ...
        guard hostAuthorized else {
            throw CouncisError.permissionDenied("Goal edits are reserved for the user/host control plane")
        }
        let objective = request.objective.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !objective.isEmpty else {
            throw CouncisError.decoding("Goal objective must be non-empty")
        ...
        guard projection.currentGoalID == request.goalID,
            ...
            throw CouncisError.notFound("current Goal \(request.goalID.rawValue)")
        }
        let edited: Goal
    ...
    func transitionGoal(_ goalID: GoalID,
        ...
        guard projection.currentGoalID == goalID,
            ...
            throw CouncisError.notFound("current Goal \(goalID.rawValue)")
        }
        if status == .blocked {
    ...
    func settleGoalRunAudit(_ audit: GoalAuditSummary,
        ...
        guard projection.currentGoalID == goalID,
            ...
            throw CouncisError.notFound("active current Goal \(goalID.rawValue)")
        }
    ...
    func clearGoal(_ goalID: GoalID,
        ...
        guard hostAuthorized else {
            throw CouncisError.permissionDenied("clearing a Goal is reserved for the user/host control plane")
        }
        await acquireAdmissionLock()
        defer { releaseAdmissionLock() }
        let projection = CoworkProjection.build(from: await log.replay())
    ...
    func spawnFromTool(requestedBy: AgentID,
        ...
        guard id != Self.judgeAgentID else {
            return "error: @\(Self.judgeAgentID.rawValue) is a fixed Cowork identity"
        }
    ...
    private func prepareAuthorization(
        ...
        guard let requesterID = request.invocation.agent,
            ...
            throw CouncisError.permissionDenied("delegating agent is not attached")
        }
        ...
        if let suppliedTarget {
            ...
            guard candidate != requesterID,
                ...
                throw CouncisError.permissionDenied("requested delegation target is reserved")
            }
        ...
        if let failure = delegationFailure(
            ...
            throw CouncisError.permissionDenied(failure)
        }
    ...
    private func prepareSpawnAuthorization(
        ...
        guard let requesterID = request.invocation.agent,
              let requester = registry.agent(requesterID) else {
            throw CouncisError.permissionDenied("spawning agent is not attached")
        }
        let arguments = try JSONDecoder().decode(
    ...
    private func semanticAuthorizationFailure(
        ...
        case "spawn_agent":
            if let target = targetAgent() {
                ...
                if target == Self.mainAgentID
                    || target == Self.judgeAgentID
                    || target == Self.automaticPermissionReviewerID {
        ...
        -> ToolRegistry
    {
        ...
        return ToolRegistry(
            ...
            registryVersion: "councis.cowork.v4")
    }
