...
private func e2eLog() throws -> EventLog {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("councis-cowork-e2e-\(UUID().uuidString)", isDirectory: true)
...
private func makeCounterFixture() throws -> CounterFixture {
    ...
    let root = fm.temporaryDirectory
        .appendingPathComponent("councis-cowork-count-fixture-\(UUID().uuidString)", isDirectory: true)
    let macSources = root
        .appendingPathComponent("Apps", isDirectory: true)
        .appendingPathComponent("CouncisMac", isDirectory: true)
        .appendingPathComponent("Sources", isDirectory: true)
    let cliSources = root
        .appendingPathComponent("Apps", isDirectory: true)
        .appendingPathComponent("councis-cli", isDirectory: true)
        .appendingPathComponent("Sources", isDirectory: true)
    ...
    return CounterFixture(
        root: root,
        macOSRoot: root.appendingPathComponent("Apps/CouncisMac", isDirectory: true),
        cliRoot: root.appendingPathComponent("Apps/councis-cli", isDirectory: true))
}
...
final class CoworkEndToEndTests: XCTestCase {
    ...
    func testMacOSCLISwiftFileCountScenarioClosesThroughSchedulerAndProjection() async throws {
        ...
        let log = try e2eLog()
        let macOSProvider = E2EProvider(text: "macOS Swift count: 2\nApps/CouncisMac/Sources/A.swift\nApps/CouncisMac/Sources/B.swift")
        let cliProvider = E2EProvider(text: "CLI Swift count: 3\nApps/councis-cli/Sources/C.swift\nApps/councis-cli/Sources/D.swift\nApps/councis-cli/Sources/E.swift")
        ...
        let globalRequest = "拉起两个子 Agent，分别统计 macOS App 和 CLI Swift 文件。"
        try await log.append(.userMessage(UserMessagePayload(text: globalRequest)))
        let rootTaskIDOptional = await orch.createRootTask(
            assignee: main,
            objective: "Count Swift files for the macOS App and CLI.",
            roleHint: "Swift count coordinator",
            expectedDeliverable: "Combined macOS App and CLI Swift file count summary.")
        let rootTaskID = try XCTUnwrap(rootTaskIDOptional)

        let macOSObjective = "Recursively count Swift files in Apps/CouncisMac only. Do not count CLI files."
        let cliObjective = "Recursively count Swift files in Apps/councis-cli only. Do not count macOS App files."
        ...
        XCTAssertTrue(macOSContract.objective.contains("CouncisMac"))
        XCTAssertFalse(macOSContract.objective.contains("councis-cli"))
        XCTAssertTrue(cliContract.objective.contains("councis-cli"))
        XCTAssertFalse(cliContract.objective.contains("CouncisMac only"))
