...
private func autoReviewTempLog() throws -> EventLog {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("councis-auto-review-\(UUID().uuidString)", isDirectory: true)
...
final class AutomaticPermissionReviewTests: XCTestCase {
    ...
    private func phaseSJudgeBinding(
        revision: String = "revision-1",
        connectionRevision: String = "connection-revision-1",
        model: String = "phase-s-judge-model",
        fingerprint: String = "sha256:phase-s-judge-safe-fingerprint"
    ) -> AgentInferenceBinding {
        AgentInferenceBinding(
            inferenceProfileRef: InferenceProfileRef(
    ...
    func testPhaseSFreshBootstrapPersistsSettingsAndThreeRegistrationsInOneBatch() async throws {
        let log = try autoReviewTempLog()
        let ws = try autoReviewWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        ...
        let projection = CoworkProjection.build(from: events)
        XCTAssertEqual(Set(projection.agentRoster.keys), Set([main, judge, reviewer]))
        XCTAssertEqual(projection.agentRoster[main]?.agentInferenceBinding, binding)
        XCTAssertEqual(projection.agentRoster[judge]?.agentInferenceBinding, binding)
        ...
        XCTAssertFalse(judgeCapabilities[0].tools.contains(.applyPatch))
        XCTAssertFalse(judgeCapabilities[0].tools.contains(.controlRun))
        XCTAssertEqual(judgeCapabilities[0].delegation, .none)
        ...
        XCTAssertNotEqual(judgeWorkspaceID, reviewerWorkspaceID)
    ...
    func testFixedJudgeIsExplicitReadonlyAndCannotBeMutatedOrAutoSelected() async throws {
        let log = try autoReviewTempLog()
        let ws = try autoReviewWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let binding = phaseSBinding()
        let provider = AutoReviewCapturingProvider([
            .textDelta("Candidate A is better supported."),
            .done(finishReason: "stop"),
        ])
        let orch = Orchestrator(
            log: log,
        ...
        let automatic = await orch.delegateTask(
            from: main,
            to: nil,
            objective: "Evaluate candidate A.")
        XCTAssertTrue(automatic.contains("no available attached delegation worker"))
        XCTAssertTrue(provider.requests.isEmpty)

        let explicit = await orch.delegateTask(
            from: main,
            to: judge.rawValue,
            objective: "Compare candidate A with the stated objective.")
        XCTAssertTrue(explicit.contains("agent_id=@judge"))
        XCTAssertTrue(explicit.contains("Candidate A is better supported."))
        let request = try XCTUnwrap(provider.requests.first)
        let toolNames = Set(request.tools.map(\.name))
        XCTAssertTrue((request.messages.first?.content ?? "").contains("fixed @judge"))
        ...
        XCTAssertFalse(toolNames.contains("finish_run"))
        XCTAssertFalse(toolNames.contains("stop_run"))
        XCTAssertFalse(toolNames.contains("apply_patch"))

        let detached = await orch.detach(judge)
        XCTAssertFalse(detached)
        let rebind = await orch.rebindAgentInferenceProfile(
            agentID: judge,
            binding: phaseSJudgeBinding(revision: "revision-2"),
            hostAuthorized: true)
        guard case .failed = rebind else {
            return XCTFail("Judge rebind must be rejected")
        }
        let spawn = await orch.spawnFromTool(
            requestedBy: main,
            name: judge.rawValue,
            path: ws.path)
        XCTAssertTrue(spawn.contains("fixed Cowork identity"))
        let remove = await orch.removeFromTool(
            requestedBy: main,
            currentTaskID: nil,
            name: judge.rawValue)
        XCTAssertTrue(remove.contains("fixed Cowork identity"))
        let agents = await orch.agentList()
        XCTAssertNotNil(agents.first { $0.name == judge })
    }

    func testPhaseSFreshBootstrapPersistsDistinctFixedRoleExactBindings() async throws {
        let log = try autoReviewTempLog()
        let ws = try autoReviewWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        ...
        let orch = Orchestrator(
            ...
            availableInferenceProfiles: [mainBinding, judgeBinding, reviewerBinding],
        ...
        XCTAssertEqual(projection.agentRoster[judge]?.model, judgeBinding.modelID)
        XCTAssertEqual(
        ...
        XCTAssertNotEqual(
            ...
            projection.agentRoster[reviewer]?.agentInferenceBinding)
        XCTAssertEqual(
            projection.agentRoster[judge]?.profile,
    ...
    func testPhaseSFreshBootstrapRejectsMissingJudgeBindingBeforeEvents() async throws {
        let log = try autoReviewTempLog()
        let ws = try autoReviewWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let binding = phaseSBinding()
        ...
        let result = await orch.bootstrapFreshSession(
            main: Agent(
                ...
                coordinationDepth: Agent.defaultCoordinationDepth),
            settings: settings,
            judgeModel: phaseSJudgeBinding().modelID,
            judgeInferenceBinding: nil,
            permissionReviewerModel: binding.modelID,
            permissionReviewerInferenceBinding: binding)

        guard case .failed = result else {
            return XCTFail("a strict bootstrap without a Judge binding must fail closed")
        }
        XCTAssertTrue(provider.requests.isEmpty)
        let events = try await log.replayChecked()
    ...
    func testPhaseSFreshBootstrapRejectsMismatchedJudgeModelBeforeEvents() async throws {
        let log = try autoReviewTempLog()
        let ws = try autoReviewWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let binding = phaseSBinding()
        ...
        let result = await orch.bootstrapFreshSession(
            main: Agent(
                ...
                coordinationDepth: Agent.defaultCoordinationDepth),
            settings: settings,
            judgeModel: ModelID(rawValue: "wrong-judge-model"),
            judgeInferenceBinding: judgeBinding,
            permissionReviewerModel: binding.modelID,
            permissionReviewerInferenceBinding: binding)

        guard case .failed = result else {
            return XCTFail("a mismatched Judge model/binding tuple must fail closed")
        }
        XCTAssertTrue(provider.requests.isEmpty)
        let events = try await log.replayChecked()
        XCTAssertTrue(events.isEmpty)
        ...
        let bootstrap = Task {
            await orch.bootstrapFreshSession(
                main: Agent(
                    ...
                    coordinationDepth: Agent.defaultCoordinationDepth),
                settings: settings,
                judgeModel: mainBinding.modelID,
                judgeInferenceBinding: mainBinding,
                permissionReviewerModel: reviewerBinding.modelID,
                permissionReviewerInferenceBinding: reviewerBinding)
        }
    ...
    func testPhaseSTwoEventLogInstancesCannotBothBootstrapTheSameFreshSession() async throws {
        ...
        XCTAssertEqual(totalRuntimeAgents, 3, "only the winning runtime may register @main, @judge, and reviewer")
    }
        ...
        for variant in variants {
            let directory = FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "councis-bootstrap-\(variant.suffix)-\(UUID().uuidString)",
                    isDirectory: true)
