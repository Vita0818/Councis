...
@testable import CouncisConversation
...
final class CouncisConversationTests: XCTestCase {

    private func tmpFile() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("councis-conv-\(UUID().uuidString)", isDirectory: true)
        ...
        let loop = ChatLoop(
            ...
            attachmentResolver: { ids in
                guard ids == [firstID] else {
                    throw CouncisError.notFound("unexpected attachment set")
                }
                return [firstImage]
            })
