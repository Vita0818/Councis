...
final class DocumentToolsIntegrationTests: XCTestCase {
    ...
    func testInstalledDocumentRuntimeEPUBWriteWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_DOCUMENT_RUNTIME_SMOKE"] == "1" else {
            throw XCTSkip(
                "set COUNCIS_REAL_DOCUMENT_RUNTIME_SMOKE=1 to run the installed document runtime smoke")
        }
        guard let runtime = councisDocumentRuntimeRoot(),
              fileManager.isExecutableFile(
                  atPath: runtime.appendingPathComponent("bin/councis-rbook-helper").path),
              fileManager.isExecutableFile(
                  atPath: runtime.appendingPathComponent("bin/councis-epubcheck").path) else {
        ...
        let observation = try await DocumentWriteTool().execute(
            ToolArgs(raw: #"""
            {
              ...
              "operations":[
                {"kind":"metadata.set","parameters":{"field":"identifier","value":"urn:councis:runtime-smoke"}},
                {"kind":"metadata.set","parameters":{"field":"title","value":"Councis Runtime Smoke"}},
    ...
    func testInstalledDocumentRuntimeCoreToolChainWhenEnabled() async throws {
        ...
        guard let runtime = councisDocumentRuntimeRoot(),
              let libreOffice = councisLibreOfficeRuntimeAppURL()?
        ...
        let context = ToolContext(
            workspaceRoot: workspace,
            documentBackend: DocumentBackendProcessRunner(timeoutSeconds: 300))
        let marker = "COUNCIS DOCUMENT RUNTIME 73921"

        let write = try await DocumentWriteTool().execute(
            ToolArgs(raw: #"""
            {"format":"docx","mode":"create","output_path":"runtime-smoke.docx",
             "operations":[{"kind":"paragraph.add","parameters":{"text":"COUNCIS DOCUMENT RUNTIME 73921"}}]}
            """#),
            in: context)
        ...
        let htmlMarker = "COUNCIS HTML RUNTIME 18426"
        ...
        let slideMarker = "COUNCIS PRESENTATION RUNTIME 48216"
        ...
        let xlsxWrite = try await DocumentWriteTool().execute(
            ...
            {"format":"xlsx","mode":"create","output_path":"runtime-smoke.xlsx",
             "operations":[
              ...
              {"kind":"cell.set","parameters":{"sheet":"Sheet","cell":"E3","value":"COUNCIS SHEET RUNTIME 93715"}}
             ]}
            """#),
            in: context)
        XCTAssertTrue(
        ...
        let xlsxRead = try await ReadXLSXTool().execute(
            ...
            in: context)
        XCTAssertTrue(xlsxRead.text.contains("COUNCIS SHEET RUNTIME 93715"), xlsxRead.text)
    ...
    func testInstalledDocumentRuntimeReadsUserCorpusWhenConfigured() async throws {
        guard let rawRoot = ProcessInfo.processInfo.environment[
            "COUNCIS_REAL_DOCUMENT_CORPUS_ROOT"],
              !rawRoot.isEmpty else {
            throw XCTSkip(
                "set COUNCIS_REAL_DOCUMENT_CORPUS_ROOT to run the external document corpus smoke")
        }
        guard let runtime = councisDocumentRuntimeRoot(),
              fileManager.isExecutableFile(
    ...
    func testInstalledDocumentRuntimePDFCPUAndOCRWhenEnabled() async throws {
        ...
        let stage = workspace.appendingPathComponent(
            ".councis-document-stage-runtime-smoke",
            isDirectory: true)
        ...
        let digest = try DocumentInputFile.freeze(
            path: ".councis-document-stage-runtime-smoke/ocr-source.pdf",
            expectedFormat: .pdf,
            workspace: workspace).identity.sha256
        let observation = try await DocumentOCRTool().execute(
            ToolArgs(raw: """
            {"input_path":".councis-document-stage-runtime-smoke/ocr-source.pdf",
        ...
        XCTAssertTrue(normalized.contains("COUNCISOCR48291"), observation.text)
        #else
    ...
    func testStandardRegistryExposesSplitDocumentReaders() throws {
        ...
        XCTAssertEqual(registry.registryVersion, "councis.standard.v4")
    }
    ...
    func testPDFRenderCommitsCompleteBundleWithoutLeakingStageDirectory() async throws {
        ...
        XCTAssertFalse(rootChildren.contains { $0.hasPrefix(".councis-document-stage-") })
        #else
        throw XCTSkip("PDFKit integration requires Apple PDF frameworks")
        #endif
    }
    ...
    func testDocumentProcessLeaseNarrowsBroadWorkspaceAuthorityToReviewedInputAndStage() throws {
        ...
        let narrowed = try documentProcessLease(
            ...
            internalWritablePaths: ["out/.councis-document-stage-test"])

        XCTAssertEqual(narrowed.access, .readWrite)
        XCTAssertEqual(
            Set(narrowed.allowedPathRules.map(\.pattern)),
            ["source.docx", "out/.councis-document-stage-test"])
        ...
        XCTAssertTrue(profile.contains("councis-document-stage-test"))
    ...
    func testLibreOfficeSandboxAllowsOnlyInvocationPrivateUnixSocket() throws {
        ...
        defer { try? fileManager.removeItem(at: workspace) }
        let socketRoot = URL(
            fileURLWithPath: "/private/tmp/councis-lo-0123456789ab",
            isDirectory: true)
        ...
        XCTAssertTrue(profile.contains(#"(subpath "/private/tmp/councis-lo-0123456789ab")"#))
    ...
    private func makeWorkspace() throws -> URL {
        let url = fileManager.temporaryDirectory.appendingPathComponent(
            "councis-document-tool-integration-\(UUID().uuidString)",
            isDirectory: true)
