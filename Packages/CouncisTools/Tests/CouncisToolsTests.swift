...
private actor ShellResultQueue {
    ...
    func next() throws -> ShellResult {
        guard results.isEmpty == false else {
            throw CouncisError.io("no fake shell results remain")
        }
...
private struct FakeGit: GitService {
    ...
    var commitsText: String = "abc123\tCouncis\t2026-01-01\tInitial"
...
final class CouncisToolsTests: XCTestCase {
    func testCheckedInternalToolCloseFailsAndRemainsSingleFlight() async {
        ...
        } catch let error as CouncisError {
    ...
    private func makeBlankPDF(pageCount: Int, at url: URL) throws {
        ...
        for index in 0..<pageCount {
            ...
            guard let page = PDFPage(image: image) else {
                throw CouncisError.io("could not create test PDF page \(index)")
            }
            document.insert(page, at: index)
        }
        guard document.write(to: url) else {
            throw CouncisError.io("could not write test PDF")
        }
    }
    #endif
    ...
    private func freeLoopbackPort() throws -> Int {
        ...
        guard descriptor >= 0 else {
            throw CouncisError.io("could not create loopback socket")
        }
        ...
        guard bindResult == 0 else {
            throw CouncisError.io("could not bind loopback socket")
        }
        ...
        guard nameResult == 0 else {
            throw CouncisError.io("could not inspect loopback socket")
        }
    ...
    private func startStaticHTTPServer(directory: URL, port: Int) throws -> Process {
        guard let python = python3Executable() else {
            throw CouncisError.io("python3 is required for the real browser persistence smoke")
        }
    ...
    private func waitForHTTPServer(port: Int, path: String = "/login.html") async throws {
        ...
        throw CouncisError.io("local browser smoke server did not start")
    }
    #endif
    ...
    func testOrdinaryFileToolsCannotBypassManagedKnowledgePublication()
        ...
        let snapshots = store.appendingPathComponent(
            ".councis-rag-snapshots",
            isDirectory: true)
        try FileManager.default.createDirectory(
            at: snapshots,
            withIntermediateDirectories: true)
        let pointer = store.appendingPathComponent(".councis-rag-store.json")
        ...
        do {
            _ = try await ReadFileTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.councis-rag-store.json"}"#),
                in: context)
        ...
        do {
            _ = try await ListFilesTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.councis-rag-snapshots"}"#),
                in: context)
        ...
        do {
            _ = try await SearchTextTool().execute(
                ToolArgs(raw: #"{"query":"snapshot","path":"knowledge/.councis-rag-snapshots"}"#),
                in: context)
        ...
        do {
            _ = try await WriteFileTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.councis-rag-store.json","content":"changed"}"#),
                in: context)
        ...
        let diff = [
            "--- a/knowledge/.councis-rag-snapshots/profile.json",
            "+++ b/knowledge/.councis-rag-snapshots/profile.json",
            "@@ -1 +1 @@",
            "-snapshot-original",
            "+changed",
        ].joined(separator: "\n")
    ...
    func testStructuredProcessShellRunnerUsesSanitizedEnvironmentAndWorkspaceLease() async throws {
        ...
        setenv("COUNCIS_HOST_SECRET_MARKER", "host-secret-marker", 1)
        defer { unsetenv("COUNCIS_HOST_SECRET_MARKER") }
        let runner = StructuredProcessShellRunner(timeoutSeconds: 5)

        let ordinary = try await runner.run("/bin/cat ordinary.txt; printf '|%s' \"$COUNCIS_HOST_SECRET_MARKER\"", cwd: ws)
    ...
    func testBrowserBackendProcessRunnerUsesSanitizedEnvironment() async throws {
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        setenv("COUNCIS_BROWSER_HOST_SECRET_MARKER", "host-secret-marker", 1)
        defer { unsetenv("COUNCIS_BROWSER_HOST_SECRET_MARKER") }

        let invocation = BrowserBackendInvocation(
            javaScript: #"process.stdout.write(`${process.env.COUNCIS_BROWSER_HOST_SECRET_MARKER || ""}|${process.env.COUNCIS_BROWSER_PROCESS_BOUNDARY || ""}`);"#,
    ...
    func testProcessShellRunnerDeniesWorkspaceExternalFile() async throws {
        ...
        let outside = ws.deletingLastPathComponent()
            .appendingPathComponent("councis-outside-\(UUID().uuidString).txt")
        defer {
            try? FileManager.default.removeItem(at: ws)
        ...
        let escapedWrite = ws.deletingLastPathComponent()
            .appendingPathComponent("councis-escaped-write-\(UUID().uuidString).txt")
    ...
    func testProcessGitServiceRejectsExternalDiffWithoutExecutingIt() async throws {
        ...
        let outsideConfig = ws.deletingLastPathComponent().appendingPathComponent("councis-included-\(UUID().uuidString).gitconfig")
    ...
    func testProcessGitServiceAuditsWorktreeConfigAndRejectsSymlink() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("councis-config-worktree-\(UUID().uuidString)")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
        }
    ...
    func testProcessGitServiceHonorsReadOnlyAndDeniedLeaseData() async throws {
        ...
        let setup = try await shell.run(
            "'\(git)' init; '\(git)' config user.email councis@example.invalid; '\(git)' config user.name Councis; '\(git)' add ordinary.txt .env; '\(git)' commit -m base",
            cwd: ws)
        XCTAssertEqual(setup.exitCode, 0, setup.stderr)
    ...
    func testProcessGitServiceStagesAndCommitsTempRepoWhenGitAvailable() async throws {
        #if canImport(Darwin)
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_GIT_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_GIT_SMOKE=1 to run the real ProcessGitService smoke")
        }
        ...
        _ = try await shell.run("git config user.email councis@example.invalid", cwd: ws)
        _ = try await shell.run("git config user.name 'Councis Test'", cwd: ws)
        ...
        _ = try await shell.run("git config user.email councis@example.invalid", cwd: patchSource)
        _ = try await shell.run("git config user.name 'Councis Test'", cwd: patchSource)
        ...
        let worktreeURL = ws.appendingPathComponent(".councis/git-worktrees/task-1", isDirectory: true)
        ...
        XCTAssertTrue(worktreeInfo.contains("/.councis/git-worktrees/task-1"), worktreeInfo)
    ...
    func testCompileLatexCannotReadOutsideWorkspace() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("councis-tex-outside-\(UUID().uuidString).tex")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
        }
    ...
    func testBrowserNavigateUsesPersistentProfileStateAndHistory() async throws {
        ...
        XCTAssertTrue(obs.text.contains("Persistent browser profile: .councis/browser/profiles/work"))
        let stateURL = ws.appendingPathComponent(".councis/browser/state/work.json")
        let historyURL = ws.appendingPathComponent(".councis/browser/history.jsonl")
    ...
    func testBrowserHandoffUsesHeadedPersistentProfileAndRecordsHistory() async throws {
        ...
        XCTAssertTrue(obs.text.contains("Persistent browser profile: .councis/browser/profiles/login"))
        ...
        let stateURL = ws.appendingPathComponent(".councis/browser/state/login.json")
    ...
    func testBrowserConcurrentProfilesKeepSeparateStateAndHistoryMetadata() async throws {
        ...
        let stateAURL = ws.appendingPathComponent(".councis/browser/state/profile-a.json")
        let stateBURL = ws.appendingPathComponent(".councis/browser/state/profile-b.json")
    ...
    func testRealBrowserBackendSmokeWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser backend smoke")
        }
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let ctx = ToolContext(workspaceRoot: ws)
        ...
        XCTAssertTrue(obs.text.contains("Persistent browser profile: .councis/browser/profiles/smoke"))
    }

    func testRealCDPBrowserIgnoresStaleDevToolsActivePortWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the stale DevToolsActivePort smoke")
        }
        ...
        let profileDirectory = ws
            .appendingPathComponent(".councis/browser/profiles/stale-port-smoke", isDirectory: true)
        try FileManager.default.createDirectory(
            at: profileDirectory,
            withIntermediateDirectories: true)
    ...
    func testRealBrowserSearchWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser search smoke")
        }
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let ctx = ToolContext(workspaceRoot: ws)
    ...
    func testRealBrowserHandoffWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_HANDOFF_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_HANDOFF_SMOKE=1 to run the headed browser handoff smoke")
        }
    ...
    func testRealBrowserProfilePersistsCookieLocalStorageAndHistoryWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser profile persistence smoke")
        }
        ...
        let loginHTML = """
        <!doctype html>
        <title>Councis Login Set</title>
        <body>pending</body>
        <script>
        document.cookie = "councis_login=present; Max-Age=3600; path=/; SameSite=Lax";
        localStorage.setItem("councisLocalLogin", "present");
        document.body.innerText = "login marker set";
        </script>
        """
        let stateHTML = """
        <!doctype html>
        <title>Councis Login State</title>
        <body>pending</body>
        <script>
        document.body.innerText = "cookie=" + document.cookie + "\\nlocal=" + localStorage.getItem("councisLocalLogin");
        </script>
        """
        ...
        XCTAssertTrue(obs.text.contains("cookie=councis_login=present"), obs.text)
    ...
    func testRealBrowserBackForwardWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser back/forward smoke")
        }
        ...
        let oneHTML = """
        <!doctype html>
        <title>Councis History One</title>
        <body>history page one marker</body>
        """
        let twoHTML = """
        <!doctype html>
        <title>Councis History Two</title>
    ...
    func testRealBrowserProfilesRemainIsolatedWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser profile isolation smoke")
        }
        ...
        let profileHTML = """
        <!doctype html>
        <title>Councis Profile Isolation</title>
        <body>pending</body>
        <script>
        const params = new URLSearchParams(window.location.search);
        const marker = params.get("marker");
        if (marker) {
          document.cookie = "councis_profile_marker=" + marker + "; Max-Age=3600; path=/; SameSite=Lax";
          localStorage.setItem("councisProfileMarker", marker);
        }
        document.body.innerText = "cookie=" + document.cookie + "\\nlocal=" + localStorage.getItem("councisProfileMarker");
        </script>
        """
        ...
        XCTAssertTrue(obsA.text.contains("councis_profile_marker=profile-a-marker"), obsA.text)
        ...
        XCTAssertTrue(obsB.text.contains("councis_profile_marker=profile-b-marker"), obsB.text)
    ...
    func testRealBrowserDifferentProfilesCanLaunchConcurrentlyWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_CONCURRENCY_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_CONCURRENCY_SMOKE=1 to run the real browser concurrent profile smoke")
        }
        ...
        let pageA = """
        <!doctype html>
        <title>Councis Concurrent A</title>
        <body>
        ...
        let pageB = """
        <!doctype html>
        <title>Councis Concurrent B</title>
        <body>
        ...
        let stateAURL = ws.appendingPathComponent(".councis/browser/state/concurrent-a.json")
        let stateBURL = ws.appendingPathComponent(".councis/browser/state/concurrent-b.json")
    ...
    func testRealBrowserUploadDownloadWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser upload/download smoke")
        }
        ...
        let formHTML = """
        <!doctype html>
        <title>Councis Upload Download</title>
        <body>
        ...
        let stateURL = ws.appendingPathComponent(".councis/browser/state/io-smoke.json")
        try FileManager.default.createDirectory(at: stateURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        let statePayload: [String: String] = [
            "profile": "io-smoke",
            "url": pageURL,
            "title": "Councis Upload Download",
        ...
        XCTAssertTrue(downloadedPath.hasPrefix(".councis/browser/downloads/io-smoke/"), downloadedPath)
        ...
        XCTAssertTrue(listed.text.contains(".councis/browser/downloads/io-smoke/"), listed.text)
    ...
    func testRealBrowserSelectAndPressKeyWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser select/press smoke")
        }
        ...
        let formHTML = """
        <!doctype html>
        <title>Councis Select Press</title>
        <body>
        ...
        try await waitForHTTPServer(port: port, path: "/index.html")
        let pageURL = "http://127.0.0.1:\(port)/index.html"
        let stateURL = ws.appendingPathComponent(".councis/browser/state/form-smoke.json")
        try FileManager.default.createDirectory(at: stateURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        let statePayload: [String: String] = [
            "profile": "form-smoke",
            "url": pageURL,
            "title": "Councis Select Press",
            "updatedAt": ISO8601DateFormatter().string(from: Date()),
        ]
        let stateData = try JSONSerialization.data(withJSONObject: statePayload, options: [.prettyPrinted, .sortedKeys])
        try stateData.write(to: stateURL, options: .atomic)
    ...
    func testRealBrowserSubmitWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser submit smoke")
        }
        ...
        let formHTML = """
        <!doctype html>
        <title>Councis Submit Form</title>
        <body>
        ...
        let submittedHTML = """
        <!doctype html>
        <title>Councis Submit Result</title>
    ...
    func testRealBrowserPopupNewPageWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser popup/new-page smoke")
        }
        ...
        let indexHTML = """
        <!doctype html>
        <title>Councis Popup Source</title>
        <body>
        ...
        let popupHTML = """
        <!doctype html>
        <title>Councis Popup Result</title>
    ...
    func testRealBrowserScrollAndWaitWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser scroll/wait smoke")
        }
        ...
        let pageHTML = """
        <!doctype html>
        <title>Councis Scroll Wait</title>
        <style>
        ...
        let stateURL = ws.appendingPathComponent(".councis/browser/state/scroll-smoke.json")
        try FileManager.default.createDirectory(at: stateURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        let statePayload: [String: String] = [
            "profile": "scroll-smoke",
            "url": pageURL,
            "title": "Councis Scroll Wait",
            "updatedAt": ISO8601DateFormatter().string(from: Date()),
        ]
        let stateData = try JSONSerialization.data(withJSONObject: statePayload, options: [.prettyPrinted, .sortedKeys])
        try stateData.write(to: stateURL, options: .atomic)
    ...
    func testRealBrowserDynamicFeedAndOnlineTaskWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set COUNCIS_REAL_BROWSER_SMOKE=1 to run the real browser dynamic feed/task smoke")
        }
        ...
        let feedHTML = """
        <!doctype html>
        <title>Councis Social Feed Smoke</title>
        <style>
        ...
        let taskHTML = """
        <!doctype html>
        <title>Councis Online Task</title>
        <body>
        <h1>Online Task</h1>
        ...
        let stateURL = ws.appendingPathComponent(".councis/browser/state/feed-task-smoke.json")
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: stateURL)) as? [String: Any])
    ...
    func testBrowserExecutionDeclaresAllManagedPaths() throws {
        ...
        XCTAssertEqual(Set(paths), Set([
            ".councis/browser/profiles/work",
            ".councis/browser/downloads/work",
            ".councis/browser/state/work.json",
            ".councis/browser/history.jsonl",
        ]))
    }
    ...
    func testBrowserExecutionRejectsReadOnlyLeaseBeforeCreatingState() async throws {
        ...
        XCTAssertFalse(
            FileManager.default.fileExists(
                atPath: ws.appendingPathComponent(".councis").path))
    }
    ...
    func testBrowserExecutionRejectsNarrowOrDeniedManagedPathsBeforeCreatingState() async throws {
        ...
        let narrowLease = WorkspaceLease(
            ...
            access: .readWrite,
            allowedPathRules: [
                PathRule(pattern: ".councis/browser/profiles/**"),
            ],
            deniedPatterns: [])
    ...
    func testBrowserDiagnosticsIgnoresWorkspacePlaywrightModule() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("councis-node-outside-\(UUID().uuidString).txt")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
            unsetenv("COUNCIS_NODE_HOST_MARKER")
        }
        try Data("outside-node-marker".utf8).write(to: outside)
        setenv("COUNCIS_NODE_HOST_MARKER", "host-node-marker", 1)
        ...
        fs.writeFileSync('workspace-playwright-loaded', `${process.env.COUNCIS_NODE_HOST_MARKER || ''}|${outside}`);
    ...
    func testBrowserProfilesListsMetadataWithoutReadingProfileDatabaseContents() async throws {
        ...
        let cookieFile = ws.appendingPathComponent(".councis/browser/profiles/work/Default/Cookies")
        ...
        let activeMarkerFile = ws.appendingPathComponent(".councis/browser/profiles/work/DevToolsActivePort")
        let lockMarkerFile = ws.appendingPathComponent(".councis/browser/profiles/work/SingletonLock")
        ...
        let downloadURL = ws.appendingPathComponent(".councis/browser/downloads/work/report.pdf")
        ...
        XCTAssertTrue(obs.text.contains("profile dir: .councis/browser/profiles/work (present;"))
        XCTAssertTrue(obs.text.contains("state file: .councis/browser/state/work.json (present)"))
    ...
    func testBrowserProfileDeleteRequiresConfirmationAndOnlyDeletesTargetProfile() async throws {
        ...
        let workCookieFile = ws.appendingPathComponent(".councis/browser/profiles/work/Default/Cookies")
        let personalCookieFile = ws.appendingPathComponent(".councis/browser/profiles/personal/Default/Cookies")
        ...
        let workStateURL = ws.appendingPathComponent(".councis/browser/state/work.json")
        let personalStateURL = ws.appendingPathComponent(".councis/browser/state/personal.json")
        ...
        let workDownload = ws.appendingPathComponent(".councis/browser/downloads/work/report.pdf")
        let personalDownload = ws.appendingPathComponent(".councis/browser/downloads/personal/keep.pdf")
        ...
        XCTAssertTrue(obs.text.contains("removed profile data: yes (.councis/browser/profiles/work)"))
        XCTAssertTrue(obs.text.contains("removed state file: yes (.councis/browser/state/work.json)"))
        XCTAssertTrue(obs.text.contains("removed downloads: yes (.councis/browser/downloads/work)"))
    ...
    func testBrowserSearchReportsResultTextLinksAndHistory() async throws {
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let stdout = #"{"action":"search","profile":"research","backend":"cdp","backendDetail":"edge","url":"https://duckduckgo.com/?q=Councis%20agent","title":"Councis agent at DuckDuckGo","text":"Search results for Councis agent","links":[{"text":"Councis project","href":"https://example.com/councis"}],"elements":[{"role":"textbox","name":"Search","selector":"input[name=q]","tag":"input","type":"search"}]}"#
        let ctx = ToolContext(
            workspaceRoot: ws,
            shell: FakeShell(result: ShellResult(stdout: stdout, stderr: "", exitCode: 0)),
            git: FakeGit(statusText: "", diffText: ""))
        ...
        let obs = try await BrowserSearchTool().execute(
            ToolArgs(raw: #"{"query":"Councis agent","engine":"duckduckgo","profile":"research","channel":"msedge","waitMillis":100}"#),
            in: ctx)

        XCTAssertTrue(obs.text.contains("browser action: search"), obs.text)
        XCTAssertTrue(obs.text.contains("title: Councis agent at DuckDuckGo"), obs.text)
        XCTAssertTrue(obs.text.contains("Councis project - https://example.com/councis"), obs.text)
        XCTAssertTrue(obs.text.contains(#"textbox "Search" selector=input[name=q] type=search"#), obs.text)
        XCTAssertTrue(obs.text.contains("Search results for Councis agent"), obs.text)

        let stateURL = ws.appendingPathComponent(".councis/browser/state/research.json")
        let stateData = try Data(contentsOf: stateURL)
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: stateData) as? [String: Any])
        XCTAssertEqual(state["url"] as? String, "https://duckduckgo.com/?q=Councis%20agent")
    ...
    func testBrowserDownloadReportsChangedFileAndDownloadsListMetadataOnly() async throws {
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        let stdout = #"{"action":"download","profile":"work","url":"https://example.com/files","title":"Files","text":"Downloaded","links":[],"downloads":[{"filename":"report.pdf","path":".councis/browser/downloads/work/report.pdf","url":"https://example.com/report.pdf","bytes":7}]}"#
        let ctx = ToolContext(
            workspaceRoot: ws,
            shell: FakeShell(result: ShellResult(stdout: stdout, stderr: "", exitCode: 0)),
            git: FakeGit(statusText: "", diffText: ""))
        ...
        XCTAssertEqual(download.changedFiles, [".councis/browser/downloads/work/report.pdf"])
        XCTAssertTrue(download.text.contains("downloads:"))
        XCTAssertTrue(download.text.contains("report.pdf -> .councis/browser/downloads/work/report.pdf"))

        let downloadedFile = ws.appendingPathComponent(".councis/browser/downloads/work/report.pdf")
        ...
        XCTAssertTrue(listed.text.contains(".councis/browser/downloads/work/report.pdf"))
    ...
    func testStandardRegistry() {
        ...
        XCTAssertEqual(reg.registryVersion, "councis.standard.v4")
