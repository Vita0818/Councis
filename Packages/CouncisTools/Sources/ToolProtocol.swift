...
import Crypto
#else
#error("CouncisTools requires CryptoKit or swift-crypto")
#endif
import CouncisCore
...
public struct ToolArgs: Sendable {
    ...
    public func decode<T: Decodable>(_ type: T.Type) throws -> T {
        guard let data = raw.data(using: .utf8) else {
            throw CouncisError.decoding("tool args are not valid UTF-8")
        }
        do {
            return try JSONDecoder().decode(T.self, from: data)
        } catch {
            throw CouncisError.decoding("tool args: \(error.localizedDescription)")
        }
    }
}
...
/// prevents clients outside CouncisTools from manufacturing a process launch.
...
/// implementations resolve the executable from a fixed Councis runtime and
...
/// callers cannot turn this into a general shell: only Councis browser tools
...
struct BrowserBackendInvocation: Sendable {
    ...
    var injectedShellCommand: String {
        ...
        COUNCIS_BROWSER_ARGS='\(encodedArguments)' node <<'COUNCIS_BROWSER_INJECTED_NODE'
        \(javaScript)
        COUNCIS_BROWSER_INJECTED_NODE
        """
    }
}
...
public struct ToolContext: Sendable {
    ...
    /// Dedicated backend for Councis-generated Playwright/CDP browser
...
public struct ToolRegistry: Sendable {
    ...
    public init(_ tools: [any Tool], registryVersion: String = "councis.standard.v1") {
        self.init(
    ...
    public static func standard(
        ...
        return ToolRegistry(
            registrations: registrations,
            registryVersion: "councis.standard.v4")
    }
}
