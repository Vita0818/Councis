...
/// renderer boundary, so Councis instead preserves Chromium's native sandbox
...
func validatedWorkspace(_ cwd: URL) throws -> URL {
    ...
    guard FileManager.default.fileExists(atPath: workspace.path, isDirectory: &isDirectory),
        ...
        throw CouncisError.io("shell workspace does not exist or is not a directory")
    }
    return workspace
}
...
func effectiveWorkspaceLease(_ candidate: WorkspaceLease?,
    ...
    guard leaseRoot.path == workspace.path else {
        throw CouncisError.permissionDenied("workspace lease root does not match the managed process workspace")
    }
    guard let rootIdentity = lease.rootIdentity else {
        throw CouncisError.permissionDenied("workspace lease is missing its reviewed root identity")
    }
    guard rootIdentity.canonicalPath == leaseRoot.path,
        ...
        throw CouncisError.permissionDenied("workspace lease root identity changed after review")
    }
    guard allowEmptyPathRules || lease.allowedPathRules.isEmpty == false else {
        throw CouncisError.permissionDenied("workspace lease process allow-list is empty")
    }
    for pattern in lease.allowedPathRules.map(\.pattern) + lease.deniedPatterns {
        ...
        guard normalized.isEmpty == false,
            ...
            throw CouncisError.permissionDenied("workspace lease contains an unsafe process path rule")
        }
    }
    return lease
}
...
func documentProcessLease(
    ...
    for rawPath in internalWritablePaths {
        let resolved = try PathConfinement.resolve(rawPath, within: workspace)
        guard resolved.lastPathComponent.hasPrefix(".councis-document-stage-") else {
            throw CouncisError.permissionDenied(
...
private func validateDocumentBackendArguments(
    ...
    guard invocation.arguments.allSatisfy({ !$0.contains("\0") }),
        ...
        throw CouncisError.config("document backend invocation contains an unsafe argv or environment value")
    }
    let allowedEnvironment = Set([
        "COUNCIS_DOCUMENT_REQUEST",
        "COUNCIS_DOCUMENT_OPERATION",
        "PYTHONHASHSEED",
    ])
...
private func trustedDocumentExecutable(
    ...
    let runtime = councisDocumentRuntimeRoot()
    let path: String?
    switch executable {
    case .pythonRuntime:
    ...
    case .rbookHelper:
        path = runtime?.appendingPathComponent("bin/councis-rbook-helper").path
    case .epubCheck:
        path = runtime?.appendingPathComponent("bin/councis-epubcheck").path
    case .libreOffice:
        #if os(macOS)
        path = councisLibreOfficeRuntimeAppURL()?
...
private func runWorkspaceProcess(executable: URL,
    ...
    let runtime = FileManager.default.temporaryDirectory
        .appendingPathComponent("councis-process-\(UUID().uuidString)", isDirectory: true)
    ...
    guard FileManager.default.createFile(
        ...
        contents: Data()) else {
        throw CouncisError.io("could not create managed process startup marker")
    }
    ...
    guard markerFlags >= 0,
        ...
        throw CouncisError.io("could not protect managed process startup marker")
    }
    ...
    guard FileManager.default.isExecutableFile(atPath: "/usr/bin/sandbox-exec") else {
        throw CouncisError.config("process execution is disabled because the macOS workspace sandbox is unavailable")
    }
    ...
    guard let bubblewrap = bubblewrapExecutable() else {
        throw CouncisError.config("process execution is disabled because Bubblewrap is unavailable; install bwrap")
    }
...
private func runMacOSBrowserBackendProcess(
    ...
    let runtime = FileManager.default.temporaryDirectory
        .appendingPathComponent("councis-browser-process-\(UUID().uuidString)", isDirectory: true)
    let home = runtime.appendingPathComponent("home", isDirectory: true)
    let temporary = runtime.appendingPathComponent("tmp", isDirectory: true)
    try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
    try FileManager.default.createDirectory(at: temporary, withIntermediateDirectories: true)
    ...
    guard FileManager.default.createFile(
        ...
        throw CouncisError.io("could not create the managed browser program")
    }

    var environment = sanitizedProcessEnvironment(home: home, temporary: temporary)
    environment["COUNCIS_BROWSER_ARGS"] = invocation.encodedArguments
    environment["COUNCIS_BROWSER_PROCESS_BOUNDARY"] = "1"
...
private func makeManagedOutputPipe(maximumBytes: Int) throws -> ManagedOutputPipe {
    ...
    guard systemPipe(&descriptors) == 0 else {
        throw CouncisError.io("could not create bounded process output pipe")
    }
...
private func runManagedProcess(spec: ManagedProcessSpec,
    ...
    case .timedOut:
        throw CouncisError.io("shell command timed out after \(timeoutSeconds.formattedForError)s")
    case .resourceLimit:
        throw CouncisError.io("document backend exceeded its generated output budget")
    }
}
...
func limitedExecutionArguments(executable: URL,
    ...
    return [
        ...
        "councis-managed", startupMarker.path, executable.path,
    ] + arguments
}
...
func managedTerminalExecutionArguments(executable: URL,
    ...
    [
        ...
        "councis-managed-terminal", startupMarker.path, executable.path,
    ] + arguments
}
...
func structuredRuntimeReadRoots() -> [URL] {
    ...
    if let documentRuntime = councisDocumentRuntimeRoot(),
...
/// Councis never installs into or mutates this directory while running a tool;
/// it is only added as a narrow read root when the user created it explicitly.
func councisDocumentRuntimeRoot() -> URL? {
    #if os(macOS)
    ...
    let current = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(".local/share/councis/document-runtime", isDirectory: true)
    if FileManager.default.fileExists(atPath: current.path) {
        return current
    }
    let legacy = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(
            "\(LegacyIntatisCompatibility.sharedDataDirectoryRelativePath)/document-runtime",
            isDirectory: true)
    return FileManager.default.fileExists(atPath: legacy.path)
        ? legacy : current
    #else
...
#if os(macOS)
func councisLibreOfficeRuntimeAppURL() -> URL? {
    councisDocumentRuntimeRoot()?
...
func macOSSandboxProfile(workspace: URL,
    ...
    case (.libreOfficeHeadless, .some(let socketRoot)):
        ...
        let suffix = String(name.dropFirst("councis-lo-".count))
        let hasExpectedParent = canonical.deletingLastPathComponent().path == "/private/tmp"
        let hasExpectedPrefix = name.hasPrefix("councis-lo-")
    ...
    case .libreOfficeHeadless:
        ...
        guard let libreOfficeApp = councisLibreOfficeRuntimeAppURL() else {
            throw CouncisError.config(
    ...
    case (.denied, .libreOfficeHeadless):
        guard let validatedLibreOfficeSocketRoot else {
            throw CouncisError.config("LibreOffice socket root is unavailable")
        }
...
private func createLibreOfficeSocketRoot() throws -> URL {
    ...
    let suffix = UUID().uuidString
        ...
        .lowercased()
        .prefix(12)
    let root = parent.appendingPathComponent("councis-lo-\(suffix)", isDirectory: true)
    do {
        ...
        throw CouncisError.io("could not create LibreOffice socket root")
    }
    var status = stat()
...
func bubblewrapArguments(workspace: URL,
    ...
    if allowsWholeWorkspace == false {
        addParents(of: workspace.path + "/.councis-placeholder")
...
func spawnManagedProcess(spec: ManagedProcessSpec,
    ...
    guard posix_spawn_file_actions_init(&actions) == 0,
        ...
        throw CouncisError.io("could not initialize managed process attributes")
    }
    defer {
    ...
    if let stdinDescriptor {
        ...
        guard descriptor >= 0 else { throw CouncisError.io("could not open null stdin") }
    ...
    guard posix_spawn_file_actions_adddup2(
        ...
        throw CouncisError.io("could not configure managed process file descriptors")
    }
    #if os(macOS)
    guard posix_spawn_file_actions_addchdir(&actions, cwd.path) == 0 else {
        throw CouncisError.io("could not confine managed process working directory")
    }
    #endif
    ...
    guard posix_spawnattr_setflags(&attributes, flags) == 0,
        ...
        throw CouncisError.io("could not configure managed process isolation")
    }
    ...
    guard result == 0 else {
        throw CouncisError.io("managed process launch failed: \(String(cString: strerror(result)))")
    }
    guard getpgid(pid) == pid else {
        _ = kill(pid, SIGKILL)
        _ = waitpid(pid, nil, 0)
        throw CouncisError.io("managed process did not start in an isolated process group")
    }
    return pid
}
...
func spawnManagedPTYProcess(spec: ManagedProcessSpec,
    ...
    var spawned = CouncisPTYSpawnResult(
        pid: -1,
        master_fd: -1,
        error_stage: Int32(COUNCIS_PTY_STAGE_NONE),
        error_number: 0)
    let result: Int32 = withMutableCStrings(argv) { argvPointer in
        withMutableCStrings(envp) { envPointer in
            councis_spawn_managed_pty(
    ...
    guard result == 0, spawned.pid > 0, spawned.master_fd >= 0 else {
        ...
        case Int32(COUNCIS_PTY_STAGE_CHDIR):
            stage = "working-directory setup"
        case Int32(COUNCIS_PTY_STAGE_EXEC):
...
private func gitWorkspaceLayout(_ workspace: URL) throws -> GitWorkspaceLayout {
    ...
    guard FileManager.default.fileExists(atPath: dotGit.path, isDirectory: &isDirectory) else {
        throw CouncisError.permissionDenied("git repository metadata must be inside the agent workspace")
    }
    let dotGitValues = try dotGit.resourceValues(forKeys: [.isSymbolicLinkKey])
    guard dotGitValues.isSymbolicLink != true else {
        throw CouncisError.permissionDenied("git metadata symlink escapes are not allowed")
    }
    if isDirectory.boolValue {
        ...
        guard resolved == root.path || resolved.hasPrefix(prefix) else {
            throw CouncisError.permissionDenied("git metadata directory escapes the agent workspace")
        }
    ...
    guard data.count <= 4_096,
        ...
        throw CouncisError.permissionDenied("git worktree metadata pointer is invalid")
    }
    ...
    guard let markerRange = target.path.range(of: marker, options: .backwards) else {
        throw CouncisError.permissionDenied("git worktree metadata must stay under the owning workspace repository")
    }
    ...
    guard expectedWorkspace.path == root.path else {
        throw CouncisError.permissionDenied("git linked worktree must use .councis/git-worktrees under its owning workspace")
    }
    ...
    guard target.path == expectedTargetPrefix || target.path.hasPrefix(expectedTargetPrefix + "/") else {
        throw CouncisError.permissionDenied("git worktree metadata pointer does not match its owning workspace")
    }
    return GitWorkspaceLayout(
        workspace: root,
...
public struct ProcessGitService: GitService {
    ...
    public func pullFastForward(remote: String, branch: String, workspace: URL) async throws -> String {
        ...
        guard current == safeBranch else {
            throw CouncisError.permissionDenied("git_pull_ff branch must match current branch (\(current))")
        }
    ...
    public func push(remote: String, branch: String, setUpstream: Bool, workspace: URL) async throws -> String {
        ...
        let current = try await currentBranch(workspace: workspace)
        guard current == safeBranch else {
            throw CouncisError.permissionDenied("git_push branch must match current branch (\(current))")
        }
        var args = ["push"]
        if setUpstream {
            args.append("-u")
    ...
    private func validateRepository(workspace: URL) async throws {
        ...
        guard repoRoot == workspacePath else {
            throw CouncisError.permissionDenied("git repository root must match the agent workspace root")
        }
    ...
    private func validateRepositoryConfiguration(workspace: URL) async throws {
        ...
        guard result.exitCode == 0 else {
            throw CouncisError.permissionDenied("git repository config could not be inspected safely")
        }
    ...
    private func validateRepositoryConfigFile(_ layout: GitWorkspaceLayout) throws {
        for configURL in layout.configURLs {
            ...
            guard (entry.st_mode & mode_t(S_IFMT)) != mode_t(S_IFLNK) else {
                throw CouncisError.permissionDenied("git repository config symlinks are not allowed")
            }
            ...
            guard isConfined else {
                throw CouncisError.permissionDenied("git repository config escapes approved metadata roots")
            }
            let byteCount = Int(entry.st_size)
            guard byteCount >= 0, byteCount <= 1_048_576 else {
                throw CouncisError.permissionDenied("git repository config is too large to inspect safely")
            }
            let data = try Data(contentsOf: configURL, options: [.mappedIfSafe])
            guard let text = String(data: data, encoding: .utf8) else {
                throw CouncisError.permissionDenied("git repository config is not valid UTF-8")
            }
            var section = ""
            var unsafe: [String] = []
    ...
    private func runRawGit(_ args: [String],
        ...
        guard let executable = gitExecutable() else {
            throw CouncisError.config("git is unavailable in a trusted fixed runtime path")
        }
        ...
        if checked, result.exitCode != 0 {
            ...
            throw CouncisError.io("git \(command) failed (\(renderedArgs)): \(message)")
        }
        return result
        #else
        throw CouncisError.io("git execution is unavailable on this platform")
        #endif
    }
    ...
    private func ensureRemoteExists(_ remote: String, workspace: URL) async throws {
        guard try await remoteNames(workspace: workspace).contains(remote) else {
            throw CouncisError.decoding("git remote is not configured: \(remote)")
        }
    }
    ...
    private func currentBranch(workspace: URL) async throws -> String {
        ...
        guard !current.isEmpty else {
            throw CouncisError.permissionDenied("git operation requires a checked-out local branch")
        }
        return current
    }
    ...
    private func ensureCleanWorktree(workspace: URL, operation: String) async throws {
        ...
        guard GitStatus.parse(statusText).isEmpty else {
            throw CouncisError.permissionDenied("\(operation) requires a clean working tree")
        }
    }
...
private enum GitToolInput {
    static func normalizedPaths(_ paths: [String], workspace: URL) throws -> [String] {
        guard !paths.isEmpty else {
            throw CouncisError.decoding("git paths must not be empty")
        }
        return try paths.map { rawPath in
            let path = rawPath.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !path.isEmpty else {
                throw CouncisError.decoding("git paths must not contain empty entries")
            }
    ...
    static func branchName(_ raw: String) throws -> String {
        ...
        guard !name.isEmpty else {
            throw CouncisError.decoding("branch name must not be empty")
        }
        guard name.count <= 255 else {
            throw CouncisError.decoding("branch name is too long")
        }
        let forbidden = ["..", "@{", "\\", "~", "^", ":", "?", "*", "[", "\n", "\r", "\t"]
        guard !forbidden.contains(where: { name.contains($0) }),
              ...
              !name.contains("//"),
              name != "@" else {
            throw CouncisError.decoding("branch name contains characters Git refs cannot safely use")
        }
        return name
    }
    ...
    static func remoteName(_ raw: String) throws -> String {
        let name = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !name.isEmpty else {
            throw CouncisError.decoding("remote name must not be empty")
        }
        guard name.count <= 80 else {
            throw CouncisError.decoding("remote name is too long")
        }
        ...
        guard name.unicodeScalars.allSatisfy({ allowed.contains($0) }),
              ...
              !name.hasPrefix("."),
              !name.contains("..") else {
            throw CouncisError.decoding("remote must be a configured remote name, not a URL or refspec")
        }
        return name
    }
    ...
    static func optionalRef(_ raw: String?) throws -> String? {
        ...
        guard value.count <= 255,
              ...
              !value.contains("\r"),
              !value.contains("\t") else {
            throw CouncisError.decoding("git start point is not a safe ref")
        }
        return value
    }
    ...
    static func requiredRef(_ raw: String) throws -> String {
        guard let value = try optionalRef(raw) else {
            throw CouncisError.decoding("git ref must not be empty")
        }
        return value
    }
    ...
    static func worktreeName(_ raw: String) throws -> String {
        let name = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !name.isEmpty else {
            throw CouncisError.decoding("worktree name must not be empty")
        }
        guard name.count <= 80 else {
            throw CouncisError.decoding("worktree name is too long")
        }
        let allowed = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "._-"))
        guard name.unicodeScalars.allSatisfy({ allowed.contains($0) }),
              !name.hasPrefix("."),
              !name.hasPrefix("-"),
              !name.contains("..") else {
            throw CouncisError.decoding("worktree name must be a simple safe directory name")
        }
        return name
    }
    ...
    private static func patchPaths(_ diff: String) throws -> [String] {
        guard !diff.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw CouncisError.decoding("git patch must not be empty")
        }
        var result: [String] = []
        var seen = Set<String>()
        ...
        guard !result.isEmpty else {
            throw CouncisError.decoding("git patch does not expose any changed paths")
        }
        return result
    }
...
public struct GitCommitTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard !message.isEmpty else {
            throw CouncisError.decoding("commit message must not be empty")
        }
        ...
        if let sensitive = stagedEntries.first(where: { PathConfinement.isSensitivePath($0.path) }) {
            throw CouncisError.permissionDenied("refusing to commit staged sensitive path: \(sensitive.path)")
        }
...
public struct GitRevertPatchTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard a.confirmRevert else {
            throw CouncisError.permissionDenied("git_revert_patch requires confirmRevert=true")
        }
        _ = try GitToolInput.normalizedPatchPaths(a.diff, workspace: context.workspaceRoot)
        let result = try await context.git.applyPatch(diff: a.diff,
                                                       reverse: true,
...
public struct GitWorktreeCreateTool: Tool {
    ...
    public static let descriptor = ToolDescriptor(
        ...
        description: "Create a workspace-contained managed Git worktree under .councis/git-worktrees. Defaults to detached HEAD.",
        sideEffect: .write,
        parameters: Schema.object([
    ...
    public func touchedPaths(_ args: ToolArgs) -> [String] {
        ...
        return [".git", ".councis/git-worktrees/\(name)"]
    }
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        let output = try await context.git.createWorktree(name: name,
                                                          ...
                                                          branch: branch,
                                                          workspace: context.workspaceRoot)
        return ToolObservation(text: output, changedFiles: [".councis/git-worktrees/\(name)"])
    }
}
...
public struct GitWorktreeRemoveTool: Tool {
    ...
    public static let descriptor = ToolDescriptor(
        ...
        description: "Destructively remove a managed worktree under .councis/git-worktrees after exact name confirmation.",
        sideEffect: .destructive,
        parameters: Schema.object([
            "name": Schema.boundedString(minLength: 1, maxLength: 80),
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard name == confirmName else {
            throw CouncisError.permissionDenied("git_worktree_remove confirmName must match name")
        }
...
public struct GitPullFastForwardTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard remote == confirmRemote, branch == confirmBranch else {
            throw CouncisError.permissionDenied("git_pull_ff confirmation must match remote and branch")
        }
...
public struct GitPushTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        let confirmBranch = try GitToolInput.branchName(a.confirmBranch)
        guard remote == confirmRemote, branch == confirmBranch else {
            throw CouncisError.permissionDenied("git_push confirmation must match remote and branch")
        }
...
public struct GitSwitchBranchTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard branch == confirmBranch else {
            throw CouncisError.permissionDenied("git_switch confirmBranch must match branch")
        }
