...
private final class SequencedHTTP: HTTPByteStreaming, @unchecked Sendable {
    private let queue = DispatchQueue(label: "councis.tests.sequenced-http")
    private var index = 0
...
private final class CapturingHTTP: HTTPByteStreaming, @unchecked Sendable {
    private let queue = DispatchQueue(label: "councis.tests.capturing-http")
...
private let nonHTTPChatEndpoint = ProviderEndpoint(id: "bad-chat",
                                                   baseURL: URL(string: "https://example.test/v1")!,
                                                   chatEndpoint: URL(fileURLWithPath: "/tmp/councis-chat"),
                                                   apiKeyRef: KeychainRef(service: "s", account: "a"),
                                                   wire: .openai)

final class CouncisProvidersTests: XCTestCase {
