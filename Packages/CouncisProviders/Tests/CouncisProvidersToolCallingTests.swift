...
private final class SequencedToolHTTP: HTTPByteStreaming, @unchecked Sendable {
    private let queue = DispatchQueue(label: "councis.tests.sequenced-tool-http")
    private var index = 0
...
private let nonHTTPToolEndpoint = ProviderEndpoint(
    id: "bad-agent",
    baseURL: URL(string: "https://example.test/v1")!,
    chatEndpoint: URL(fileURLWithPath: "/tmp/councis-agent"),
    apiKeyRef: KeychainRef(service: "s", account: "a"),
    wire: .openai
)

final class CouncisProvidersToolCallingTests: XCTestCase {
