...
enum ProviderErrorFormatting {
    ...
    static func httpStatus(_ status: Int,
        ...
        return CouncisError.provider(parts.joined(separator: " "))
    }
    ...
    static func transport(_ error: Error) -> Error {
        ...
        if let councis = error as? CouncisError {
            return sanitized(councis)
        }
        if let urlError = error as? URLError {
            return CouncisError.provider(transportMessage(urlError))
        }
        return CouncisError.provider(clean(error.localizedDescription))
    }
    ...
    static func invalidStreamPayload(_ payload: String, underlying: Error) -> CouncisError {
    ...
    static func incompleteStream(operation: String) -> CouncisError {
        .decoding(
            "\(operation) ended before a completion marker. " +
            "Check endpoint compatibility, proxy buffering, or provider stability.")
    }

    static func invalidToolCallStream(_ message: String) -> CouncisError {
        .decoding(
    ...
    static func invalidResponsePayload(_ data: Data,
                                       operation: String,
                                       expected: String,
                                       underlying: Error? = nil) -> CouncisError {
        var parts = [
    ...
    private static func sanitized(_ error: CouncisError) -> CouncisError {
        switch error {
