...
enum ProviderRuntime {
    ...
    static func exhausted(_ error: Error, attempts: Int, operation: String) -> Error {
        ...
        if let councis = normalized as? CouncisError {
            switch councis {
            case .provider(let message):
                return CouncisError.provider(message + suffix)
            case .config(let message):
                return CouncisError.config(message + suffix)
            case .decoding(let message):
                return CouncisError.decoding(message + suffix)
            case .io(let message):
                return CouncisError.io(message + suffix)
            case .notFound(let message):
                return CouncisError.notFound(message + suffix)
            case .permissionDenied(let message):
                return CouncisError.permissionDenied(message + suffix)
            case .cancelled:
                return councis
            }
        }
        return CouncisError.provider("\(operation) failed. \(normalized.localizedDescription)\(suffix)")
    }
        ...
        return try await withThrowingTaskGroup(of: T.self) { group in
            ...
            do {
                guard let value = try await group.next() else {
                    throw CouncisError.provider("\(operation) did not produce a result.")
                }
                group.cancelAll()
                return value
            } catch {
