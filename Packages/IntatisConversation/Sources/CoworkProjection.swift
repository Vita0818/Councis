...
public struct CoworkProjection: Equatable, Sendable {
    ...
    public private(set) var agentModelBindings: [AgentID: AgentModelBinding] = [:]
    ...
    public mutating func apply(_ envelope: Envelope) {
        ...
        case .agentModelBound(let payload):
            agentModelBindings[payload.agent] = payload.binding
        case .agentDetached(let payload):
            agentRoster.removeValue(forKey: payload.agent)
            agentModelBindings.removeValue(forKey: payload.agent)
        ...
        case .userMessage, .messageDelta, .messageCompleted, .error,
             ...
             .permissionReviewSettled, .taskReviewRequested, .taskReviewSettled,
             .taskReviewExhausted, .artifactAdded,
    ...
    private mutating func applyLifecycleBinding(agent: AgentID,
                                                providerID: String?,
                                                model: ModelID) {
        guard let providerID,
              let binding = try? AgentModelBinding(
                  validatingProviderID: providerID,
                  modelID: model) else { return }
        agentModelBindings[agent] = binding
    }
