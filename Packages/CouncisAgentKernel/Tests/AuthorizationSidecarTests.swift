...
final class AuthorizationSidecarTests: XCTestCase {
    func testLegacyReservedFieldIsStrippedAndRejectedBeforeExecution() {
        let raw = #"{"path":"report.md","__intatis_authorization_context":"legacy"}"#
        let extraction = AuthorizationSidecarCodec.extract(from: raw)
        XCTAssertEqual(
            extraction.sidecarStatus,
            .malformed(.unexpectedField(
                AuthorizationSidecarCodec.legacyReservedFieldName)))
        XCTAssertEqual(
            extraction.canonicalBusinessArguments,
            #"{"path":"report.md"}"#)
        XCTAssertTrue(
            AuthorizationSidecarCodec.containsReservedField(in: raw))

        let prepared = AuthorizationSidecarCodec.prepare(
            ToolCall(
                id: "call_legacy_brand",
                name: "write_file",
                arguments: raw),
            sessionID: SessionID(rawValue: "sess_brand"),
            turnID: TurnID(rawValue: "turn_brand"),
            providerGenerationID: "generation-brand",
            registrySnapshotID: "registry-brand")
        XCTAssertNil(prepared.executableCall)
        XCTAssertNil(prepared.binding)
    }
    ...
    func testShippedStrictSchemasRemainProviderCompatibleAfterDecoration()
        ...
        let workspace = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-sidecar-strict-\(UUID().uuidString)",
                isDirectory: true)
