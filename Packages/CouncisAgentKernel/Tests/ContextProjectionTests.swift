...
final class ContextProjectionTests: XCTestCase {
    ...
    func testCoordinatorPromptRequiresExactBundledOrchestrationSkill() {
        ...
        XCTAssertTrue(coordinator.contains(
            "`\(CouncisBundledSkills.coworkAgentOrchestrationName)`"))
        ...
        XCTAssertTrue(coordinator.contains("Inspect the bounded COUNCIS_SKILL_CATALOG"))
    ...
    func testFirstAgentRequestDeclaresCouncisRuntimeAndToolProtocol() {
        for (runtime, modeName, productName) in [
            (RuntimeEnvironmentManifest.code, "Code", "Councis"),
    ...
    func testRuntimeProjectsSkillCatalogAsDeveloperAndExplicitBodyAsUser() async throws {
        let workspace = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-skill-context-\(UUID().uuidString)",
                isDirectory: true)
        ...
        XCTAssertTrue(system.contains("COUNCIS_SKILL_CATALOG"))
        ...
        async throws
    {
        let workspace = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-skill-rejection-\(UUID().uuidString)",
                isDirectory: true)
        defer { try? FileManager.default.removeItem(at: workspace) }
    ...
    func testWorkerPromptDoesNotReplayOriginalSpawnInstructionAsFreshUserMessage() async throws {
        let ws = FileManager.default.temporaryDirectory
            .appendingPathComponent("councis-context-\(UUID().uuidString)", isDirectory: true)
