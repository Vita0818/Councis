...
final class ModelDrivenKnowledgeAgentLoopTests: XCTestCase {
    func testAgentLoopKeepsTwoExternalKnowledgeStoresSnapshotIsolated() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "councis-agentloop-knowledge-\(UUID().uuidString)",
            isDirectory: true)
        let externalStores = ["a", "b"].map { label in
            FileManager.default.temporaryDirectory.appendingPathComponent(
                "councis-agentloop-external-knowledge-\(label)-\(UUID().uuidString)",
                isDirectory: true)
        }
        defer {
            makeWritable(root)
        ...
        try write(
            ...
            Councis answers from an immutable snapshot and revalidates cited evidence before publishing the final response.
            """,
        ...
        XCTAssertTrue(calls.allSatisfy {
            $0.args == #"{"_councis":"arguments_redacted"}"#
                && $0.argsRedacted == true
        })
    ...
    func testDeniedExternalBuildStopsBeforeAuthorityFilesystemAndModels()
        async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "councis-agentloop-knowledge-deny-\(UUID().uuidString)",
            isDirectory: true)
        let externalStore = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-agentloop-external-denied-\(UUID().uuidString)",
                isDirectory: true)
        defer {
            makeWritable(root)
            try? FileManager.default.removeItem(at: root)
    ...
    func stream(_ request: AgentRequest)
        ...
        case 2:
            chunks = [
                .toolCalls([ToolCall(
                    ...
                    arguments: Self.arguments([
                        "store_path": storePaths[0],
                        "query": "How does Councis validate evidence?",
                        "limit": 4,
                    ]))]),
                .done(finishReason: "tool_calls"),
            ]
        ...
        default:
            ...
            if evidenceIDs.count == 2 {
                chunks = [
                    .textDelta(
                        "Both exact stores say Councis revalidates evidence before the final response [[evidence:\(evidenceIDs[0])]] [[evidence:\(evidenceIDs[1])]]."),
...
private actor AgentLoopKnowledgeEmbeddingProvider: KnowledgeEmbeddingProvider {
    nonisolated let modelIdentity = KnowledgeEmbeddingModelIdentity(
        identity: "org.vita.councis.tests.agentloop-embedding",
...
private actor AgentLoopKnowledgeRerankerProvider: KnowledgeRerankerProvider {
    nonisolated let modelIdentity = KnowledgeRerankerModelIdentity(
        identity: "org.vita.councis.tests.agentloop-reranker",
