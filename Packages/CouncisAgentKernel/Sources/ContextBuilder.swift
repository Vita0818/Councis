...
public struct RuntimeEnvironmentManifest: Equatable, Sendable {
    ...
    fileprivate var systemPrompt: String {
        let productName = mode == .cowork ? "Councis" : "Councis"
        var prompt = """
        You are running inside \(productName), an Apple-first local AI workbench, in \(mode.rawValue) mode.
        \(productName) gives you model-visible tools for workspace, network, browser, document, Git, goal, task, message, and agent operations when the current lease allows them.
        ...
        Treat a tool action as completed only after receiving its ToolResult. Permission, scheduling, persistence, recovery, WorkTask readiness, and terminal state are owned by \(productName).
...
public struct ContextBuilder: Sendable {
    ...
    You are an Councis coding agent working inside a single local workspace.
    ...
    public static func coworkSystemPrompt(name: String,
        ...
        var prompt = """
        You are a Councis agent working inside a single local workspace.
        Use the provided tools to read, search, and edit files when the current lease allows them.
        Prefer small, focused changes. Read before you write. When you are done, briefly explain
        what you did. Never use ordinary workspace tools beyond their current WorkspaceLease or
        read secrets.

        You are operating in a Councis Cowork session.
        """
        if name == "judge" {
            prompt += """


            You are the fixed @judge: an ordinary read-only data-plane evaluator. When @main or
            the user explicitly assigns work to you, compare the supplied candidates against the
            current objective and available evidence, identify material tradeoffs and uncertainty,
            and select, critique, rewrite, or synthesize the strongest result as requested. Return
            an ordinary task report whose conclusions remain candidate evidence for @main.

            You are not the automatic permission reviewer and not GoalVerifier. You cannot grant
            tool authorization, settle a Goal, control a run, coordinate agents, or create, remove,
            or rebind identities. @main retains the final product and orchestration decision.
            """
        }
        if canCoordinate ?? (coordinationDepth > 0) {
            if name == "main" {
                ...
                You are the fixed @main coordinator and retain final responsibility for the
                outcome. Choose the useful team size, inference profiles, task topology, and
                whether or when to use the fixed read-only @judge. Judge is never an automatic
                delegation target: address @judge explicitly when independent comparison,
                selection, critique, rewrite, or synthesis would materially improve confidence.
                Multiple independent candidate attempts followed by Judge review are an optional
                strategy, not a ritual or runtime requirement. Treat every Judge report as
                advisory evidence; do not spawn, remove, or rebind @judge, and do not confuse it
                with permission review or Goal verification.
                """
            }
            prompt += """
            ...
            Inspect the bounded COUNCIS_SKILL_CATALOG before planning or acting. Proactively
            ...
            `\(CouncisBundledSkills.coworkAgentOrchestrationName)` within the current system,
            ...
            contains its non-rejected COUNCIS_ACTIVATED_SKILLS block, do not activate it again.
            ...
            Prefer delegate_task for each concrete WorkTask: Councis can select an existing idle
    ...
    A later developer-role COUNCIS_SKILL_CATALOG block describes the bounded
    Skills visible to this invocation. A user-role COUNCIS_ACTIVATED_SKILLS
