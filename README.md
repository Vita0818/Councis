...
headless Chat/Code/Cowork 和外部 MCP client。项目不再提供 iOS 或 Mac App Store 产品面。
所有运行时能力围绕结构化 EventLog、共享 AgentKernel、显式工具注册和权限链
...
- 设置：provider catalog、Councis JSON/JSONC 配置、MCP、renderer fallback、第三方声明，
  以及只在本机生成且不上传的脱敏诊断 ZIP。

macOS 唯一发行 target 是 `CouncisMac`，产物为 `Councis.app` / executable `Councis`，
Bundle ID 为 `com.Vita0818.Councis`，通过 Developer ID、Apple notarization 和直接下载分发。

### CLI

`councis` 支持 Chat/Code/Cowork REPL、managed execution、Skills、per-agent inference
...
- `PlatformProfile.current` 默认 `.restricted`；只有 `CouncisMac` 显式选择
  `.macDeveloperID` 才获得 workspace/shell/MCP 能力。
...
```text
Apps/                 CouncisMac 与 councis CLI 入口
...
```sh
COUNCIS_NOTARY_PROFILE=<profile-name> scripts/package-macos-release.sh
```
...
COUNCIS_NOTARY_PROFILE=<profile-name> \
...
产物并打印 `COUNCIS_RESUME_RELEASE_DIR` 恢复命令。恢复同一 submission，不要重复上传。
...
- macOS/CLI 高级配置读取 `COUNCIS_CONFIG`、Councis-owned JSON/JSONC 路径及兼容 fallback；
...
`permission_reviewer_model` 是 Councis 的顶层授权控制面字段，格式为
`<provider>/<model-id>`，并引用 provider `models` 中已配置的 base profile。Councis 在启动/恢复
...
`image_model` 是 Councis 的顶层扩展字段，格式为 `<provider>/<model-id>`。专用图片 provider
...
`transcription_model` 同样是 Councis 的顶层扩展字段，格式为 `<provider>/<model-id>`。专用语音
...
`search_knowledge`。上例中的 URL 和模型 ID 是需要替换的配置值；`councis:siliconflow-v1`
...
`councis:cohere-v2`。Knowledge-only provider 的 `models` 可保持空对象，不会进入普通推理模型菜单；
...
Councis 自有代码和第三方采用状态见 [`NOTICE.md`](NOTICE.md)、
