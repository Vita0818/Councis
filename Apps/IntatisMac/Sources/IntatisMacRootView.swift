...
struct IntatisMacRootView: View {
    ...
    @State private var selection: IntatisNavItem = AppIdentity.isCouncis ? .cowork : .chat
    ...
    private var items: [IntatisNavItem] {
        // Councis is the heterogeneous-team product profile. Its macOS shell
        // exposes only the strict Cowork surface; the single-agent ChatLoop and
        // Code AgentLoop remain available exclusively in the IntatisMac build.
        if AppIdentity.isCouncis {
            return PlatformProfile.current.supports(.cowork) ? [.cowork] : []
        }
        return IntatisNavItem.allCases.filter { item in
            switch item {
