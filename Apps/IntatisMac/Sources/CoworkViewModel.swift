...
private actor ProviderRegistryBox {
    ...
    func agentProvider(for binding: AgentModelBinding) async throws -> ToolCallingProvider {
        try await registry.agentProvider(for: binding)
    }

    func defaultAgentBinding() async throws -> AgentModelBinding {
        if let ref = await sessionAgentRef() {
            return try ref.validatedAgentModelBinding()
        }
        let models = await registry.models()
        return try (models.agent ?? models.chat).validatedAgentModelBinding()
    }
...
final class CoworkViewModel: ObservableObject, PermissionResponder {
    ...
    @Published private(set) var taskReviewerHealth: TaskReviewerLifecycleHealth = .disabled
    ...
    private var didRequestTaskReviewerAttach = false
    ...
    func start() {
        ...
        let team = AppIdentity.isCouncis ? projectSettings.teamConfiguration : nil
        if AppIdentity.isCouncis, team == nil {
            projectionError = "Councis team configuration is missing. Open Provider Settings and configure at least two unique provider/model bindings."
            return
        }
        if AppIdentity.isCouncis {
            taskReviewerHealth = .disabled
        }
        ...
        do {
            orchestrator = try Orchestrator.runtime(
                ...
                taskReviewPolicy: team == nil ? nil : .always,
                modelAssignmentPolicy: team?.strictModelAssignmentPolicy ?? .legacy,
                surfaceProfile: team == nil ? .legacy : .work,
                legacyProviderID: team?.mainBinding.providerID ?? projectSettings.defaultProviderID,
                imageGeneratorFor: { _ in await registryBox.imageToolService() }
            ) { agent in
                try await registryBox.agentProvider(for: agent.modelBinding)
            }
        } catch {
        ...
        subscription = Task { @MainActor [weak self] in
            ...
            guard let runningOrchestrator = self.orchestrator else { return }
            await self.synchronizeTaskReviewerHealth(using: runningOrchestrator)
            let replayed = await self.log.replay()
            let codePresentationPolicy: CodePresentationPolicy = AppIdentity.isCouncis
                ? .mandatoryTaskReview
                : .standard
            var codeProjection = CodeProjection.build(
                from: replayed,
                presentationPolicy: codePresentationPolicy)
            ...
            await runningOrchestrator.restore(from: coworkProjection)
            await self.synchronizeTaskReviewerHealth(using: runningOrchestrator)
            ...
            if let mismatch = await self.restoredTeamMismatch(projection: coworkProjection) {
                self.projectionError = mismatch
                self.composerError = mismatch
                self.agents = []
                self.setPermissionReviewerStatus(.failed(mismatch))
                self.taskReviewerHealth = .disabled
                // Drop the runtime (and its writer lease) without resuming any
                // restored work. The persisted log remains untouched for
                // inspection or recovery with the original provider catalog.
            ...
            await self.bootstrapTaskReviewerIfNeeded(existingProjection: coworkProjection)
            await self.synchronizeTaskReviewerHealth(using: runningOrchestrator)
            await runningOrchestrator.resumePendingTasks()
            ...
            for await envelope in stream {
                ...
                self.latestTurnStats = turnStats.latest
                self.applyCoworkProjection(coworkProjection)
                if Self.shouldRefreshTaskReviewerHealth(after: envelope.event) {
                    await self.synchronizeTaskReviewerHealth(using: runningOrchestrator)
                }
            }
        }
    ...
    func stop() async {
        ...
        if AppIdentity.isCouncis {
            taskReviewerHealth = .shuttingDown
        }
    ...
    private func applyCoworkProjection(_ projection: CoworkProjection) {
        ...
        let presentedActiveTasks = presentationTasks(projection.activeTasks)
        let presentedRunningTasks = presentationTasks(projection.runningTasks)
        let presentedCompletedTasks = presentationTasks(projection.completedTasks)
        let presentedFailedTasks = presentationTasks(projection.failedTasks)
        let presentedCancelledTasks = presentationTasks(projection.cancelledTasks)
        summary = CoworkStatusSummary(
            activeCount: presentedActiveTasks.count,
            runningCount: presentedRunningTasks.count,
            completedCount: presentedCompletedTasks.count,
            failedCount: presentedFailedTasks.count,
            ...
            runningTasks: presentedRunningTasks.map(taskLine),
            failedTasks: presentedFailedTasks.map(taskLine),
            recentCompletedTasks: presentedCompletedTasks.map(taskLine))
        project = Self.makeProjectInfo(
            sessionID: sessionID,
            settings: projectSettings,
            projection: projection)
        let retryable = presentedFailedTasks + presentedCancelledTasks
    ...
    private func taskLine(_ task: CoworkTaskView) -> CoworkTaskLine {
        ...
        let detail: String
        if AppIdentity.isCouncis, task.contract?.kind != .root {
            detail = "Internal team task · \(task.status.rawValue)"
        } else {
            detail = task.contract?.objective ?? task.report?.summary ?? task.error ?? task.result ?? ""
        }
        return CoworkTaskLine(id: task.id.rawValue, title: title, detail: detail, status: task.status.rawValue)
    }

    private func presentationTasks(_ tasks: [CoworkTaskView]) -> [CoworkTaskView] {
        guard AppIdentity.isCouncis else { return tasks }
        return tasks.filter {
            $0.contract?.kind != .review && $0.contract?.kind != .agentAdmission
        }
    }
    ...
    private func ensureAutomaticPermissionReview(existingProjection projection: CoworkProjection) async {
        ...
        let modelBinding: AgentModelBinding
        let workspaceURL: URL

        if let team = councisTeamConfiguration {
            modelBinding = team.mainBinding
        ...
        } else if let main = projection.agentRoster[mainID] {
            do {
                if let persisted = projection.agentModelBindings[mainID] {
                    modelBinding = persisted
                } else if let providerID = main.providerID ?? projectSettings.defaultProviderID {
                    modelBinding = try AgentModelBinding(
                        validatingProviderID: providerID,
                        modelID: main.model)
                } else {
                    modelBinding = try await defaultAgentBinding()
                }
            } catch {
                setPermissionReviewerStatus(.failed(
                    "Permission reviewer model could not be resolved: \(error.localizedDescription)"))
                return
            }
            workspaceURL = WorkspaceAccess.restoreAccess(forPath: main.path)
                ?? URL(fileURLWithPath: main.path)
    ...
    private func synchronizeTaskReviewerHealth(using orchestrator: Orchestrator) async {
        guard AppIdentity.isCouncis else { return }
        guard self.orchestrator != nil else {
            if taskReviewerHealth != .shuttingDown {
                taskReviewerHealth = .disabled
            }
            return
        }
        taskReviewerHealth = await orchestrator.taskReviewerHealth()
    }

    private static func shouldRefreshTaskReviewerHealth(after event: Event) -> Bool {
        switch event {
        case .taskCompleted, .taskFailed, .taskCancelled,
             .taskReviewSettled, .taskReviewExhausted:
            return true
        default:
            return false
        }
    ...
    private var taskReviewerSubmissionError: String? {
        guard AppIdentity.isCouncis else { return nil }
        switch taskReviewerHealth {
        case .healthy:
            return nil
        case .disabled:
            return "Councis cannot submit this task because @judge is unavailable. Wait for the review agent to finish starting or restart this project."
        case .degraded(let reason):
            return "Councis cannot submit this task because @judge is degraded: \(reason)"
        case .quarantined(let reason):
            return "Councis cannot submit this task because @judge is quarantined after an uncertain review failure: \(reason)"
        case .shuttingDown:
            return "Councis cannot submit this task because @judge is shutting down."
        }
    }
    ...
    private func bootstrapMainAgentIfNeeded(existingProjection projection: CoworkProjection) async {
        ...
        let url = WorkspaceAccess.restoreAccess(forPath: workspace.path)
            ?? URL(fileURLWithPath: workspace.path)
        let modelBinding: AgentModelBinding
        do { modelBinding = try await defaultAgentBinding() }
        catch {
            projectionError = "Main agent model could not be resolved: \(error.localizedDescription)"
        ...
        await synchronizeTaskReviewerHealth(using: orchestrator)
        if attached {
            rememberWorkspace(url, agentName: mainID.rawValue, isPrimary: true)
        }
    }

    private func bootstrapTaskReviewerIfNeeded(existingProjection projection: CoworkProjection) async {
        guard let team = councisTeamConfiguration,
              !didRequestTaskReviewerAttach,
              let orchestrator else { return }
        let reviewerID = Orchestrator.taskReviewerID
        guard projection.agentRoster[reviewerID] == nil else { return }
        let names = await orchestrator.agentNames()
        guard names.contains(AgentID(rawValue: projectSettings.mainAgentName)) else {
            projectionError = "@judge could not start because @main is not attached."
            return
        }
        guard let workspace = projectSettings.primaryWorkspace else {
            projectionError = "@judge requires the primary Cowork workspace."
            return
        }
        didRequestTaskReviewerAttach = true
        let url = WorkspaceAccess.restoreAccess(forPath: workspace.path)
            ?? URL(fileURLWithPath: workspace.path)
        let attached = await orchestrator.attachTaskReviewer(Agent(
            name: reviewerID,
            workspaceRoot: url,
            modelBinding: team.judgeBinding,
            profile: .readOnly,
            coordinationDepth: 0))
        await synchronizePermissionReviewerHealth(using: orchestrator)
        await synchronizeTaskReviewerHealth(using: orchestrator)
        if !attached {
            projectionError = "@judge could not be attached with fixed binding \(bindingDescription(team.judgeBinding))."
            didRequestTaskReviewerAttach = false
        }
    }
    ...
    func updateProjectSettings(_ settings: CoworkProjectSettings) {
        ...
        if AppIdentity.isCouncis {
            normalized.mainAgentName = CoworkTeamConfiguration.mainAgentID.rawValue
        } else {
            let trimmedMainAgentName = normalized.mainAgentName
                .trimmingCharacters(in: .whitespacesAndNewlines)
            normalized.mainAgentName = trimmedMainAgentName.isEmpty ? "main" : trimmedMainAgentName
        }
        if AppIdentity.isCouncis, let fixedTeam = projectSettings.teamConfiguration {
            normalized.teamConfiguration = fixedTeam
            normalized.defaultProviderID = fixedTeam.mainBinding.providerID
            normalized.defaultModelID = fixedTeam.mainBinding.modelID.rawValue
        }
    ...
    func removeAgent(name rawName: String) {
        ...
        guard AgentID(rawValue: name) != Orchestrator.taskReviewerID else {
            composerError = "@\(Orchestrator.taskReviewerID.rawValue) is controlled by the Councis review policy."
            return
        }
        isWorking = true
        let operationID = UUID()
        let operation = Task { @MainActor [weak self] in
            ...
            await self.synchronizePermissionReviewerHealth(using: orchestrator)
            await self.synchronizeTaskReviewerHealth(using: orchestrator)
            guard detached else {
    ...
    func addAgent(name rawName: String, workspace: URL) {
        ...
        Task { @MainActor [weak self] in
            ...
            let modelBinding: AgentModelBinding
            do {
                modelBinding = try await self.nextManualAgentBinding(using: orchestrator)
            } catch {
                self.addAgentStatus = .failed(
                    "Agent model could not be resolved: \(error.localizedDescription)")
                return
            }
            let attached = await orchestrator.attach(Agent(
                name: AgentID(rawValue: normalizedName),
    ...
    func send() {
        guard !isWorking, let orchestrator else { return }
        if let taskReviewerSubmissionError {
            composerError = taskReviewerSubmissionError
            return
        }
    ...
    private func validateNewAgentName(_ rawName: String) -> AgentNameValidation {
        ...
        if AgentID(rawValue: name) == Orchestrator.taskReviewerID
            || AgentID(rawValue: name) == Orchestrator.automaticPermissionReviewerID {
            return .failure("@\(name) is reserved by the Cowork review system.")
        }
    ...
    private func defaultAgentBinding() async throws -> AgentModelBinding {
        if let team = councisTeamConfiguration { return team.mainBinding }
        return try await registryBox.defaultAgentBinding()
    }

    private func nextManualAgentBinding(using orchestrator: Orchestrator) async throws -> AgentModelBinding {
        guard let team = councisTeamConfiguration else {
            return try await defaultAgentBinding()
        }
        let attachedAgents = await orchestrator.agentList()
        let occupied = Set(attachedAgents.compactMap { agent -> AgentModelBinding? in
            guard agent.name != Orchestrator.automaticPermissionReviewerID,
                  agent.modelBinding.isResolved else { return nil }
            return agent.modelBinding
        })
        return try team.strictModelAssignmentPolicy.resolve(
            providerID: nil,
            modelID: nil,
            parentBinding: nil,
            occupiedBindings: occupied,
            requiredCapabilities: [.toolCalling]).binding
    }

    private func restoredTeamMismatch(projection: CoworkProjection) async -> String? {
        guard let team = councisTeamConfiguration, let orchestrator else { return nil }
        let expected: [AgentID: AgentModelBinding] = [
            CoworkTeamConfiguration.mainAgentID: team.mainBinding,
            CoworkTeamConfiguration.judgeAgentID: team.judgeBinding,
        ]
        let workerBindings = Set(team.workerModelPool)

        // Inspect the durable roster before relying on the runtime registry.
        // Strict restore intentionally drops invalid entries; without this
        // check a dropped legacy/swapped identity could remain as a ghost in
        // the UI projection and then be silently replaced.
        for (agentID, payload) in projection.agentRoster
            where agentID != Orchestrator.automaticPermissionReviewerID {
            let projectedBinding: AgentModelBinding?
            if let rebound = projection.agentModelBindings[agentID], rebound.isResolved {
                projectedBinding = rebound
            } else if let providerID = payload.providerID {
                projectedBinding = AgentModelBinding(
                    providerID: providerID,
                    modelID: payload.model)
            } else {
                projectedBinding = nil
            }
            guard let binding = projectedBinding else {
                return "Restored @\(agentID.rawValue) has a legacy model-only identity. Councis requires an explicit provider/model binding for every data-plane role."
            }
            if let fixed = expected[agentID], binding != fixed {
                return "Restored @\(agentID.rawValue) uses \(bindingDescription(binding)), but this project fixes it to \(bindingDescription(fixed)). Start a new Cowork project or restore the original provider catalog."
            }
            if expected[agentID] == nil, !workerBindings.contains(binding) {
                return "Restored worker @\(agentID.rawValue) uses \(bindingDescription(binding)), which is not in this project's fixed worker pool. Start a new Cowork project or restore the original provider catalog."
            }
        }

        let restored = await orchestrator.agentList()
        var occupied = Set<AgentModelBinding>()
        for agent in restored where agent.name != Orchestrator.automaticPermissionReviewerID {
            if let binding = expected[agent.name], agent.modelBinding != binding {
                return "Restored @\(agent.name.rawValue) uses \(bindingDescription(agent.modelBinding)), but this project fixes it to \(bindingDescription(binding)). Start a new Cowork project or restore the original provider catalog."
            }
            if expected[agent.name] == nil, !workerBindings.contains(agent.modelBinding) {
                return "Restored worker @\(agent.name.rawValue) uses \(bindingDescription(agent.modelBinding)), which is not in this project's fixed worker pool. Start a new Cowork project or restore the original provider catalog."
            }
            if expected[agent.name] == nil, agent.coordinationDepth > 0 {
                return "Restored worker @\(agent.name.rawValue) has coordinator authority. Councis reserves coordination for @\(CoworkTeamConfiguration.mainAgentID.rawValue). Start a new Cowork project."
            }
            if !occupied.insert(agent.modelBinding).inserted {
                return "Restored agents reuse \(bindingDescription(agent.modelBinding)); Councis requires one unique provider/model binding per data-plane agent."
            }
        }
        return nil
    }

    private var councisTeamConfiguration: CoworkTeamConfiguration? {
        AppIdentity.isCouncis ? projectSettings.teamConfiguration : nil
    }

    private func bindingDescription(_ binding: AgentModelBinding) -> String {
