...
struct CoworkProjectSettings: Codable, Equatable {
    ...
    /// Additive v2 field. Older Intatis/Councis settings decode with nil and are
    /// migrated from the current provider catalog only for Councis.
    var teamConfiguration: CoworkTeamConfiguration?
    ...
    init(sessionID: SessionID,
         ...
         teamConfiguration: CoworkTeamConfiguration? = nil) {
    ...
    static func fresh(sessionID: SessionID,
                      ...
                      catalog: AppProviderCatalog) throws -> CoworkProjectSettings {
        let team = try makeCouncisTeamIfNeeded(catalog: catalog)
        return CoworkProjectSettings(
            sessionID: sessionID,
            defaultProviderID: team?.mainBinding.providerID ?? catalog.selectedProviderID,
            defaultModelID: team?.mainBinding.modelID.rawValue ?? catalog.selectedModelID,
            workspaces: [
    ...
    private static func makeCouncisTeamIfNeeded(
        catalog: AppProviderCatalog
    ) throws -> CoworkTeamConfiguration? {
        guard AppIdentity.isCouncis else { return nil }
        return try CoworkTeamConfiguration.derive(
            availableBindings: catalog.availableAgentBindings,
            preferredMain: catalog.selectedAgentBinding)
    }
}
...
enum CoworkProjectSettingsStore {
    static func load(sessionID: SessionID,
        ...
        guard let data = UserDefaults.standard.data(forKey: key(sessionID)),
            ...
            return try CoworkProjectSettings.restored(sessionID: sessionID, catalog: catalog)
        }
        var settings = decoded
        settings.sessionID = sessionID
        if AppIdentity.isCouncis {
            // Councis has one reserved user-facing coordinator identity. Older
            // project settings may contain an arbitrary Intatis main name; do
            // not let that value redefine the strict data-plane role.
            settings.mainAgentName = CoworkTeamConfiguration.mainAgentID.rawValue
            let team: CoworkTeamConfiguration
            if let persisted = settings.teamConfiguration {
                team = try persisted.validated(
                    availableBindings: catalog.availableAgentBindings)
            } else {
                team = try CoworkTeamConfiguration.derive(
                    availableBindings: catalog.availableAgentBindings,
                    preferredMain: catalog.selectedAgentBinding)
            }
            settings.teamConfiguration = team
            settings.defaultProviderID = team.mainBinding.providerID
            settings.defaultModelID = team.mainBinding.modelID.rawValue
        } else {
    ...
    private static func key(_ sessionID: SessionID) -> String {
        "\(AppIdentity.defaultsPrefix).cowork.projectSettings.\(sessionID.rawValue)"
    }
...
private extension AppProviderCatalog {
    var availableAgentBindings: [AgentModelBinding] {
    ...
    var selectedAgentBinding: AgentModelBinding {
        AgentModelBinding(
            providerID: selectedProvider?.id ?? selectedProviderID,
            modelID: ModelID(rawValue: selectedModel?.id ?? selectedModelID))
    }
}
...
struct CoworkProjectSettingsSheet: View {
    ...
    @ViewBuilder private var teamConfigurationRows: some View {
        if let team = draft.teamConfiguration {
            formRow("Main model") {
                Text(bindingLabel(team.mainBinding))
                    .font(.system(.caption, design: .monospaced).weight(.semibold))
                    .textSelection(.enabled)
            }
            formRow("Judge model") {
                Text(bindingLabel(team.judgeBinding))
                    .font(.system(.caption, design: .monospaced).weight(.semibold))
                    .textSelection(.enabled)
            }
            formRow("Worker pool") {
                VStack(alignment: .leading, spacing: 3) {
                    if team.workerModelPool.isEmpty {
                        Text("No unused worker bindings")
                            .foregroundStyle(.secondary)
                    } else {
                        ForEach(team.workerModelPool, id: \.self) { binding in
                            Text(bindingLabel(binding))
            ...
            formRow("Team models") {
                Text("Unavailable — configure at least two unique provider/model bindings.")
                    .font(.caption)
                    .foregroundStyle(.red)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func bindingLabel(_ binding: AgentModelBinding) -> String {
