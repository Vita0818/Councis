...
enum WorkspaceAccess {
    ...
    private static let bookmarkStoreKey = "\(AppIdentity.defaultsPrefix).workspace.bookmarks"

    private static func sessionBookmarkKey(_ session: SessionID) -> String {
        "\(AppIdentity.defaultsPrefix).workspace.sessionBookmark.\(session.rawValue)"
    }

    private static func sessionPathKey(_ session: SessionID) -> String {
        "\(AppIdentity.defaultsPrefix).workspace.sessionPath.\(session.rawValue)"
    }
