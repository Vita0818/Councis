...
struct RendererFixtureView: View {
    ...
    | Councis facade | Latest-only admission and raw fallback | active |
    ...
    | Councis facade | Backpressure and safe fallback |
...
private struct RendererFixtureLaunchConfiguration {
    static let stageFlag = "-CouncisRendererFixtureStage"
    static let incidentFixtureFlag = "-CouncisRendererIncidentFixture"
    static let autoExitFlag = "-CouncisRendererFixtureAutoExitSeconds"
    static let resultPathFlag = "-CouncisRendererFixtureResultPath"
    static let disableSingleDollarMathFlag = "-CouncisDisableSingleDollarMath"
...
private final class RendererIncidentReplayModel: ObservableObject {
    ...
    func loadIfNeeded() {
        ...
        guard let fixturePath else {
            fail("Missing -CouncisRendererIncidentFixture PATH")
            return
        }
    ...
    private func loadFixture() throws -> RendererIncidentFixture {
        guard let fixturePath else {
            throw RendererThreadBurstError.invalidFixture(
                "Missing -CouncisRendererIncidentFixture PATH")
        }
        let data = try Data(contentsOf: URL(fileURLWithPath: fixturePath))
