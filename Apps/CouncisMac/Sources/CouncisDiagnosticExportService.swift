...
struct CouncisDiagnosticExportResult: Sendable {
    let archiveFileName: String
    let archiveByteCount: Int
    let collectionErrorCount: Int
}

enum CouncisDiagnosticExportError: Error, LocalizedError {
...
private struct CouncisDiagnosticExportContext: Sendable {
...
private struct CouncisDiagnosticExportManifest: Codable {
...
private struct CouncisDiagnosticBundleBuilder {
    ...
    func build() throws -> CouncisDiagnosticExportResult {
        ...
        let bundleName = "Councis-Diagnostics-\(Self.fileTimestamp(context.generatedAt))"
        ...
        let archiveURL = temporaryRoot.appendingPathComponent(
            "\(bundleName).zip")
        let execution = try CouncisDiagnosticFixedProcessRunner.run(
    ...
    private func makeManifest() -> CouncisDiagnosticExportManifest {
        CouncisDiagnosticExportManifest(
            schemaVersion: 1,
    ...
    private func collectHangDiagnostics(
        ...
        guard let root = context.hangDiagnosticsRoot else {
            manifest.collectionErrors.append(.init(
                ...
                message: "The Councis hang diagnostics directory could not be resolved."))
            return
        }
        let keys: Set<URLResourceKey> = [
            .contentModificationDateKey,
    ...
    private func collectCrashReports(
        ...
        for root in context.crashReportRoots {
            ...
            reports.append(contentsOf: urls.compactMap { url in
                ...
                guard (name.hasPrefix("CouncisMac") || name.hasPrefix("Councis")),
    ...
    private func writeOwnerOnly(
        ...
        guard !components.isEmpty,
            ...
            throw CouncisDiagnosticExportError.couldNotPrepare
        }
...
private struct CouncisDiagnosticProcessExecution: Sendable {
...
private enum CouncisDiagnosticFixedProcessRunner {
    static func run(
        executableURL: URL,
        arguments: [String],
        timeoutSeconds: TimeInterval,
        maximumOutputBytes: Int
    ) throws -> CouncisDiagnosticProcessExecution {
        try Task.checkCancellation()
        let process = Process()
        ...
        let output = CouncisDiagnosticBoundedDataBuffer(
            maximumBytes: maximumOutputBytes)
        let errors = CouncisDiagnosticBoundedDataBuffer(
        ...
        return CouncisDiagnosticProcessExecution(
    ...
    private static func drain(
        _ handle: FileHandle,
        into buffer: CouncisDiagnosticBoundedDataBuffer
    ) {
        while true {
            do {
...
private final class CouncisDiagnosticBoundedDataBuffer: @unchecked Sendable {
    struct Snapshot {
        let data: Data
        let truncated: Bool
    }
