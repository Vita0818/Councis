...
// The shell lives in CouncisMacRootView; root-owned session state feeds the
...
#if canImport(AppKit)
@MainActor
final class CouncisApplicationDelegate: NSObject, NSApplicationDelegate {
    private var terminationTask: Task<Void, Never>?
    #if COUNCIS_RENDERER_VALIDATION
    private var rendererValidationWindowController:
        NSWindowController?
    #endif

    func applicationDidFinishLaunching(_ notification: Notification) {
        CouncisMacProcessDiagnostics.shared.start(
            application: NSApplication.shared)
        #if COUNCIS_RENDERER_VALIDATION
        let arguments = ProcessInfo.processInfo.arguments
        guard arguments.contains("-CouncisRendererFixture"),
              let rawSeconds = Self.argumentValue(
                  after: "-CouncisRendererFixtureAutoExitSeconds"),
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        CouncisMacProcessDiagnostics.shared.beginTermination()
        #if COUNCIS_RENDERER_VALIDATION
        ...
        #if DEBUG
        if let raw = Self.argumentValue(after: "-CouncisShutdownDeadlineMilliseconds"),
...
struct CouncisMacApp: App {
    #if canImport(AppKit)
    @NSApplicationDelegateAdaptor(CouncisApplicationDelegate.self)
    ...
    private var launchAppearance: ColorScheme? {
        ...
        let arguments = ProcessInfo.processInfo.arguments
        if arguments.contains("-CouncisAppearanceDark") { return .dark }
        if arguments.contains("-CouncisAppearanceLight") { return .light }
        #endif
        return nil
    }
    ...
    private var launchesCoworkAgentConversationFixture: Bool {
        ProcessInfo.processInfo.arguments.contains(
            "-CouncisCoworkAgentConversationFixture")
...
@MainActor
private struct CouncisProductionRootView: View {
