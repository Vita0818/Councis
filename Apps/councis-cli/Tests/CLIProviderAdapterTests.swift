...
final class CLIProviderAdapterTests: XCTestCase {
    func testFixedCoworkRolesFallBackToJSONModelNotEnvironmentMainOverride()
        ...
        let config = try CLIConfig.load(
            ...
            environment: ["COUNCIS_MODEL": "runtime/runtime-model"])
    ...
    func testExplicitPermissionReviewerModelMustResolveCanonicalConfiguredModel()
        throws {
        let directory = try makeTemporaryDirectory(
            prefix: "councis-cli-reviewer-invalid")
        defer { try? FileManager.default.removeItem(at: directory) }
        let file = directory.appendingPathComponent("councis.json")
    ...
    func testMissingReviewerCannotInventAnUnavailableJSONDefaultModel()
        throws {
        let directory = try makeTemporaryDirectory(
            prefix: "councis-cli-reviewer-missing-default")
        defer { try? FileManager.default.removeItem(at: directory) }
        let file = directory.appendingPathComponent("councis.json")
        let configuredProvider = provider(
            baseURL: "https://main.example.invalid/v1",
    ...
    func testOpenRouterKnowledgeRoleModelsStayOutOfInferenceProfiles()
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-cli-openrouter-knowledge-\(UUID().uuidString)",
                isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(
            at: directory,
    ...
    func testKnowledgeRoleOnlyRoutesComposeStrictToolsWithoutResolvingSecrets()
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-cli-knowledge-models-\(UUID().uuidString)",
                isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(
            at: directory,
            ...
            "provider": [
                "chat": [
                    "options": [
                        ...
                        "apiKey": "{env:COUNCIS_TEST_CHAT_KEY}",
                    ],
    ...
    func testKnowledgeRolesRejectImplicitSelectedProviderFallback() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-cli-invalid-knowledge-route-\(UUID().uuidString)",
                isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(
            at: directory,
    ...
    func testCLIExternalKnowledgeCreationTouchesOnlyExactLeaf() throws {
        let parent = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "councis-cli-exact-knowledge-root-\(UUID().uuidString)",
                isDirectory: true)
    ...
    func testModernConfigRoutesDedicatedImageModelWithoutSelectingItForChat()
        ...
        let directory = FileManager.default
            .temporaryDirectory
            .appendingPathComponent(
                "councis-cli-image-model-\(UUID().uuidString)",
                isDirectory: true)
        defer {
            ...
            "provider": [
                ...
                "images": [
                    "options": [
                        ...
                        "apiKey": "{env:COUNCIS_TEST_IMAGE_KEY}",
                    ],
    ...
    func testModernConfigWithoutImageModelHasNoHiddenFallback()
        ...
        let directory = FileManager.default
            .temporaryDirectory
            .appendingPathComponent(
                "councis-cli-no-image-model-\(UUID().uuidString)",
                isDirectory: true)
        defer {
            try? FileManager.default.removeItem(
                at: directory)
            ...
            "provider": [
                "test": [
                    "options": [
                        ...
                        "apiKey": "{env:COUNCIS_TEST_API_KEY}",
                    ],
                    "models": [
                        "chat-model": ["name": "Chat Model"],
                    ],
    ...
    func testModernConfigPreservesProviderAndModelNPMAdapters()
        ...
        let directory = FileManager.default
            .temporaryDirectory
            .appendingPathComponent(
                "councis-cli-provider-adapter-\(UUID().uuidString)",
                isDirectory: true)
        defer {
            try? FileManager.default.removeItem(
                at: directory)
