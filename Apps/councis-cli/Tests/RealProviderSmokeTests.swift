...
final class RealProviderSmokeTests: XCTestCase {
    func testRealKnowledgeRerankQualityWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment[
            "COUNCIS_REAL_KNOWLEDGE_QUALITY"
        ] == "1" else {
            throw XCTSkip(
                "Set COUNCIS_REAL_KNOWLEDGE_QUALITY=1 to run the configured embedding and semantic reranker over the frozen mixed-language quality set.")
        }
    ...
    func testRealKnowledgeEmbeddingAndRerankerWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment[
            "COUNCIS_REAL_KNOWLEDGE_SMOKE"
        ] == "1" else {
            throw XCTSkip(
                "Set COUNCIS_REAL_KNOWLEDGE_SMOKE=1 to spend one embedding request and one reranker request using the exact configured Knowledge routes.")
        }
        ...
        let embeddingResponse = try await models.embedding.embedWithUsage(
            ["Councis knowledge route smoke test."],
            instruction: models.embedding.configuration.documentInstruction)
    ...
    func testRealModelBuildsSearchesAndCitesExternalKnowledgeWhenEnabled()
        async throws {
        guard ProcessInfo.processInfo.environment[
            "COUNCIS_REAL_KNOWLEDGE_AGENT_E2E"
        ] == "1" else {
            throw XCTSkip(
                "Set COUNCIS_REAL_KNOWLEDGE_AGENT_E2E=1 to let the configured Agent, embedding, and reranker routes perform the full read-organize-build-search-cite flow. This can make multiple billable requests.")
        }
        ...
        let source = """
        Councis has three permission layers. DeterministicPolicyGate applies
        ...
        three Councis permission layers have? Cite only evidence returned by the
    ...
    func testRealModelBuildsSearchesAndCitesPDFKnowledgeWhenEnabled()
        ...
        guard environment["COUNCIS_REAL_KNOWLEDGE_PDF_E2E"] == "1" else {
            throw XCTSkip(
                "Set COUNCIS_REAL_KNOWLEDGE_PDF_E2E=1 and COUNCIS_REAL_KNOWLEDGE_PDF_SOURCE to let the configured Agent read selected real PDFs, build, rerank, and cite. This can make multiple billable requests.")
        }
        guard let sourcePath = environment[
            "COUNCIS_REAL_KNOWLEDGE_PDF_SOURCE"
        ], !sourcePath.isEmpty else {
            return XCTFail("COUNCIS_REAL_KNOWLEDGE_PDF_SOURCE is required")
        }
    ...
    {
        guard ProcessInfo.processInfo.environment[
            "COUNCIS_REAL_MULTIMODAL_SMOKE"
        ] == "1" else {
            throw XCTSkip(
                "Set COUNCIS_REAL_MULTIMODAL_SMOKE=1 to spend one real provider request containing both user and function-output images.")
        }
    ...
    func testConfiguredAgentRouteWithRealProvider() async throws {
        guard ProcessInfo.processInfo.environment["COUNCIS_REAL_PROVIDER_SMOKE"] == "1" else {
            throw XCTSkip(
                "Set COUNCIS_REAL_PROVIDER_SMOKE=1 to spend one minimal real provider request.")
        }
    ...
    func testRealAgentAuthorizationSidecarShapeWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment[
            "COUNCIS_REAL_TOOL_SHAPE_DIAGNOSTIC"
        ] == "1" else {
            throw XCTSkip(
                "Set COUNCIS_REAL_TOOL_SHAPE_DIAGNOSTIC=1 to verify that the configured Agent route emits a same-call authorization sidecar. This makes one billable request.")
        }
    ...
    func testRealPermissionReviewControlPlaneWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment[
            "COUNCIS_REAL_PERMISSION_REVIEW_SMOKE"
        ] == "1" else {
            throw XCTSkip(
                "Set COUNCIS_REAL_PERMISSION_REVIEW_SMOKE=1 to spend one real permission-review request.")
        }
