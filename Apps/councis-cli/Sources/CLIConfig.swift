...
struct CLIConfig {
    ...
    /// The explicitly selected Councis config is retained only so a lazy
    ...
    static func load() throws -> CLIConfig {
        ...
        func value(_ envKey: String, _ fileKey: String, fallback: String?) -> String? {
            if let e = cliEnvironmentValue(env, currentKey: envKey) {
                return e
            }
        ...
        let mode = Mode(rawValue: value("COUNCIS_MODE", "mode", fallback: "chat")!.lowercased()) ?? .chat
        // Ask the endpoint for token usage (default on). Set COUNCIS_USAGE=0 if an
        // endpoint rejects the stream_options field.
        let usageStr = value("COUNCIS_USAGE", "usage", fallback: "1")!.lowercased()
    ...
    /// Deterministic seam used by the offline self-test and by `COUNCIS_CONFIG`.
    /// The schema is the same Councis/OpenCode-compatible provider map used by
    ...
    static func load(configurationFileURL: URL,
        ...
        if let baseOverride = cliEnvironmentValue(
            environment,
        ...
        let selectedInlineKey = cliEnvironmentValue(
            environment,
            currentKey: "COUNCIS_API_KEY")?
        ...
        let rawReasoning = value("COUNCIS_REASONING", fallback: nil)
        let reasoning = try parsedReasoningEffort(rawReasoning)
        let mode = Mode(rawValue: value("COUNCIS_MODE", fallback: "chat")!.lowercased()) ?? .chat
        let usageStr = value("COUNCIS_USAGE", fallback: "1")!.lowercased()
        let includeUsage = !(usageStr == "0" || usageStr == "false" || usageStr == "off")
        let configuredMaxSteps = value(
            "COUNCIS_MAX_STEPS",
            fallback: nil)
