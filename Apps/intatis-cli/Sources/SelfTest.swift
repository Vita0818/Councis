...
func runSelfTest() async throws {
    ...
    let listing = try await ListFilesTool().execute(ToolArgs(raw: #"{"path":"."}"#),
                                                    in: ToolContext(workspaceRoot: workspace))
    let okList = listing.text.split(separator: "\n").contains("note.txt")
    let okEscape = !PathConfinement.isWithin("../outside.txt", root: workspace)
    out(okCode && okList && okEscape
        ? "\(green)PASS\(reset) wrote, read, listed note.txt, and rejected workspace escape\n"
               : "\(red)FAIL\(reset) file not written (got: \(onDisk.isEmpty ? "<empty>" : onDisk))\n")

    out("\n" + (okChat && okCode && okList && okEscape
