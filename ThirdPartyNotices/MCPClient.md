...
authorizes an MCP server product: Councis excludes every upstream server
...
Tools wire schemas. Councis moved those five wire-schema files out of the
...
an Councis MCP client package dependency.
...
Production MCP HTTP/OAuth I/O uses the Councis-owned
`CouncisCurlTransport` boundary, not the SDK HTTP transport. macOS links the
