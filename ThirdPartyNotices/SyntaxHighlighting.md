...
derivative used by Councis. Code blocks are rendered as plain text with native
...
contain no HighlightSwift or highlight.js dependency. The former Councis
vendored resources are removed from the current working tree and from the
`CouncisSharedUI` resource list:
...
present in the current Councis macOS product artifact.

## Historical provenance

Councis previously shipped a selectively vendored highlight.js 11.11.1 engine
