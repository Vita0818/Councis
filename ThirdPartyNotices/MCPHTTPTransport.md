...
This notice covers the native `CouncisCurlTransport` dependency used by the
...
`Package.swift` links `CouncisCurlTransport` with `.linkedLibrary("curl")`.
For the macOS Developer ID product this resolves to the libcurl
provided by the Apple SDK/operating system. Councis does not vendor a Darwin
...
Councis ships no iOS App product. Conditional library portability does not
create another distributed transport surface.
...
that executable even though Councis does not redistribute the SDK archives as
standalone files.
...
pkg-config SPDX identifier. Councis resolves that ambiguity in the
