...
- Local path: `.agents/skills/councis-skill-creator/`
...
Every distributed file above is modified by Councis. The upstream
`agents/openai.yaml`, icons, images, branded assets, and all other Codex
system Skills are not copied.

## Councis modifications

- Renamed the Skill from `skill-creator` to `councis-skill-creator` so the
  project copy does not collide with the separately discovered Codex system
  Skill of the same upstream name.
- Reworked the operating instructions for Councis project roots,
...
- Removed examples that trigger Councis' credential-shaped-content scanner
...
- Narrowed `agents/openai.yaml` guidance to Councis' supported MCP dependency
  subset. Interface and policy fields remain optional cross-harness metadata;
  they are not described as Councis runtime permissions. The helper does not
...
normal Councis permission and durable-execution path. It adds no network,
filesystem, shell, MCP, communication, or delegation capability of its own.
Councis currently ships no iOS App target; the Skill is not an App-linked
runtime component.
...
document all local modifications, and rerun the Councis Skill validation and
