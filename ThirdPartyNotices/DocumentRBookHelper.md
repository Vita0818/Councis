...
- Local component: `Packages/CouncisTools/Runtime/rbook-helper`
- Reuse class: `dependency` inside an Councis-owned `external-runtime` helper
- Product scope: macOS/Linux document tools only; Councis ships no iOS App
- Runtime protocol: fixed `json-v1` argv plus a versioned JSON envelope
- Network: denied by the Councis document process boundary
...
against operation postconditions before Councis runs EPUBCheck and considers
the staged commit.

## Binary distribution gate

Before a compiled helper is put into a signed Councis distribution, the
...
typed `backend_missing`; Councis must not download a helper or switch to a
