...
: "${COUNCIS_SWIFT_BIN:?set COUNCIS_SWIFT_BIN to the validated Swift executable}"
: "${COUNCIS_LINUX_SDKS_PATH:?set COUNCIS_LINUX_SDKS_PATH to the directory containing the validated artifact bundles}"
: "${COUNCIS_LINUX_SDK_AARCH64:?set COUNCIS_LINUX_SDK_AARCH64 to the aarch64-swift-linux-musl SDK selector}"
: "${COUNCIS_LINUX_SDK_X86_64:?set COUNCIS_LINUX_SDK_X86_64 to the x86_64-swift-linux-musl SDK selector}"
: "${COUNCIS_LINUX_VALIDATION_ROOT:?set COUNCIS_LINUX_VALIDATION_ROOT to an empty or reusable output directory}"

case "$COUNCIS_LINUX_VALIDATION_ROOT" in
    ""|"/"|"."|"..")
        echo "unsafe COUNCIS_LINUX_VALIDATION_ROOT" >&2
        exit 2
        ;;
esac

if [ ! -x "$COUNCIS_SWIFT_BIN" ]; then
    echo "COUNCIS_SWIFT_BIN is not executable" >&2
    exit 2
fi
if [ ! -d "$COUNCIS_LINUX_SDKS_PATH" ]; then
    echo "COUNCIS_LINUX_SDKS_PATH is not a directory" >&2
    exit 2
fi

repository_root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
mkdir -p "$COUNCIS_LINUX_VALIDATION_ROOT"
...
build_one() {
    ...
    scratch="$COUNCIS_LINUX_VALIDATION_ROOT/$architecture"
    ...
    "$COUNCIS_SWIFT_BIN" build \
        --package-path "$repository_root" \
        --disable-sandbox \
        --swift-sdks-path "$COUNCIS_LINUX_SDKS_PATH" \
    ...
    binary="$binary_directory/councis"
    if [ ! -f "$binary" ]; then
        echo "missing councis product for $architecture" >&2
        exit 1
    fi
...
build_one aarch64 aarch64-swift-linux-musl "$COUNCIS_LINUX_SDK_AARCH64"
build_one x86_64 x86_64-swift-linux-musl "$COUNCIS_LINUX_SDK_X86_64"
