// Validation-only macOS process watchdog for the Councis renderer fixture.
//
// Build (does not launch any app):
//   xcrun swiftc -O -module-cache-path /private/tmp/councis-renderer-watchdog-module-cache \
//     scripts/RendererValidationWatchdog.swift \
//     -o /private/tmp/councis-renderer-validation-watchdog
//
// The `run` command intentionally accepts an Councis validation app plus a
...
private let watchdogLockPath = "/private/tmp/com.Vita0818.Councis.renderer-validation-watchdog.lock"
private let expectedBundleIdentifier = "com.Vita0818.Councis"
private let expectedFixtureSHA256 = "fb548849d0b708d31e8c6d055805f29f5c09ee4c8306bf9adc537a48e95707f1"
private let validationExecutableMarker = Data("Councis renderer fixture".utf8)
...
private enum RendererMode: String, Encodable {
    ...
    var launchArgument: String {
        switch self {
        case .microsoft: "-CouncisMicrosoftMarkdownMessages"
        case .plainSafe: "-CouncisPlainSafeMessages"
        }
    }
}
...
private enum MathMode: String, Encodable {
    ...
    var launchArgument: String? {
        switch self {
        case .disabled: "-CouncisDisableSingleDollarMath"
...
private enum Appearance: String, Encodable {
    ...
    var launchArgument: String {
        switch self {
        case .light: "-CouncisAppearanceLight"
        case .dark: "-CouncisAppearanceDark"
        }
    }
}
...
private struct SampleRecord: Encodable {
    let schema = "councis.renderer-watchdog.sample.v1"
    let sequence: Int
...
private struct EventRecord: Encodable {
    let schema = "councis.renderer-watchdog.event.v1"
    let sequence: Int
    let elapsedSeconds: Double
    let kind: String
    let detail: String
...
private struct Manifest: Encodable {
    let schema = "councis.renderer-watchdog.manifest.v4"
...
private struct ResultRecord: Encodable {
    let schema = "councis.renderer-watchdog.result.v1"
    var outcome: Outcome
    var detail: String
    let elapsedSeconds: Double
    let childPID: Int32
...
private struct RuntimeLogAuditRecord: Encodable {
    let schema = "councis.renderer-watchdog.runtime-log-audit.v1"
...
private struct FixtureMachineResult: Codable {
    static let currentSchema =
        "councis.renderer-fixture.result.v1"
...
private func isFamilyMember(_ identity: ProcessIdentity, familyNames: Set<String>) -> Bool {
    ...
    return candidates.contains { candidate in
        familyNames.contains(candidate)
            || candidate.hasPrefix("CouncisRendererValidation")
            || candidate.hasPrefix("CouncisRenderer")
    }
}
...
private func validationEnvironment() -> [String: String] {
    ...
    environment["COUNCIS_RENDERER_WATCHDOG"] = "1"
    return environment
}
...
private func validateApp(_ configuration: RunConfiguration) throws -> (String, Manifest) {
    ...
    guard URL(fileURLWithPath: executablePath).lastPathComponent == "Councis" else {
    ...
    guard executableData.range(of: validationExecutableMarker) != nil else {
        throw WatchdogFailure.preflight(
            "validation fixture marker is absent; refusing to launch a normal Councis build")
    }
...
private func runValidation(_ arguments: [String]) throws -> Int32 {
    ...
    var launchArguments = [
        ...
        "-CouncisRendererIncidentFixture",
        canonicalPath(configuration.fixturePath),
        "-CouncisRendererFixtureAutoExitSeconds",
        String(format: "%.3f", limits.observationSeconds),
        "-CouncisRendererFixtureResultPath",
    ...
    var result = try runMonitor(
        specification: MonitorSpecification(
            ...
            familyNames: ["Councis", "CouncisRendererValidation"],
            excludedPIDs: [getpid()],
            limits: limits,
...
private struct SelfTestSummary: Encodable {
    let schema = "councis.renderer-watchdog.self-test.v1"
...
private func validateClosedSetSelfTestContracts() throws {
    ...
    guard isolatedMathStages.allSatisfy({ $0.accepts(.isolation) }),
          ...
          MathMode.disabled.launchArgument == "-CouncisDisableSingleDollarMath",
...
private func printUsage() {
    ...
    The run command only accepts a hash-pinned Councis validation build.
