# Councis SwiftStreamingMarkdown Vendored Patch Ledger
...
This ledger describes the Councis-maintained vendored derivative of Microsoft
SwiftStreamingMarkdown. The vendored snapshot lives at
`Vendor/SwiftStreamingMarkdown` and is versioned by the containing Councis Git
revision. It is third-party-derived code, not independently authored Councis
renderer code.
...
- Reserve the public `@concurrent` transfer boundary for Councis' external
  parse scheduler. Councis must own the result on `@MainActor` and retain a
...
- Keep any still-compiled legacy image/citation compatibility source out of the
  Councis production parse profile; it is not a first-release capability.
...
- `Sources/MarkdownText/UI/TableView.swift`
- `Packages/CouncisSharedUI/Sources/MessageRendering/CouncisMessageContentView.swift`
...
- `Packages/CouncisSharedUI/Tests/MessageRenderingTests.swift`
...
- Track the last valid laid-out paragraph width independently from the
  ...
  contract.
- Remove the Councis facade's whole-rich-document
...
- Run the full retained Release suite under strict concurrency and warnings as
  errors on macOS.
- Add Councis-level golden rendering, streaming replacement, selection/copy,
...
- containing Councis revision:
  `Packages/CouncisSharedUI/Tests/MessageRenderingTests.swift`
- containing Councis revision:
  `Apps/CouncisMac/Sources/RendererFixtureView.swift`
- containing Councis revision:
...
- Host accepted formulas through a live TextKit 2
  ...
  Councis' display revision so an old-size attachment cannot overwrite a
  newer request.
...
- Treat iosMath's eight unmodified OpenType fonts and accompanying math-table,
  ...
  They do not replace or determine Councis' product-interface font.
...
- The Councis root and derivative `Package.resolved` files currently agree on
...
- The interactive Computer Use soak recorded 18 AppKit negative-geometry
  issues in the Councis PID. Each cluster immediately followed system
...
- containing Councis revision:
  `Packages/CouncisSharedUI/Tests/MessageRenderingTests.swift`
- containing Councis revision:
  `Apps/CouncisMac/Sources/{CouncisChatScreen,RendererFixtureView}.swift`
- containing Councis revision:
...
- Councis SharedUI's syntax-agnostic whole-message rich-admission boundary and
...
- Root `MessageRenderingTests` passed 41/41; `swift build --disable-sandbox
  --target CouncisSharedUI`, `xcodegen generate`, CouncisMac macOS Debug, and
  the then-existing iOS Simulator target all succeeded. This is historical
  evidence; Councis no longer ships an iOS App target.
...
`Councis Renderer Validation` instances were accidentally allowed to coexist.
...
- The then-existing macOS Debug/Release and iOS Simulator Debug/Release
  products built successfully. This predates the current macOS-only product
  graph. The two historical app bundles each contain the audited
...
The Councis root revision is the immutable identity for a distributed source
...
notices and provenance, rerun the full Councis cutover suite, and delete the
