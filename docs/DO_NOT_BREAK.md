...
- **Fresh Cowork bootstrap**：唯一无需普通模型审批的初始路径是 brand-new empty session 的严格 settings-first 十事件合同：settings；`@main` workspace/capability/agent；`@permission-reviewer` workspace/capability/agent；`@judge` workspace/capability/agent，连续 `seq 0...9`。三 agent identity 和 leases 必须不同；reviewer 继承 Main exact binding，必须 read-only、空工具、无 communication/delegation、depth 0；Judge 使用 independently resolved exact binding、read-only ordinary-worker capability、depth 0。任何事件存在、非空 roster、binding/profile mismatch、路径/sensitive-root/root-identity 问题或持久化失败都必须 fail closed；bootstrap 不得调用模型/provider，也不能被普通 attach/spawn/tool/recovery 复用，历史 session 不得自动回填 Judge。
- **Judge 配置与身份边界**：canonical `judge_model` 与顶层 `model` 使用同一 `<provider>/<model-id>` 解析规则，只影响未来 fresh Cowork；字段缺失时继承 `model`，显式值无法 exact resolve 时必须在创建 session durable state 前失败，不得退回 Main/当前 Chat 模型。`@judge` 是 fixed ordinary data-plane agent：不可 attach/spawn/replace/remove，不得被 `delegate_task(to:auto)` 选择，但可以被显式 task/message/delegation 使用并只返回普通文本；不得获得 coordinator、permission reviewer 或 Goal Verifier 权限。
...
- **Cowork project-mode agent 管理**：GUI 无 @mention 的用户消息必须默认发送给项目 `@main`，不得在已有多个 agent 时强迫用户手动选择目标。子 agent 应由 `@main` 通过 `spawn_agent` / `delegate_task` 等工具按任务需要创建和管理；`spawn_agent` 默认创建 worker（`coordinationDepth=0`），只有显式 `canCoordinate:true` 或未来明确 capability lease/任务契约授予时，子 agent 才可获得 coordinator 工具并继续管理下级 agent。右侧 inspector 不提供 agent 删除或详情管理；如未来在 settings/专门管理面板恢复人工删除普通子 agent，也不得删除 `@main`、`@permission-reviewer` 或 `@judge`，且不得删除用户文件或清理未提交工作区，只能更新 Orchestrator roster 与非主 workspace metadata。
...
- 改 session settings/projection/workspace bookmark/bootstrap/recovery：至少覆盖 `SessionStateProtocolTests`、`SessionProjectionStoreTests`、`IntatisCoreTests`、`AutomaticPermissionReviewTests`；触及模型改名工具时追加 `SessionNamingToolTests`、`SessionRenameAgentLoopTests`、`IntatisPermissionTests`、`ToolRegistryLeaseTests` 与 main/worker tool-surface 回归。必须证明 strict ten-event/no-provider bootstrap、EventLog-wins cache repair、owner-only binary bookmark、legacy provenance+marker 幂等、historical main/reviewer repair、historical Judge no-backfill、Rename EventLog-first、operation retry/冲突/晚改名保护、secret pre-authorization denial 与 exact-session capability isolation，并构建受影响 macOS target；只有共享源码实际影响 iOS 时才构建 iOS target。
