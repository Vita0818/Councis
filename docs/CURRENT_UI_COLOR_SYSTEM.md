...
> 2026-08-17 产品边界：当前只发行 macOS `Councis.app`。iOS App target、source
> 与 resources 已删除；本文后续按日期保留的 iOS 设计/验证记录仅是历史，不属于
> 当前实现入口或 release gate。共享 `CouncisSharedUI` 的条件编译不代表 iOS 产品。
...
4. 功能层（导航、composer、模型菜单、主要操作与紧凑交互控件）在 macOS 26 采用原生 Liquid Glass；继承代码中存在的模式切换同样遵守该规则，但 Councis macOS 当前不渲染它。内容层只允许用户消息气泡与 Cowork 紧凑 trailing status rail 两类明确例外。
...
- macOS detail 区由 `CouncisSystemCanvas` 渲染动态 window surface。
- macOS sidebar 不设置 `CouncisTheme.canvas` 或其他背景覆盖层；`NavigationSplitView` 继续提供系统侧栏材质，内部是 `Councis` 标题、Cowork session history/New 与底部 Settings 的连贯结构。Chat/Code/Cowork 模式导航不渲染，初始 selection 固定为 Cowork；隐藏的 enum/view/runtime/history 实现仍保留。
- 已删除 iOS App 的旧 `NavigationStack`/drawer 设计只保留在历史记录中，不属于当前组件映射。
...
- Councis macOS 当前只在 sidebar navigation/session center 渲染 Cowork sessions、New 与
  Settings；Chat / Code 模式行不渲染但继承实现不删除。Cowork session 保留明确选中态与原生
  Rename/Delete context menu。
- 设置表单继续优先使用原生控件；主要操作按语义使用 glass 或 glass prominent，不画自定义黑白按钮。

## 4. API 与部署边界

- `glassEffect`、`GlassEffectContainer`、`.glass` 和 `.glassProminent` 在当前 macOS 26 App 中启用。
- 当前 App deployment target 是 macOS 26；源码中的 Material / bordered fallback 只保留为防御性实现，不属于当前产品验收矩阵，也不能被替换成手绘静态“仿玻璃”。
- `CouncisSharedUI` 通过可用性检查共享实现，不反向依赖 macOS app target，也不构成另一个 App 产品。
- 系统 Reduce Transparency、Increase Contrast、accent、active / inactive window 与其他辅助功能设置应由原生 API 自动响应，不能用固定值覆盖。

## 5. 事实来源

- `Apps/CouncisMac/Sources/CouncisDesign.swift`：系统 window canvas、macOS 13 兼容表面、语义色与内容卡片。
- `Apps/CouncisMac/Sources/CouncisMacRootView.swift`：系统 split-view sidebar 材质、Councis title、
  当前 Cowork-only history/Settings 结构、隐藏但未删除的 mode 实现与 detail canvas。
- `Packages/CouncisSharedUI/Sources/ThreadSurfaces.swift`：用户消息原生 regular glass helper、结构化内容 Material、30×30 sidebar New 圆形 glass control、原生圆形 icon controls、40pt composer/selection-menu 几何合同、两排 composer、首排 usage strip 与可选 accessories。
- `Packages/CouncisSharedUI/Sources/Views.swift`：共享 Chat 消息和 composer；仅用户消息使用 glass 气泡，其余对话角色继承系统 canvas。
- `Packages/CouncisSharedUI/Sources/CodeViews.swift`、`CoworkViews.swift`、`ArtifactViews.swift`：各产品面的内容层 / 功能层映射。
- `Apps/CouncisMac/Sources/CouncisChatScreen.swift`、`CouncisMacApp.swift`：macOS Chat、设置与 home CTA。
...
- SwiftPM build 通过。
- CouncisMac macOS Debug 与 IntatisiOS Simulator Debug 构建通过。
...
- Light / Dark 验收使用 DEBUG-only 启动参数 `-CouncisAppearanceLight` / `-CouncisAppearanceDark` 隔离测试，不修改用户的全局系统 Appearance；生产启动不设置偏好，始终跟随系统。
...
- `swift build`、CouncisMac macOS Debug、IntatisiOS Simulator Debug 与 `CoworkInferencePresentationTests` 4/4 通过。
...
- 没有硬编码白色背景，也没有修改字体。`MessageRenderingTests` 22/22、`swift build --disable-sandbox`、CouncisMac macOS Debug 与 IntatisiOS Simulator Debug build 通过；运行态 Light / Dark 和真实长回复视觉复核仍待用户检查。
...
- Swift parse、`swift build --target CouncisSharedUI`、`CouncisSharedUITests` 50/50、`PerAgentInferenceProfileTests` 20/20、`SubmittedIntentStoreTests` 11/11、`SubmissionProjectionTests` 4/4、XcodeGen、CouncisMac macOS Debug 与 IntatisiOS generic Simulator Debug build 均通过。
...
- sidebar 当前为系统 `NavigationSplitView` 材质内的 `Councis` 标题、带 SF Symbol 的 Chat/Code/Cowork 竖向三行导航、mode-specific `Recent` history/New 与底部 Settings；仅当前模式行使用 interactive Liquid Glass。该状态取代同日较早的单一 `List(selection:)` 和横向 segmented control 修订；session Rename/Delete、busy delete gate 与 durable selection 逻辑保持不变。
...
- 原生控件 fitting-size probe 确认 Recent `+` 为 30×30，plain native `Menu` 加共享 interactive glass label 后为 40pt 高；第二排 glass/glassProminent/bordered 按钮与单行输入均为 40pt。Swift parse、SharedUI build、`CouncisSharedUITests` 50/50、`PerAgentInferenceProfileTests` 20/20、XcodeGen、macOS Debug 与 iOS generic Simulator Debug build 均通过。
...
- Chat、Code、Cowork 和共享 iOS Chat 的用户气泡继续靠右并保留既有 Material/宽度合同，但不再重复显示 `You`；assistant、agent、system 的 structured identity header 与 agent timestamp 保留。macOS sidebar 品牌块只显示 `Councis`。active Chat/Code/Cowork session header 和 sidebar Recent row 都只显示 session name，不在其下显示灰色 model/provider/host、workspace/state、agent/running、event/date/path/runtime metadata；空态首页与 Settings 的说明性 subtitle 不属于 session metadata，继续保留。
...
- iOS 不再在 App 根视图设置全局 `.fontDesign(.serif)`。与 macOS 相同，serif 只用于
  品牌 `Councis`、当前 session 和 Settings 页面标题；正文、composer、按钮、菜单、
...
- iPhone 17e Simulator 已检查 Light/Dark 主界面、Light 抽屉、Settings 与主屏幕安装态；
  根 `Councis.icon` 的 2026-08-02 22:26:51 版本正确显示为新版指针图标。对比图与详细
...
- Code/Cowork 删除 `CouncisThreadViewportFramesPreferenceKey`、GeometryReader frame probe 和
...
- macOS Chat、共享 iOS Chat 与 Code/Cowork 共用消息行都收敛为同一角色规则：只有用户消息
  ...
  conversation canvas。
- 用户气泡复用 `ThreadSurfaces.swift` 已有的 `councisLiquidGlass`，在当前 macOS 26 / iOS 26
...
- `ThreadLayoutTests` 18/18、`swift build --disable-automatic-resolution`、CouncisMac macOS Debug
...
- `ThreadLayoutTests` 21/21、`swift build --disable-automatic-resolution`、CouncisMac macOS Debug
