...
托管搜索时，Councis 向模型提供厂商对应的搜索能力，并由模型通过 `tool_choice: auto` 自己决定
当前问题是否需要搜索。该 route 不支持、未声明、无法确认或尚未适配时，Councis 静默发送普通
Chat 请求：不搜索、不提示，也不切换到其他模型或搜索服务。

## 用户可观察行为

| 当前所选 exact Chat route | Councis 发给模型的能力 | 最终行为 | 用户界面 |
|---|---|---|---|
...
“模型自己选择”只表示当前所选模型决定是否调用已经提供给它的托管搜索能力。Councis 不按关键词
...
在运行时没有效果。Councis 新生成的配置不应再主动加入该字段。是否在未来 schema 版本中彻底
...
上表描述的是 Chat 的 provider-hosted 能力，不是 Chat 中的 Councis Tool。现有
`Capability.toolSearch` 表示 MCP deferred `tool_search` 合同，不得复用为网络搜索能力。
Code/Cowork 另有一个显式 `hosted_web_search` Councis Tool 包装同一类 provider-hosted wire；它与
...
- Councis 只拥有运行时结构字段。支持搜索时只注入该 dialect 必需的 tool/endpoint 字段；不支持
...
- provider annotation 若同时暴露 `content`、`start_index`、`end_index`，Councis 必须和
...
- Chat 不注册或调用 Councis 搜索工具，也不调用 `hosted_web_search`、`web_fetch`、
...
因此，保留在 macOS 源码中的 Chat runtime 仍是无 Councis Tools、无
PermissionEngine 的产品面；托管搜索只是在当前 exact provider wire 上向当前
模型提供的一项可选能力。Councis 当前 GUI 默认仍只呈现 Cowork。
...
- macOS 新生成的 Councis 配置不再写出 `web_search_model`；读取与内存兼容字段仍保留，更新既有
...
- 离线 provider/request、capability、fallback 与 ChatLoop citation tests 已覆盖本文核心矩阵；
  ...
  fail-closed 与 lease 隔离。旧 macOS/iOS 构建结果只属于删除 iOS target 前的历史；当前验证必须
  重新使用 `CouncisMac` 和 SwiftPM/CLI 矩阵。完整 `swift test` 是否通过以 `docs/TESTING.md`
  的最近一次真实结果为准。
