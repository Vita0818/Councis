...
修改 Councis 固定 Judge、fresh Cowork bootstrap、fixed-role 配置、Main/Judge prompt、Cowork-only
macOS 入口或 orchestration Skill 时，至少运行：
...
xcodebuild -project Intatis.xcodeproj -scheme IntatisMac -configuration Debug \
  -derivedDataPath /tmp/councis-judge-derived CODE_SIGNING_ALLOWED=NO build
```

必须证明：macOS 初始 selection 为 Cowork 且 sidebar 不渲染 mode navigation，同时 Chat/Code
enum/view/runtime/history 源码未删除；modern macOS/CLI `judge_model` 只接受 configured exact base route，
missing key 只继承同一 JSON 顶层 `model`，显式 null/错误类型/空值/未知 route/损坏配置 fail closed，且
UI/session/Main/rebind 不能改写 Judge。fresh empty session 必须以十个连续事件原子登记 settings、
Main/Judge/reviewer 三份独立 identity/workspace/capability lease，bootstrap 前后 exact-resolve 且不调用
provider；Judge 必须 read-only、不可 attach/spawn/detach/remove/rebind/recycle、无 coordinator/run-control、
不进入省略目标/auto delegation，但显式 delegation 能收到 Judge prompt 并返回 ordinary report。还要
负断言 Judge 不能代替 Permission Reviewer/GoalVerifier，legacy non-empty session 不自动补写 Judge，
Main/Judge prompt 与 Skill 的多候选比较只是可选建议，Skill validator 与 realistic routing exercise 均
通过。修改 CLI bootstrap 签名时另运行 `swift test --filter CLIAttachmentTests`；修改共享/iOS-linked target
或项目配置时追加对应 iOS generic Simulator build。

2026-08-15 本目标的实际定向结果：`xcodegen generate`、版本一致性脚本与 `swift build` 通过；
`ContextProjectionTests` 24/24、`AutomaticPermissionReviewTests` 42/42、
`CLIProviderAdapterTests` 15/15、`ThreadLayoutTests` 22/22、`CLIAttachmentTests` 4/4，全部
0 failures；`IntatisSkillsTests` 29/29。Skill validator 返回 `Skill is valid`，Main → explicit Judge 的 scripted routing、
fixed-role 负边界与独立 binding/bootstrap 断言共同构成 realistic exercise。`IntatisMac` macOS
Debug unsigned build 和 `IntatisiOS` generic Simulator Debug unsigned build 均通过；没有构建遗留
`IntatisMacAppStore`。本轮未运行整仓 `swift test`、真实 provider / credential / network、GUI /
Computer Use、真机、签名、公证或发行 smoke。
