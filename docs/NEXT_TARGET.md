...
## 目标：完成 v0.5 Developer ID 直接分发候选

把当前 v0.5 源码收敛为可验证、可安装、可直接分发的 macOS ZIP/DMG；不走 App Store。

## 已完成

- 来源版本事实源此前已从滞留的 `0.12 (1)` 校准为 `0.32 (32)`；Councis 当前版本随后按
  用户要求调整为 `0.5 (33)`，build number 保持单调递增。
...
- 当前仓库文档已重新划分为当前规范和历史证据，README/状态/测试不再以旧 v0.9/v0.16
  里程碑冒充当前版本。
- Xcode 工程已重新生成；v0.5 的 macOS Debug 与 iOS Simulator Debug 均构建通过，最终
  bundle 均为 `0.5 (33)`。先前的 universal Release 结果只属于 v0.32 历史证据，v0.5
  universal Release 仍须在正式发行 gate 中重新验证。
...
- 不把历史 v0.10/v0.16 或 Intatis v0.32 来源记录批量改名为 v0.5；它们是历史证据。
