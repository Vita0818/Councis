...
```text
macOS Councis Cowork-only sidebar or iOS drawer/top New Chat -> selected SessionID -> per-session EventLog
...
Cowork session open/new -> checked EventLog is canonical for settings, roster, leases and migrations
  ...
  -> brand-new empty session: one strict settings-first ten-event batch
       ...
       seq 7...9 distinct @judge workspace/capability leases + agent registration
       reviewer inherits @main exact binding and remains read_only/no-tools/no-communication/no-delegation/depth 0
       judge uses exact judge_model binding (missing inherits model), read_only ordinary-worker capabilities/depth 0
...
- Cowork session 可绑定一个或多个用户选择的工作目录；EventLog settings 只保存 secret-free path/agent/primary/future-profile/permission/token-budget metadata，bookmark bytes 只进入 session-owned capability plist。brand-new session 中，用户明确选择 primary workspace 后，固定十事件 bootstrap 同时记录 settings、`@main`、reviewer 与 Judge 的独立 leases/identities，不再让 reviewer 重复审批同一次选择；Judge 使用同一 primary workspace 的 read-only lease 与自己的 exact binding。任何后续目录新增、普通 agent attach 或 spawn 仍依赖 workspace bookmark 与既有权限流，历史缺 main/reviewer 只能走上文专用 host recovery，历史 session 不自动补 Judge，也不得复用 fresh bootstrap。
...
- `AppSessionRuntimeManager` 是进程级 runtime owner，使用 exact `{SessionKind, SessionID}` 缓存 Chat/Code/Cowork runtime，并发布 busy/status/removal。窗口级 `AppEnvironment` 只持有展示状态；Councis 当前 macOS 根窗口固定呈现 Cowork，但 Cowork session/History 切换、Command-W、关闭最后窗口仍都不触发 runtime stop。Command-N 创建新窗口时仍连接同一个 manager，因此不会为已有 session 重复创建 runtime。
...
- **Chat/Code/Cowork session/history**：macOS `IntatisMacRootView` 通过 root-owned view model 和 `SessionHistoryStore.recentSessions(kind:)` 将 Cowork 最近 sessions 投影到 sidebar navigation/session center；继承的 Chat/Code session store 与 runtime 路径仍保留，但当前 Councis 根界面没有它们的可见模式入口。iOS `IOSAppEnvironment` 仍只恢复 Chat session，无历史时才使用 `sess_ios`。新建会话生成新的 `SessionID.new()` 并打开独立 `EventLog` 与 artifact store；窗口只切换 presentation，不停止其他 retained runtime。当前 macOS sidebar `+` 只触发 Cowork 新建。恢复历史会话只切换到对应 `events.jsonl`，不会把新消息继续追加到旧的固定默认日志。macOS session row 的原生右键菜单支持 Rename/Delete；Code 与 Cowork `@main` 另可通过 `rename_session` 改当前会话，model 不提供 SessionID/kind。两条 Rename 路径都先追加 EventLog settings rename 事件并刷新派生 `<session>/session.json`，再通过 exact-session、revision/seq 有序的低频 publisher 更新所有窗口；不改目录名、`SessionID` 或既有 envelope。Delete 在二次确认后删除目标 session 目录及其 session-owned bookmark/settings/projection，不触碰绑定工作区内容，当前运行中的 session 禁止删除。路径与元数据规则在 `IntatisCore` 复用，平台层只传不同 application-support root。
...
- **macOS advanced config**：macOS GUI 启动时先检查用户显式设置的 `INTATIS_CONFIG` 文件；未设置时按顺序只检查 Intatis-owned 路径：`~/.config/intatis/intatis.json` / `intatis.jsonc`、app support 的 `intatis.json` / `intatis.jsonc`，最后才兜底读取旧 Intatis `config.json` / `config.jsonc`。不会自动发现任何名为 `opencode.json` 的文件，也不读取 `~/.config/opencode/` 下的 OpenCode app 配置；OpenCode-compatible 只表示 JSON shape 兼容。找到可解码的 JSON/JSONC 后覆盖 UserDefaults provider catalog；聊天页当前选择覆盖层仍可覆盖 JSON 顶层 `model`，但不覆盖 `judge_model`。设置页的 Open Intatis Config 按钮会打开当前生效的 Intatis-owned 文件或 `INTATIS_CONFIG` 显式指定文件；若只发现旧 `config.json`，会从当前 catalog 生成新的 `~/.config/intatis/intatis.json` 模板并优先打开。保存设置时，用户本次主动输入的 API key 会写入同一个可编辑 provider JSON 的 `provider.<id>.options.apiKey`；写入用户 config 失败时回退到 app support `intatis.json`。创建的模板来自当前 provider catalog，输出 OpenCode-compatible `$schema` / `enabled_providers` / `model` / `judge_model` / role-specific model fields / `provider.<id>.npm` / `name` / `options.baseURL` / `options.apiKey` / `models`，`options.apiKey` 默认是 `{env:...}` 引用而非明文。支持兼容读取旧 direct `providers` 数组，也兼容读取 Intatis 扩展字段 `chatEndpoint` / `apiKeySource`；推荐新配置使用 `provider.<id>.options.baseURL`、`options.apiKey`（OpenCode-style 明文、`{env:NAME}` 或 `{file:path}` 均可由真实请求懒加载）、`models`，以及顶层 `model` / `judge_model` 形如 `<provider>/<model-id>`。支持 OpenCode-compatible 的 `enabled_providers` / `disabled_providers` 过滤；`disabled_providers` 优先。省略 `options.apiKey` 时 provider 请求按 provider id 尝试 Intatis auth JSON 与当前 Intatis-owned OpenCode-compatible config，不再回落 OS Keychain。
- **Judge model route**：macOS 与 modern CLI 读取 canonical 顶层 `judge_model` 作为未来 fresh Cowork
  `@judge` 的 exact provider/model route；省略字段时继承顶层 `model`，显式字段无法解析时在 fresh
  session durable state 创建前 fail closed。该 route 不改变 Chat selection、Main 或 reviewer，
  配置变化不回填、不重绑定已有 session。Judge profile revision 与 leases 在 fresh 十事件 batch 中
  独立持久化。
