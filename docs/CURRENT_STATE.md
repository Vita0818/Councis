...
- 当前根工作树已整体替换为 `/Users/vita/Vitemis/Intatis` 的干净提交
  `120eda64fcb098f1bdc4852fee886450e80b3722`（标题 `v0.54`，tree
  `7fe2842aeec8fa08bec80e34342f971dc4226dcd`）。提交标题不是产品版本事实源；
  `project.yml` 仍定义 `0.48 (48)`。
- 这是直接落在 Councis 根目录的唯一活跃实现，不存在并列 `Intatis/`、`Upstream/Intatis`
  或参考树/实现树双轨。根 `.git` 和 Councis 六份控制/覆盖文档保留，其余 tracked 内容来自
  固定 source tree；边界见 `docs/INTATIS_BASELINE.md`。
- 本次整体替换有意没有把旧 Councis 源码差异三方合并回来。2026-08-15 按用户两次明确授权
  重新实现了用户 UI 控制面的品牌覆盖，以及 macOS Cowork-only 可见入口、fresh Cowork 固定
  `@judge`、`judge_model`、Main/Judge prompt 与 Cowork Skill。Chat/Code 仅隐藏，源码、session
  数据与 runtime 未删除。内部 target/module/type、bundle ID、可执行文件、App 图标资源名、CLI
  名称、配置/存储/日志/协议命名空间仍为 Intatis。
- `docs/COUNcis_IDENTITY.md` 记录当前有限产品差异与不可扩大的边界。未经用户明确要求，不得从
  旧 Git 历史机械恢复其他代码，也不得给 Judge 增加控制面、UI 或自动编排 authority。
- 基线替换后旧 `.build`、`.swiftpm` 与生成的 `Intatis.xcodeproj` 已清除；随后已从当前
  `project.yml` 重新生成工程，并在 Councis 根工作树重新执行 SwiftPM build、Judge/config/prompt
  定向测试与 `IntatisMac` unsigned Debug build。下方更早的安装、公证和环境证据仍属于 Intatis
  来源记录，不能冒充本次 Councis 发行验证。

## Intatis 来源状态（继承记录；Councis 定向复验另见“最近验证状态”）

- 业务内容来源 HEAD 为标题 `v0.54` 的固定提交 `120eda64fcb098f1bdc4852fee886450e80b3722`；
  Councis 保留自己的 Git 历史，因此 Councis 当前 `HEAD` 不等于来源提交。仓库没有以提交标题
  作为产品版本事实源，`project.yml` 把当前产品基线定义为 `0.48 (48)`。
...
macOS 继承 Chat、Code、Cowork、Settings 和本地诊断导出实现；Councis 当前根界面只显示
Cowork history/New 与 Settings，初始选择固定为 Cowork。Chat/Code/模式导航只是隐藏，以下能力与
历史兼容仍在源码中保留。
...
- brand-new GUI/CLI Cowork 在任何模型请求前，以一个 settings-first 十事件 EventLog batch 原子登记
  `@main`、`@judge` 与 `@permission-reviewer` 各自的 workspace/capability lease 和 identity。三者共享
  canonical primary workspace，但 exact inference binding、identity 与 lease 都独立。Judge 固定为
  read-only ordinary data-plane agent、`coordinationDepth=0`：不可 spawn/remove/rebind，不获得
  coordinator/run-control 工具，也不进入省略目标/auto delegation；Main 可通过既有显式
  delegation/message/ask 路径使用它。既有非空历史 session 不自动补写 Judge，已持久化 Judge 按普通
  roster 恢复。该实现复用既有 event types，未增加 session/EventLog schema 字段。
- macOS/modern CLI 高级配置已接入 canonical 顶层 `judge_model` 与
  `permission_reviewer_model`；两者只接受 enabled provider 的已配置 `<provider>/<model-id>` base
  profile，均没有新增 UI。任一字段缺失时只在配置解析层一次性继承同一 JSON 文档的顶层 `model`；
  兼容来源缺失/未知、显式空值、错误类型、未知/禁用/不可解析 route 或整份已选配置损坏/不可读均
  fail closed，不会回退 UI selection、Cowork session default、live/historical `@main` 或后续 rebind。
  fresh bootstrap 分别冻结 Judge 与 reviewer exact binding；每个权限审查 generation 仍从 reviewer
  binding fresh-resolve provider wrapper。GoalVerifier 继续冻结首个可解析的 exact `@main` binding，
  三种职责互不替代。
- Cowork runtime manifest 与 model-facing Cowork prompt 使用 Councis 名称；Code prompt 仍保持 Intatis
  基线。exact `@main` prompt 与 bundled `cowork-agent-orchestration` Skill 只建议在确有收益时取得多个
  独立候选、再显式交给 Judge 比较/选择/改写/综合；exact Judge prompt 明确其报告只是 Main 的候选
  证据，不是 Permission Reviewer、GoalVerifier、run authority 或最终产品决定。该策略不是强制
  spawn 数量、自动调度或新的运行时工作流。
...
- macOS/iOS 用户可见 App 名和 UI 品牌已覆盖为 `Councis`；系统麦克风说明、iOS Settings bundle、
  MCP 授权/批准界面与本地诊断导出说明同步使用 Councis。该覆盖不改变内部 Intatis 工程身份、
  配置/持久化命名空间、CLI 或运行时合同。
...
- 2026-08-15 Councis 有限产品面与固定 Judge 定向验证：`xcodegen generate`、
  `scripts/check-version-consistency.sh`（`0.48 (48)`）和 `swift build` 均通过；
  `ContextProjectionTests` 24/24、`AutomaticPermissionReviewTests` 42/42、
  `CLIProviderAdapterTests` 15/15、`ThreadLayoutTests` 22/22、`CLIAttachmentTests` 4/4，
  `IntatisSkillsTests` 29/29，全部 0 failures。Cowork orchestration Skill 经
  `.agents/skills/intatis-skill-creator/scripts/quick_validate.py` 校验为 valid，并由上述
  Main → explicit Judge 路由、fixed-role 边界和 prompt 测试完成 realistic exercise。
  `IntatisMac` macOS Debug unsigned build 与 `IntatisiOS` generic Simulator Debug unsigned
  build 均通过；没有构建遗留 `IntatisMacAppStore`。未运行整仓 `swift test`、真实 provider /
  credential / network、GUI / Computer Use、真机、签名、公证或发行 smoke，不能把定向离线
  验证外推为这些环境已经通过；详见 `docs/TESTING.md`。
