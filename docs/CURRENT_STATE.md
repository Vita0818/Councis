...
- 来源提交标题为 `v0.37`，但 commit 标题不是产品版本事实源；`project.yml` 把当前产品基线
  定义为 `0.36 (36)`。
...
macOS 仍继承完整的 Chat、Code、Cowork 实现、Settings 和本地诊断导出；Councis 当前根界面只
呈现 Cowork、Cowork Recent/New 与 Settings，Chat/Code 的模式入口不显示。

- 根 `NavigationSplitView` 的窗口初始选择是 Cowork；侧栏不显示任何模式按钮，`Recent` 只列出
  Cowork sessions，旁边的 `+` 继续先选择主 workspace，再走既有 Cowork session 初始化。
...
- fresh Cowork 的 `@judge` 使用 primary workspace 的独立 read-only workspace lease 和普通只读
  worker capability lease，`coordinationDepth=0`；它是可选择、可显式委派的 ordinary data-plane
  agent，但固定身份不能由 Main attach/spawn/remove，也不会被 `delegate_task(to:auto)` 选中。
  顶层 canonical `judge_model` 选择其 exact binding；缺失时继承配置 `model`，显式无效值使 fresh
...
- Chat/Code/Cowork 都从稳定 `TurnID` 和结构化事件投影 UI。Councis macOS 根窗口的可见选择固定
  为 Cowork；runtime 仍由进程级 `AppSessionRuntimeManager` 按 exact session key 持有。
...
- macOS 底层 Chat/Code/Cowork transcript history 均使用最多 16-row eager page 与显式分页，避免
  旧的 rich + lazy session-entry layout cycle；Councis 当前根侧栏只投影 Cowork session history。
  旧性能数字只保留在 Git/report 历史，不是当前 release readiness 证明。
...
- 2026-08-06 fixed `@judge` / `judge_model`：`IntatisCoworkTests` 322/322、
  ...
  `IntatisSkillsTests` 29/29；`xcodegen generate` 与 `IntatisMac` macOS Debug unsigned build 通过。
  未运行完整无 filter 的 `swift test`、iOS build、真实 GUI session 或线上 Judge provider smoke。
- 2026-08-06 Councis 可见品牌覆盖：plist、`.strings` 与 string catalog 格式检查通过，
  `xcodegen generate` 和版本一致性检查通过；`IntatisMac` macOS Debug 与 `IntatisiOS` generic
  Simulator Debug unsigned build 通过。两个最终 bundle 的 `CFBundleDisplayName` 均为 `Councis`，
  bundle ID 仍分别为 `com.Vita0818.IntatisMac` 与 `com.Vita0818.Intatis`；CLI 源码静态 parse
  通过。本轮未完成 SwiftPM build/test：受限沙箱禁止写 Clang module cache，沙箱外重试因当前
  Codex 用量限制未获执行权限。
